...
if (-not $Quarantine) {
    $Quarantine = Join-Path $repoRoot 'downloads\phase-two\quarantine\green-ridge-s2-background-extended-v0.1.0'
}
if (-not $RawParent) {
...
try {
    ...
    & $PythonExecutable -m burnlens.acquire_green_ridge_background_optical `
