...
if (-not $PythonExecutable.StartsWith($repoPrefix, [System.StringComparison]::OrdinalIgnoreCase)) {
    [Console]::Error.WriteLine('WARD_CREEK_U02_WRAPPER_FAILURE; reason=PYTHON_OUTSIDE_REPOSITORY')
    exit 3
}
if (-not (Test-Path -LiteralPath $PythonExecutable -PathType Leaf)) {
    [Console]::Error.WriteLine('WARD_CREEK_U02_WRAPPER_FAILURE; reason=PYTHON_NOT_FOUND')
    exit 3
}

try {
    & $PythonExecutable -m burnlens.acquire_replacement_event_optical `
        --repository-root $repoRoot `
        --generated-at-utc $GeneratedAtUtc `
        --preflight-only
    if ($LASTEXITCODE -ne 0) {
        throw [System.InvalidOperationException]::new('WARD_CREEK_U02_PREFLIGHT_FAILED')
    }
    ...
    if ($cdse -isnot [System.Management.Automation.PSCredential]) {
        throw [System.InvalidOperationException]::new('WARD_CREEK_U02_CDSE_CREDENTIAL_INVALID')
    }
    ...
    if ($transactionExitCode -ne 0) {
        ...
        & $PythonExecutable -m burnlens.acquire_replacement_event_optical `
            ...
            --verify-only
        if ($LASTEXITCODE -ne 0) {
            throw [System.InvalidOperationException]::new('WARD_CREEK_U02_FINAL_VERIFY_FAILED')
        }
        $wrapperExitCode = 0
    }
}
catch {
    [Console]::Error.WriteLine("WARD_CREEK_U02_WRAPPER_FAILURE; type=$($_.Exception.GetType().Name)")
    $wrapperExitCode = 3
}
finally {
    Remove-Item Env:BURNLENS_CDSE_USERNAME -ErrorAction SilentlyContinue
