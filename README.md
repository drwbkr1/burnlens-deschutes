...
BurnLens is not yet model-bearing portfolio ready. The BurnLens 0.51.0 milestone candidate replaces Darlene with Ward Creek in the exact six-event pool. [Open the passing sufficiency surface](samples/labels/readiness/phase-two/SIX-EVENT-DATASET-SUFFICIENCY-2026-002.html) or [read the decision](docs/phase-two/objective-five/REPLACEMENT_SIX_EVENT_DATASET_SUFFICIENCY_DECISION.md). All ten required non-count gates pass, and 54 whole-event 2/2/2 assignments satisfy the frozen transfer, program, and regime rules. This pass authorizes only a separate dataset, split, QA, and baseline checkpoint. Dataset, split, baseline, U-Net, evaluation, and model-to-GEOINT integration remain absent.
...
[Open the local portfolio reviewer experience](portfolio/BURNLENS-PORTFOLIO-REVIEWER-EXPERIENCE-2026-001.html), then use the [two-minute quickstart](portfolio/README.md).
...
The latest verified repository release remains `v0.50.0-six-event-dataset-sufficiency`. P2O4-T39 merged through [PR #557](https://github.com/drwbkr1/burnlens-deschutes/pull/557) at `7f7b332...`, but its first fresh-main Windows checkout exposed exact-byte drift and withheld the v0.51 tag. [BL-EXC-003 / PR #559](https://github.com/drwbkr1/burnlens-deschutes/pull/559) preserves one intentional CRLF-bound precheck and enforces LF for every other affected text path. Its candidate passes an independent fresh clone, 25 focused tests, 658 full-suite tests plus 86 subtests, exact six-output replay, deterministic 993,480-byte packaging, and a fresh isolated 99-command runtime. Corrected-main repetition and annotated-tag verification remain pending. The analytical result is unchanged: McKay, Tepee, Green Ridge, Grandview, Windigo, and Ward Creek; 531 unknown-ring exclusions; 54 valid assignments; and `training_authorized=false`.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; the latest verified repository tag is `v0.48.0-portfolio-reviewer-experience`. Tag object `c6ed831c106da44bad3ced010fafe5abc46bd537` remotely peels to checkpoint `05140217066277b254e78abb74cd8f61295449d0`. Annotated `v0.21.0-current-reference-inventory` remains preserved as failed-release audit evidence.
...
| 2 | One legally usable, versioned, leakage-resistant data/label/baseline foundation can support a defensible model-or-stop decision. | Active; candidate v0.4 contains 12 balanced prototype regions across Darlene, McKay, Tepee, Green Ridge, Grandview, and Windigo. The full sufficiency evaluator blocks on source-regime split fitness, so the accepted dataset, split, baseline, and model-readiness decision remain closed. |
