...
The project has shipped verified Phase Two burn-scar target-decision, real-pixel optical-pair, pair-local content-registration, five-state label-proposal, and cross-event metadata-feasibility baselines. BurnLens 0.11.0 is the candidate exact-source checkpoint: all four frozen Tepee/McKay archives are registered and rendered, with Tepee exclusions binding. It still creates no dataset, split, or model result.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; shipped Phase Two evidence now runs through verified `v0.10.0-cross-event-feasibility-baseline`.
...
- P2O4-T03 / issue #361 is the `0.11.0` candidate. Acquisition run `BL-2026-07-16-cross-event-optical-intake-r005` registers four exact archives / 4,551,170,756 ignored local bytes. Source-fitness run `BL-2026-07-16-cross-event-source-fitness-r005` accepts McKay at 100% pair-eligible quality and 3/3 registration windows, while Tepee retains 8.3441% review-needed, 5.1341% excluded, one excluded window, and one review-needed window. No label or dataset is created.
...
- The local pipeline exercised the authorized credentials without recording secrets, tokens, cookies, signed URLs, or credential-store details. Existing source/observation/optical packages and the four-archive 4,551,170,756-byte cross-event package are retained only in ignored local raw storage; zero raw provider bytes are committed. The repository contains one bounded label proposal, QA evidence, and exact source-fitness evidence, but no accepted dataset, split, baseline output, trained model, analytical metric, application demonstration, deployment, or public analytical result.
- The latest shipped repository evidence baseline remains analytical merge `5bfa1527410e98d8034b35ad68f6c50d5a1ec628` from issue #357 / PR #358 and verified `v0.10.0-cross-event-feasibility-baseline`. Issue #361 is the fully rendered `0.11.0` candidate at artifact commit `fe1136cad90bf85bd2ce5843a413a7b401196bf7`; PR/merge/tag gates remain pending.
...
- Corrected run `BL-2026-07-14-target-decision-r002` activates the fallback target without creating label pixels; `r001` is preserved as pre-remediation evidence. Optical, registration, proposal, and QA runs are the shipped one-event evidence. Cross-event metadata runs freeze the whole-event groups; acquisition `BL-2026-07-16-cross-event-optical-intake-r005` and source-fitness `BL-2026-07-16-cross-event-source-fitness-r005` prove the exact local pixels with exclusions while dataset construction remains deferred.
