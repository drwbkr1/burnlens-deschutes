...
BurnLens now has model-bearing evidence, but it is not yet portfolio-submission ready because the GEOINT application path remains. P2O5-T03 / issue #562 creates and independently audits `burnlens-dataset-v0.1.0`, locks `burnlens-whole-event-split-v0.1.0`, and evaluates preregistered baselines. RBR scores 1.0 Dice and IoU on the 89 frozen selected test cores under one sealed opening. P3O1-T01 / issue #566 then trains the one authorized CPU U-Net and consumes its Ward Creek/Windigo opening once. [Open the test surface](samples/evaluation/phase-three/bounded-unet-test-v0.1.0/BOUNDED-UNET-TEST-EVALUATION-2026-001.html): the model predicts all 89 cores as burned and scores event-class macro Dice `0.29874213836477986`, IoU `0.21474358974358976`, and worst-event Dice `0.2641509433962264`. [Open the exact Phase Three decision](samples/model-packages/burnlens-unet-binary-v0.1.0/PHASE-THREE-MODEL-DECISION.html): the 35-epoch replay and weights reproduce exactly, but the model is rejected as the analytical winner. `burnlens-unet-rejected-package-v0.1.1` preserves the model card, inference contract, weights, evaluation lineage, and limitations. RBR remains the accepted analytical method; the U-Net may appear only as a visibly rejected diagnostic. This is not ground truth, generalization, model superiority, or an operational product.

The verified August 6 ZIP is an interim contingency technical case study. It is preserved and usable for that limited purpose, but it is not the intended final BurnLens submission. The trained-U-Net and reproducible-evaluation parts of the minimum threshold now exist; an inspectable GEOINT application path still does not.

[Open the current local portfolio reviewer experience](portfolio/BURNLENS-PORTFOLIO-REVIEWER-EXPERIENCE-2026-007.html), then use the [two-minute quickstart](portfolio/README.md).
...
[Use the verified August 6 submission ZIP](portfolio/submission/BURNLENS-AUGUST-6-SUBMISSION-2026-001.zip) for the one-step extract-and-open handoff. P2O4-T37 / issue #544 / PR #545 binds it to exact verified public evidence; deterministic replay, safe extraction, real desktop/narrow Chrome, fresh-main suite, package, isolated-install, and remote-tag gates pass.

[Use the verified final submission handoff](portfolio/submission/SUBMISSION-HANDOFF.md). P2O4-T38 / issue #548 / PR #549 repeats the exact bundle as a recipient and passes every material integrity, render, navigation, claims, rights, privacy, comprehension, and exact-main records gate.

## Verified status and current work

The latest verified repository release is `v0.53.0-bounded-unet-rejected-model`. P3O1-T01 / issue #566 / PR #567 is merged at `5a0f8ac027ae20ff9193948506f590afbfb64554`; annotated tag object `d66dd7fdc9f2f155d1799cb1e85e77dadd76311a` peels exactly to that checkpoint. Fresh main passes 729 tests with one expected skip and 86 subtests. Two independent 1,106,162-byte wheels match the reviewed candidate at SHA-256 `15b8b84f...`, and an isolated CPython 3.12.10 model runtime passes all 107 command-help probes. The package retains the unchanged 479,573-byte weights at SHA-256 `703d9257...`; 15 exact lineage bindings; and eight tracked files / 583,992 bytes at inventory SHA-256 `ce39c41c...`. RBR remains the accepted analytical method; the rejected U-Net remains diagnostic only. No GitHub Release, deployment, georeferenced inference, or final-submission-ready claim exists.
...
The [aggregate Windigo intake report](samples/labels/review/windigo/phase-two/intake/WINDIGO-OWNER-RESPONSE-INTAKE-2026-001.html) is the latest accepted-prototype-label interface; its original-resolution PNG and owner-confirmed desktop/narrow HTML render pass. BL-GOV-003 / issue #525 adopts [`checkpoint-policy-v0.1.0`](docs/governance/CHECKPOINT_POLICY.md). The verified [Petes Lake material-defer decision](docs/phase-two/objective-four/PETES_LAKE_MATERIAL_DEFER_DECISION.md) preserves the complete chain: U01/U02 pass; [planned U03](samples/cross-event/phase-two/petes-lake/PETES-LAKE-SOURCE-FITNESS-2026-001.png) visibly fails on snow; the exact 19 October replacement passes [native-pixel source fitness](samples/cross-event/phase-two/petes-lake/PETES-LAKE-REPLACEMENT-SOURCE-FITNESS-2026-001.png) with explicit spatial exclusions; and U04 passes the [terms-first MTBS native contract](samples/cross-event/phase-two/petes-lake/PETES-LAKE-REFERENCE-NATIVE-CONTRACT-2026-001.json) while accepting zero reference pixels. U05 r001-r003 remain immutable. Final r003 promotes seven ordered NWI assets, then its only Data Source pre-count request observes HTTP 500 at `PROVIDER_OPEN`; four assets remain unexecuted. R003 is terminal, no r004 is authorized, and no partial package may satisfy U05. Production U06-U10 remain unexecuted/deferred, so Petes Lake does not become event six. Historical Petes runs remain pinned to the exact 0.44.0 source state that generated them; verified 0.45.0 versions the material packaged custody, source-fitness, review-batching, localhost, and geospatial-environment capabilities without changing the accepted label set, dataset, split, baseline, or model.

U11R1 retains `9f440999...` as failed candidate evidence because its installed U04 replay stopped before output; correction `fcbaa4c...` repairs descendant trace enforcement without changing the 32,991-byte U04 evidence or material-defer decision. PR #527 merged reviewed head `925b660...` through true merge `7d5c07e...`; the first merged base-runtime wheel then loaded only 80 of 81 commands and opened BL-EXC-001 before tagging. Exception U03 passes at package-sensitive code `4bf76ea...`: all 81 commands load and show help, lean `dev` and full `geo-research` profiles pass their exact suites with 81 subtests, installed-but-unusable optional geo imports fail with sanitized profile guidance, `uv.lock` is fresh-checkout stable, all three pipeline products remain byte-identical, and two fresh clones reproduce the corrected 801,906-byte wheel / SHA-256 `8e17c18a...`. The earlier 801,911-byte / `055e56b1...` wheel remains superseded pre-review evidence. Exception U04 passes through PR #529 and merge `d65c24f...`; annotated tag object `0b466402cdc36b0eacfff97d29dd16fe2a88868a` remotely peels exactly to that verified checkpoint. No GitHub Release object or deployment exists.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; the latest verified repository tag is `v0.53.0-bounded-unet-rejected-model`. Tag object `d66dd7fdc9f2f155d1799cb1e85e77dadd76311a` peels exactly to merge `5a0f8ac027ae20ff9193948506f590afbfb64554`. BurnLens 0.53.0 versions the exact trained, evaluated, reproducible, and rejected U-Net while keeping RBR as the accepted analytical method.
- BL-GOV-002 / issue #400 reconciles obsolete Phase One GitHub backlog against current repository truth. Historical issues, PR bodies, and control artifacts remain audit evidence; they do not represent active work or narrow the controlling execution goal. BL-GOV-003 / issue #525 activates the [checkpoint policy](docs/governance/CHECKPOINT_POLICY.md) without changing software or scientific state. The [single-reviewer path decision](docs/phase-two/objective-four/SINGLE_REVIEWER_PATH_DECISION.md) and its 6/0/50 result remain historical evidence. Issue #521 / PR #527 and BL-EXC-001 issue #528 / PR #529 close the verified Petes Lake material-defer release. P2O4-T34 then defers McKenzie and Milli without acquisition. Final r003 remains immutable, and T35 may not create r004, reopen either deferred route, or reuse partial custody. Reaching six events remains necessary but cannot by itself bypass dataset fitness.
...
- P2O4-T33 / issue #521 / PR #527 plus BL-EXC-001 / issue #528 / PR #529 ship verified BurnLens 0.45.0 at checkpoint `d65c24f59ce0c854ba230aa977eaf718d881d952`; lifecycle issue #530 / PR #531 records reconciliation. The milestone preserves the complete Petes optical/reference/NWI failure chain, contains bounded custody/review/environment capabilities, and stops before candidate generation when final r003 cannot complete source-project custody. [Read the material-defer decision](docs/phase-two/objective-four/PETES_LAKE_MATERIAL_DEFER_DECISION.md). U11 remains `defer` while execution is `complete` and release is `verified`; Petes Lake is not event six, the five-event prototype label set is unchanged, and no dataset, split, baseline, or model exists.
- P2O4-T34 / issue #532 / PR #533 ships verified BurnLens 0.46.0. [Open the exact source-gate report](samples/reference/phase-two/OFFICIAL-FALLBACK-SOURCE-GATE-2026-001.html) or [read the decision](docs/phase-two/objective-four/OFFICIAL_FALLBACK_SOURCE_GATE_DECISION.md). McKenzie and Milli remain separately blocked and unselected; `provider_bytes_authorized` is false. U01/U04 pass, U02/U03/U05 defer, fresh-main gates pass, and the remote annotated tag peels exactly to the merge.
- P2O4-T35 / issue #534 / PR #535 plus BL-EXC-002 #536 / PR #537 ships verified BurnLens 0.47.0. U01-U06 pass the exact Windigo source, custody, fitness, proposal, owner-custody, two-class promotion, owner-confirmed render, corrected-main, reproducible-package, isolated-install, and public-claim/privacy gates. Windigo is event six while dataset, split, baseline, and model versions remain null. Lifecycle issue #538 hands off directly to the August 6 portfolio critical path.
- P2O4-T36 / issue #540 / PR #541 ships verified BurnLens 0.48.0 as one local/offline reviewer path. [Open the page](portfolio/BURNLENS-PORTFOLIO-REVIEWER-EXPERIENCE-2026-001.html) or [read the decision](docs/phase-two/objective-four/PORTFOLIO_REVIEWER_EXPERIENCE_DECISION.md). Exact outputs, real rendered QA, full suite, package, isolated install, fresh-main repetition, and remote tag peel pass.
- P2O4-T37 / issue #544 / PR #545 ships verified BurnLens 0.49.0 as one deterministic August 6 extract-and-open bundle. [Open the submission folder](portfolio/submission) or [read the decision](docs/phase-two/objective-four/AUGUST6_SUBMISSION_BUNDLE_DECISION.md). Exact ZIP/receipt replay, safe extraction, owner-confirmed rendering, 586-test fresh main, reproducible wheel, isolated install, 88 commands, and remote tag peel pass. Final submission-readiness review is next; dataset, split, baseline, model, deployment, access, and public-sharing state remain unchanged.
- P2O4-T38 / issue #548 / PR #549 verifies final submission readiness at main `c1e91637f39bc84bd5a8c08440a0117ec4173bd6`. It re-extracts the exact v0.49 ZIP, walks the strongest-result path, passes 9/9 destinations plus claims/rights/privacy checks, retains non-material browser limitations, and changes no bundle byte or analytical/public-sharing state. External submission remains an owner action.
...
- The local pipeline exercised the authorized credentials without recording secrets, tokens, cookies, signed URLs, or credential-store details. Exact provider archives remain in ignored local custody; zero raw provider bytes are committed. The repository now contains the accepted prototype dataset, split, QA, baseline, one trained and evaluated U-Net, and local/offline review and portfolio surfaces. The U-Net is rejected as the analytical winner; no georeferenced inference output, integrated analytical application, deployment, generalization, or public operational result exists.
...
| 2 | One legally usable, versioned, leakage-resistant data/label/baseline foundation can support a defensible model-or-stop decision. | Accepted and verified in BurnLens 0.52.0: dataset v0.1.0, whole-event split v0.1.0, independent QA, baseline v0.1.0, and an explicit rejection-first U-Net authorization. |
| 3 | One bounded model either adds reproducible value beyond the strongest baseline or is rejected honestly. | Accepted and verified in BurnLens 0.53.0: one exact U-Net is trained, evaluated once, reproduced byte-for-byte, and rejected against RBR; package v0.1.1 preserves the result. |
| 4 | The accepted model or baseline can become a valid, reproducible, georeferenced run and evidence interface. | Ready to begin: RBR-primary analytical raster plus the rejected U-Net as diagnostic evidence. |
...
The executable surface now includes fail-closed access validation, deterministic AOI evidence, exact-pair transaction rehearsal, secret-safe authenticated acquisition, registered-package re-verification, real Sentinel/VIIRS source inspection, bounded NOAA-21 observation-geometry screening, deterministic target-decision rendering, exact optical-pair inspection, pair-local content registration, five-state proposal and cross-event transfer paths, controlled human review, dataset/split materialization, independent dataset QA, preregistered baseline evaluation, and model-readiness auditing. It is not a trained segmentation model or operational application.
