...
The latest verified release is `v0.43.0-grandview-owner-review-surface`. It turns the two exact unreviewed Grandview cores and 98 unknown-ring pixels into a blank, offline yes/no/uncertain owner-review surface. The verified surface records zero responses and creates no labels. The cumulative prototype set remains balanced at eight approved regions across four events; dataset, split, baseline, and model work remains blocked below six events.

Issue #511 / PR #518 ships the BurnLens 0.43.0 surface at checkpoint `aeea6f5cc488ae975badbdf654d0164570db77c4`. Fresh merged main preserves all six artifacts, passes 313 tests with 17 expected custody skips, and rebuilds the canonical clean-clone package twice. Issue #519 / PR #520 records that lifecycle.

The active BurnLens 0.44.0 release candidate under issue #517 locks the exact completed Grandview response before reveal and admits one burned plus one background region only after every non-owner gate passes. The candidate v0.3 label set is balanced at ten regions, 236 core pixels / 9.44 ha, and 431 excluded unknown-ring pixels across five complete events. The [aggregate intake report](samples/labels/review/grandview/phase-two/intake/GRANDVIEW-OWNER-RESPONSE-INTAKE-2026-001.html) withholds unit decisions and notes. The owner-confirmed local HTML render passes; repository merge, fresh-main, tag, and lifecycle gates remain pending. Issue #521 records Petes Lake as the sixth event, not an automatic dataset unlock.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; the latest verified repository tag is `v0.43.0-grandview-owner-review-surface`. Tag object `d679521edcc1d5b1bfd3e8022036cae06a9f1978` remotely peels to checkpoint `aeea6f5cc488ae975badbdf654d0164570db77c4`. Annotated `v0.21.0-current-reference-inventory` remains preserved as failed-release audit evidence.
- BL-GOV-002 / issue #400 reconciles obsolete Phase One GitHub backlog against current repository truth. Historical issues, PR bodies, and control artifacts remain audit evidence; they do not represent active work or narrow the controlling execution goal. The [single-reviewer path decision](docs/phase-two/objective-four/SINGLE_REVIEWER_PATH_DECISION.md) and its 6/0/50 result remain historical evidence. Issue #443 / PR #445 plus remediation #446 / PR #447 ship verified prototype-label sufficiency; issue #449 / PR #451 ships bounded remediation planning; issue #453 / PR #455 ships the no-promotion pilot. The active next scientific checkpoint is Petes Lake under issue #521.
...
- P2O4-T31 / issue #511 / PR #518 ships verified BurnLens 0.43.0 at checkpoint `aeea6f5cc488ae975badbdf654d0164570db77c4`. [Open the blank Grandview owner-review surface](samples/labels/review/grandview/phase-two/GRANDVIEW-OWNER-REVIEW-SURFACE-2026-001.html) or [read the decision](docs/phase-two/objective-four/GRANDVIEW_OWNER_REVIEW_SURFACE_DECISION.md). It binds the exact v0.42 proposal, renders optical/reference/context evidence for both regions, accepts only yes/no/uncertain, and records zero responses or labels. Issue #519 / PR #520 records the lifecycle; at that checkpoint issue #517 owned the still-unreceived export, which the separate T32 candidate below now locks and evaluates.
- P2O4-T32 / issue #517 is the BurnLens 0.44.0 release candidate. [Open the aggregate Grandview intake report](samples/labels/review/grandview/phase-two/intake/GRANDVIEW-OWNER-RESPONSE-INTAKE-2026-001.html) or [read the decision](docs/phase-two/objective-four/GRANDVIEW_OWNER_RESPONSE_INTAKE_DECISION.md). It pins the exact response/receipt plus 22 evidence records, accepts one region per class, and moves the cumulative prototype set to five complete events. The owner-confirmed local HTML render and all pre-merge verification pass; repository publication remains pending.
