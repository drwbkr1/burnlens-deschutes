...
BurnLens now has one verified baseline-primary CV-to-GEOINT release. P2O5-T03 / issue #562 creates and independently audits `burnlens-dataset-v0.1.0`, locks `burnlens-whole-event-split-v0.1.0`, and evaluates preregistered baselines. RBR scores 1.0 Dice and IoU on the 89 frozen selected test cores under one sealed opening. P3O1-T01 / issue #566 then trains the one authorized CPU U-Net and consumes its Ward Creek/Windigo opening once. [Open the test surface](samples/evaluation/phase-three/bounded-unet-test-v0.1.0/BOUNDED-UNET-TEST-EVALUATION-2026-001.html): the model predicts all 89 cores as burned and scores event-class macro Dice `0.29874213836477986`, IoU `0.21474358974358976`, and worst-event Dice `0.2641509433962264`. The model is reproducible and rejected as the analytical winner; RBR remains the accepted analytical method.

[Open the Phase Four evidence interface](samples/runs/phase-four/burnlens-geoint-evidence-interface-v0.1.0/PHASE-FOUR-EVIDENCE-INTERFACE-2026-001.html). It turns exact Ward Creek RBR output into native-grid rasters, accepted vectors, bounded official context, deterministic spatial observations, visible false-positive-risk evidence, and a self-contained local application. The rejected U-Net is off by default and appears only as labeled diagnostic hatching. [Inspect the immutable run package](samples/runs/phase-four/burnlens-ward-creek-rbr-run-v0.1.0/README.md) or use the exact [ZIP](portfolio/phase-four/BURNLENS-WARD-CREEK-RBR-RUN-2026-001.zip). This is not ground truth, generalization, model superiority, field validation, official information, or an operational product.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; the latest verified repository tag is `v0.54.0-rbr-geoint-milestone`. It adds the RBR-primary geospatial run, repository-owned interface, and immutable package without changing the accepted method or promoting the rejected U-Net.
...
| 4 | The accepted model or baseline can become a valid, reproducible, georeferenced run and evidence interface. | Accepted and verified in BurnLens 0.54.0: exact RBR-primary Ward Creek run, rejected U-Net diagnostic, native rasters/vectors, bounded context, self-contained interface, and immutable package pass U01-U08. |
| 5 | The integrated system is reproducible, accessible, secure, failure-visible, performant, and reversible. | P5O1-T01 / issue #574 has frozen the baseline-first 0.55.0 candidate with U01-U06 evidence; milestone merge, fresh-main, and tag verification remain pending. |
