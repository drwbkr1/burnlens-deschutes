...
The current release candidate is BurnLens `0.31.0` / P2O4-T19 / issue #461. It preserves one exact completed six-region owner response, reruns every promotion gate, and accepts three burned plus three background regions as explicitly owner-approved prototype labels. The 136 core pixels advance; all 246 ring pixels remain unknown and excluded. Three event groups remain below the required six, so no accepted dataset, split, baseline, model result, accuracy claim, or deployed analytical application exists.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; the latest verified repository tag is `v0.30.0-region-owner-review-surface`. Tag object `dc0b27e47a71353d8e1e7c8691dbf6b9c2a7598f` remotely peels to merge `674e9edb04a4016ef71cad805be52f4459670d6a`. Annotated `v0.21.0-current-reference-inventory` remains preserved as failed-release audit evidence.
...
- P2O4-T19 / issue #461 accepts the exact completed region response only after custody, reproducibility, source/terms, quality/registration, uncertainty, and event-leakage gates pass. [Open the aggregate intake report](samples/labels/review/regions/phase-two/intake/REGION-OWNER-RESPONSE-INTAKE-2026-001.html) or [read the decision](docs/phase-two/objective-four/REGION_OWNER_RESPONSE_INTAKE_DECISION.md). Six prototype regions pass; dataset fitness remains blocked at three event groups.
...
| 2 | One legally usable, versioned, leakage-resistant data/label/baseline foundation can support a defensible model-or-stop decision. | Active; P2O4-T19 accepts six owner-approved prototype regions after all gates pass: three burned and three background, 136 core pixels, and 246 excluded ring pixels. Three events remain insufficient against the six-event/source-regime gate. No accepted dataset, split, baseline, or model-readiness decision exists. |
