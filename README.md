...
The project has shipped verified Phase Two burn-scar target-decision, real-pixel optical-pair, pair-local content-registration, five-state label-proposal, cross-event metadata-feasibility, exact cross-event source-fitness, and cross-event label-transfer baselines. BurnLens 0.12.0 renders Tepee/McKay five-state and binary proposal rasters from exact sources and independently reproduces all 63,930 pixels. It still creates no accepted dataset, split, or model result.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; shipped Phase Two evidence runs through verified `v0.12.0-cross-event-label-transfer-baseline` at merge `9679e53783500c437de44fc0d033b64f0bacb0df`.
...
- The latest verified tag is `v0.12.0-cross-event-label-transfer-baseline`: annotated tag object `83a0371b9c7e75163b2e4ef5c6368103347740b4` remotely peels to merge `9679e53783500c437de44fc0d033b64f0bacb0df`. A fresh no-hardlink merged-main clone passes all 137 tests, exact ten-output manifest/LF/raw gates, compilation, dependency health, and the canonical fixed-epoch wheel hash; fresh real-source reconstruction reproduces all ten outputs byte for byte.
