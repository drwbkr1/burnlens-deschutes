...
The latest verified Phase Two baseline is P2O4-T12. Two exact USGS deliveries supply all seven accepted BAER/RAVG/MTBS products. BurnLens `0.24.0` verifies their bytes, terms, structure, grids, classes, and real rendered comparisons: current MTBS and RAVG affirm every McKay/Tepee burned proposal pixel and remain non-affirmative for all 55 McKay and 493 of 494 Tepee background candidates. Restricted thresholded Tepee BARC stays private and excluded. The historical 6 burned / 0 background / 50 ignored reconciliation remains immutable history; issue #432 will reopen all 56 units for owner yes/no/uncertain rather than inherit those exclusions. No owner responses, accepted label set, dataset, split, baseline, model result, or deployed analytical application exists.
