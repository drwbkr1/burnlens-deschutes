...
The latest verified baseline is P2O4-T15; P2O5-T01 is the current candidate. BurnLens `0.27.0` proves that the 24 owner-approved prototype points are not sufficient for a segmentation dataset: they cover 0.012662% of 189,541 candidate pixels, provide only one event per train/validation/test role, and confound event with reference regime. BurnLens therefore requires more contiguous reviewed coverage and event diversity. No accepted dataset, split, baseline, model result, accuracy claim, or deployed analytical application exists.
...
- P2O5-T01 / issue #443 assesses dataset and split sufficiency without creating either. [Open the sufficiency report](samples/labels/readiness/phase-two/PROTOTYPE-LABEL-SUFFICIENCY-2026-001.html) or [read the decision](docs/phase-two/objective-five/PROTOTYPE_LABEL_SUFFICIENCY_DECISION.md).
...
| 2 | One legally usable, versioned, leakage-resistant data/label/baseline foundation can support a defensible model-or-stop decision. | Active; exact proposal/QA, review/custody, seven-product fitness, owner surface, and 24 gated owner-approved prototype labels exist. Issue #443 must test their coverage and leakage-resistant split feasibility. No independent review, accepted dataset, split, baseline, or model-readiness decision exists. |
