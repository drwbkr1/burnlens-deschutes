...
The latest verified baseline is `v0.27.0-prototype-label-sufficiency`. Current P2O4-T16 / issue #449 turns its stop decision into a region-first owner-review and event-diversity plan. BurnLens will not buffer points or tile them into masks: it requires deterministic evidence-coherent regions, explicit unknown boundaries, at least six whole-event groups, and replicated source regimes before dataset fitness can reopen. No region label, accepted dataset, split, baseline, model result, accuracy claim, or deployed analytical application exists.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; the latest verified repository tag is `v0.26.0-owner-response-intake`. Tag object `fa199dd2e63ab3607b732d331c6ecdc898a227b4` remotely peels to merge `0f46ec5f1b4586f076a59beee163bbb8aee3b62a`. Annotated `v0.21.0-current-reference-inventory` remains preserved as failed-release audit evidence.
- BL-GOV-002 / issue #400 reconciles obsolete Phase One GitHub backlog against current repository truth. Historical issues, PR bodies, and control artifacts remain audit evidence; they do not represent active work or narrow the controlling execution goal. The [single-reviewer path decision](docs/phase-two/objective-four/SINGLE_REVIEWER_PATH_DECISION.md) and its 6/0/50 result remain historical evidence. Issue #443 / PR #445 plus remediation #446 / PR #447 ship verified prototype-label sufficiency; issue #449 owns bounded label-coverage remediation planning.
...
- P2O5-T01 / issue #443 ships verified `v0.27.0-prototype-label-sufficiency` without creating a dataset or split. [Open the sufficiency report](samples/labels/readiness/phase-two/PROTOTYPE-LABEL-SUFFICIENCY-2026-001.html) or [read the decision](docs/phase-two/objective-five/PROTOTYPE_LABEL_SUFFICIENCY_DECISION.md). Issue #449 owns the next contiguous-region and event-diversity plan.
- P2O4-T16 / issue #449 defines the candidate region-first owner review and six-event advancement gates without creating region labels or acquiring new pixels. [Open the region plan](samples/labels/readiness/phase-two/LABEL-REGION-REMEDIATION-PLAN-2026-001.html) or [read the decision](docs/phase-two/objective-four/LABEL_REGION_REMEDIATION_PLAN_DECISION.md).
...
| 2 | One legally usable, versioned, leakage-resistant data/label/baseline foundation can support a defensible model-or-stop decision. | Active; verified v0.27.0 proves the 24 reviewed points are insufficient. Candidate v0.28.0 specifies region-first review, explicit unknown boundaries, and six-event/source-regime gates. No region label, accepted dataset, split, baseline, or model-readiness decision exists. |
