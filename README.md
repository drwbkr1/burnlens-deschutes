...
The latest verified release baseline is `v0.30.0-region-owner-review-surface`. P2O4-T18 / issue #457 / PR #459 supplies an exact owner yes/no/uncertain surface for the six shipped regions, with bound optical/reference evidence, deterministic reconstruction, and zero responses or promoted labels. Fresh-main tests, package identity, exact reconstruction, responsive rendering, isolated install, and remote tag verification pass. Issue #461 owns exact owner-response intake. At least six whole-event groups and replicated source regimes are still required before dataset fitness can reopen. No region label, accepted dataset, split, baseline, model result, accuracy claim, or deployed analytical application exists.
