...
The latest verified release is `v0.31.0-region-owner-response-intake`. P2O4-T19 preserves one exact completed six-region owner response, reruns every promotion gate, and accepts three burned plus three background regions as explicitly owner-approved prototype labels. The 136 core pixels advance; all 246 ring pixels remain unknown and excluded. Three event groups remain below the required six, so no accepted dataset, split, baseline, model result, accuracy claim, or deployed analytical application exists. Issue #466 owns metadata-first selection of three additional comparable event groups.
