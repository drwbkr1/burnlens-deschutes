...
The latest verified release is `v0.31.0-region-owner-response-intake`. The current `0.32.0` release candidate from issue #466 / PR #469 freezes Green Ridge 2020, Grandview 2021, and Petes Lake 2023 as three additional metadata-only acquisition groups. Fresh remote `main` reconstructs its outputs exactly; issue #470 corrects only the canonical fresh-clone package binding before tagging. The planned pool now has six immutable event identities across six years and repeated MTBS/BAER/RAVG regimes, but zero new provider pixels have been downloaded. Source fitness, both classes, unknown boundaries, owner review, dataset, split, baseline, model result, accuracy, and a deployed analytical application remain absent.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; the latest verified repository tag is `v0.31.0-region-owner-response-intake`. Tag object `3ead929db9129da8da836373a177f2e5ba87c89b` remotely peels to merge `70dca180d851fadc9ae3b7a897bdf55cf698b3c3`. Annotated `v0.21.0-current-reference-inventory` remains preserved as failed-release audit evidence.
...
- P2O4-T20 / issue #466 / PR #469 builds the `0.32.0` release candidate; issue #470 remediates only its release-package binding. [Open the additional-event plan](samples/cross-event/phase-two/ADDITIONAL-EVENT-GROUP-PLAN-2026-001.html) or [read the decision](docs/phase-two/objective-four/ADDITIONAL_EVENT_GROUP_PLAN_DECISION.md). Six whole-event identities are now frozen at metadata level; exact pixel/source fitness remains the next scientific gate.
