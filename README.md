...
The latest verified repository release is `v0.51.0-replacement-six-event-sufficiency`. P2O4-T39 merged through [PR #557](https://github.com/drwbkr1/burnlens-deschutes/pull/557), then its first fresh-main Windows checkout exposed exact-byte drift and withheld the tag. [BL-EXC-003 / PR #559](https://github.com/drwbkr1/burnlens-deschutes/pull/559) preserves one intentional CRLF-bound precheck and enforces LF for every other affected text path. Corrected main `c39a2c5...` passes a new Windows clone, 25 focused tests, 658 full-suite tests plus 86 subtests, exact six-output replay, deterministic 993,480-byte packaging, and a fresh isolated 99-command runtime. Annotated tag object `61fad1ab...` peels exactly to that checkpoint. The analytical result is unchanged: McKay, Tepee, Green Ridge, Grandview, Windigo, and Ward Creek; 531 unknown-ring exclusions; 54 valid assignments; and `training_authorized=false`.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; the latest verified repository tag is `v0.51.0-replacement-six-event-sufficiency`. Tag object `61fad1ab87d2158b4a862f9866f72ac6ad70da7e` remotely peels to checkpoint `c39a2c543be2f7884eff264a733821f367f49776`. Annotated `v0.21.0-current-reference-inventory` remains preserved as failed-release audit evidence.
