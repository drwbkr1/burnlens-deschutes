...
The latest verified release is `v0.42.0-grandview-region-proposal`. The active BurnLens 0.43.0 candidate turns its two exact unreviewed cores and 98 unknown-ring pixels into a blank, offline Grandview yes/no/uncertain owner-review surface. The candidate records zero responses and creates no labels. The cumulative prototype set remains balanced at eight approved regions across four events; dataset, split, baseline, and model work remains blocked below six events.

Issue #508 / PR #512 supplies the BurnLens 0.42.0 analytical candidate. Fresh merged-main verification caught checkout-dependent JSON line endings and correctly withheld the tag. Issue #513 / PR #514 applies an LF-only checkout-contract remediation without changing evidence or candidate bytes. Verified checkpoint `6124f76de04b5e503a9da7655a293a023480b39c` preserves all five original hashes. Issue #511 now owns the active, separately versioned blank owner surface; issue #517 is blocked until that surface is verified and the owner supplies a completed export.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; the latest verified repository tag is `v0.42.0-grandview-region-proposal`. Tag object `388c81677b3c49d2312779f8fc98ad3dc7b9048c` remotely peels to checkpoint `6124f76de04b5e503a9da7655a293a023480b39c`. Annotated `v0.21.0-current-reference-inventory` remains preserved as failed-release audit evidence.
...
- P2O4-T31 / issue #511 is the active BurnLens 0.43.0 candidate. [Open the blank Grandview owner-review surface](samples/labels/review/grandview/phase-two/GRANDVIEW-OWNER-REVIEW-SURFACE-2026-001.html) or [read the decision](docs/phase-two/objective-four/GRANDVIEW_OWNER_REVIEW_SURFACE_DECISION.md). It binds the exact v0.42 proposal, renders optical/reference/context evidence for both regions, accepts only yes/no/uncertain, and records zero responses or labels. Issue #517 owns later exact response intake and remains blocked on a completed owner export.
