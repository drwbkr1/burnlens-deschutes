...
The project has shipped verified Phase Two burn-scar target-decision, real-pixel optical-pair, pair-local content-registration, five-state label-proposal, cross-event metadata-feasibility, and exact cross-event source-fitness baselines. BurnLens 0.11.0 registers and renders all four frozen Tepee/McKay archives with Tepee exclusions binding. It still creates no dataset, split, or model result.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; shipped Phase Two evidence now runs through verified `v0.11.0-cross-event-source-fitness-baseline`.
