...
The project has shipped verified Phase Two burn-scar target-decision, real-pixel optical-pair, pair-local content-registration, five-state label-proposal, cross-event metadata-feasibility, exact cross-event source-fitness, cross-event label-transfer, proposal-blinded label-review-readiness, isolated offline reviewer-handoff, live-browser reviewer-handoff QA, and first returned-response hash-lock baselines. BurnLens 0.16.0 preserves one real returned response and pre-reveal receipt only in ignored storage, while tracked JSON/HTML/PNG publishes completeness, cryptographic bindings, and the one-of-two response state. The active BurnLens 0.17.0 candidate proves that the historical first lock and a current-protocol software fixture can be verified together without treating the fixture as human evidence. Review content remains withheld, the reveal remains operator-declared unopened, and software does not qualify a reviewer. A second qualifying human response and adjudication remain required. No accepted label set, dataset, split, baseline, model result, or deployed analytical application exists.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; the latest verified tag is `v0.16.0-first-reviewer-response-lock` at corrected checkpoint `27fcd3eadb1473bb603b4275f986bf62022c10bf`.
...
- The latest verified tag is `v0.16.0-first-reviewer-response-lock`: annotated tag object `da94fc97efc07b07d9520022fdbff42a85e8ba00` remotely peels to corrected checkpoint `27fcd3eadb1473bb603b4275f986bf62022c10bf`. A fresh remote-main clone passes 158 tests, isolated compilation, Node syntax, dependency health, exact three-output reconstruction, manifest/privacy/link/LF gates, and the canonical 292,068-byte wheel / SHA-256 `c6a0f320b393ea7aca3aebdc93da97f7ed34901d30df298c1986d1ee4b78ee28`.
