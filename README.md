...
The latest verified baseline remains P2O4-T13 while P2O4-T12 bundle fitness is the current candidate checkpoint. Two exact USGS deliveries now supply all seven accepted BAER/RAVG/MTBS products. BurnLens `0.24.0` verifies their bytes, terms, structure, grids, classes, and real rendered comparisons: current MTBS and RAVG affirm every McKay/Tepee burned proposal pixel and remain non-affirmative for all 55 McKay and 493 of 494 Tepee background candidates. Restricted thresholded Tepee BARC stays private and excluded. The historical 6 burned / 0 background / 50 ignored reconciliation remains immutable history; the owner-confirmed yes/no/uncertain route will reopen all 56 units rather than inherit those exclusions. No owner responses, accepted label set, dataset, split, baseline, model result, or deployed analytical application exists.
...
| 2 | One legally usable, versioned, leakage-resistant data/label/baseline foundation can support a defensible model-or-stop decision. | Active; exact proposal/QA evidence, review/custody history, and a current seven-product bundle-fitness candidate exist. Exact delivery, terms, structure, and bounded product fitness pass with restricted Tepee BARC excluded. The historical 6/0/50 reconciliation is preserved, while the owner-confirmed route will reopen all 56 units with yes/no/uncertain and require source/reproducibility/quality/leakage gates after any yes. No independent review, owner-approved labels, accepted dataset, split, baseline, or model-readiness decision exists. |
