...
The project has shipped verified Phase Two burn-scar target-decision, real-pixel optical-pair, pair-local content-registration, five-state label-proposal, cross-event metadata-feasibility, exact cross-event source-fitness, and cross-event label-transfer baselines. BurnLens 0.12.1 ships the topology-stable provenance remediation: it preserves the exact Tepee/McKay analytical result while making all ten outputs byte-identical under both approved MTBS link topologies. It still creates no accepted dataset, split, or model result.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; shipped Phase Two evidence runs through verified `v0.12.1-topology-stable-label-transfer` at merge `e00028509145b439d95eb302591e7cb19bd073fd`.
...
- The latest verified tag is `v0.12.1-topology-stable-label-transfer`: annotated tag object `8606c61f0e1668f2b057abca177144937eae1036` remotely peels to merge `e00028509145b439d95eb302591e7cb19bd073fd`. A fresh no-hardlink merged-main clone passes all 137 tests, exact ten-output manifest/LF/raw gates, compilation, the canonical fixed-epoch wheel hash, and fresh real-source 10-of-10 reconstruction.
