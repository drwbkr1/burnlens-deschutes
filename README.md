...
The latest verified release is `v0.42.0-grandview-region-proposal`. It publishes one intact 25-pixel Grandview proposal per class plus 98 explicit unknown-ring pixels after the verified optical, reference, background-route, and checkout-stability gates. Both regions remain unreviewed. The cumulative prototype set remains balanced at eight approved regions across four events; dataset, split, baseline, and model work remains blocked below six events.

Issue #508 / PR #512 supplies the BurnLens 0.42.0 analytical candidate. Fresh merged-main verification caught checkout-dependent JSON line endings and correctly withheld the tag. Issue #513 / PR #514 applies an LF-only checkout-contract remediation without changing evidence or candidate bytes. Verified checkpoint `6124f76de04b5e503a9da7655a293a023480b39c` now preserves all five original hashes; issue #511 owns the separately versioned blank owner surface.
