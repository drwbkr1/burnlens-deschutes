...
The latest verified release is `v0.39.0-grandview-source-fitness`; `v0.40.0-grandview-reference-fitness` is the current release candidate. Exact Grandview MTBS evidence passes, RAVG is explicitly context-limited by its sparse/non-tree warning, and no candidate or label is created. The cumulative prototype set remains balanced at eight regions across four events; dataset, split, baseline, and model work remains blocked below six events.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; the latest verified repository tag is `v0.39.0-grandview-source-fitness`. Tag object `faacd1a33c9f486daba54d94ec7962694742fbd2` remotely peels to checkpoint `c89282da086bed98c009e440689eb06c864fb267`. Annotated `v0.21.0-current-reference-inventory` remains preserved as failed-release audit evidence.
