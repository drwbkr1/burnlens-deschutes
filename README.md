...
| 2 | One legally usable, versioned, leakage-resistant data/label/baseline foundation can support a defensible model-or-stop decision. | Active; ten balanced prototype regions span Darlene, McKay, Tepee, Green Ridge, and Grandview. Petes Lake now passes replacement optical source fitness and the exact MTBS native contract, but reference fitness/wetland precedence, background, proposal, unknown-boundary, owner-review, and intake gates remain. No accepted dataset, split, baseline, or model-readiness decision exists. |
