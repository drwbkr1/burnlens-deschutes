...
The project has shipped verified Phase Two burn-scar target-decision, real-pixel optical-pair, pair-local content-registration, five-state label-proposal, and cross-event metadata-feasibility baselines. BurnLens 0.10.0 freezes exact future acquisitions and leakage groups but does not create a dataset, split, or model result.
...
- The local pipeline exercised the authorized credentials without recording secrets, tokens, cookies, signed URLs, or credential-store details. The exact three-asset source package, 24-asset observation-screen package, and 2,254,805,631-byte optical pair are retained only in ignored local raw storage; zero raw provider bytes are committed. Cross-event feasibility used public metadata and zero provider imagery bytes. The repository contains one bounded label proposal, QA evidence, and frozen future acquisition groups, but no accepted dataset, split, baseline output, trained model, analytical metric, application demonstration, deployment, or public analytical result.
- The latest verified repository evidence baseline is analytical merge `5bfa1527410e98d8034b35ad68f6c50d5a1ec628` from issue #357 / PR #358; annotated tag object `dbfda10ca50c39d8e8924096e740e71643e1f133` remotely dereferences to that exact commit as `v0.10.0-cross-event-feasibility-baseline`.
...
- Corrected run `BL-2026-07-14-target-decision-r002` activates the fallback target without creating label pixels; `r001` is preserved as pre-remediation evidence. Optical run `BL-2026-07-15-optical-pair-evidence-r001` accepts the exact pair for protocol evidence only. Registration run `BL-2026-07-15-content-registration-r001` clears the local translation prerequisite. Proposal run `BL-2026-07-15-label-proposal-r001` and QA run `BL-2026-07-15-label-qa-r001` are the shipped reviewable one-event evidence. Cross-event source/report runs `RUN-2026-07-15-CROSS-EVENT-SOURCE-001` and `RUN-2026-07-15-CROSS-EVENT-FITNESS-001` freeze the next whole-event acquisitions while dataset construction remains deferred.
...
| 2 | One legally usable, versioned, leakage-resistant data/label/baseline foundation can support a defensible model-or-stop decision. | Active; one exact proposal passes software QA and two additional acquisition groups are frozen. Their imagery/labels, independent human evidence, dataset, split, baselines, and model-readiness decision remain unproved. |
