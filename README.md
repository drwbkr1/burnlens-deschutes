...
The latest verified Phase Two checkpoint is P2O4-T15. BurnLens `0.26.0` preserves the owner's exact completed 56-unit response and applies the disclosed reproducibility, source, frozen-origin quality, and event-leakage gates. It accepts 24 explicitly owner-approved prototype labels: 12 burned and 12 background, balanced eight per immutable Darlene, McKay, and Tepee group. Twenty-nine additional yes decisions remain quality-blocked; two no and one uncertain remain excluded. Restricted thresholded Tepee BARC stays private and unused. No accepted dataset, split, baseline, model result, accuracy claim, or deployed analytical application exists.
