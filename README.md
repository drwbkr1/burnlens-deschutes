...
BurnLens is not yet model-bearing portfolio ready. P2O5-T03 / issue #562 converts the exact passing six-event candidate into `burnlens-dataset-v0.1.0`, locks `burnlens-whole-event-split-v0.1.0` before patching, independently reconstructs and audits the 12 native-grid patches, and evaluates preregistered non-model baselines. The selected relative burn-ratio baseline scores 1.0 Dice and IoU on the 89 frozen selected test cores, under a single sealed test opening. [Open the model-readiness surface](samples/model-readiness/phase-two/MODEL-READINESS-2026-001.html) or [read the decision](docs/phase-two/objective-five/MODEL_READINESS_DECISION.md). U06 authorizes one rejection-first, CPU-only U-Net after the coherent U07 release; no model, weights, training run, evaluation, inference, or model-to-GEOINT application exists yet.
...
[Open the current local portfolio reviewer experience](portfolio/BURNLENS-PORTFOLIO-REVIEWER-EXPERIENCE-2026-002.html), then use the [two-minute quickstart](portfolio/README.md).
...
## Verified status and current candidate

The latest verified repository release remains `v0.51.0-replacement-six-event-sufficiency` while the BurnLens 0.52.0 U07 release candidate is verified. P2O5-T03 has accepted and versioned the dataset, split, QA, baseline, and model-readiness evidence on its issue-backed branch. The accepted package contains 12 native-grid 64-by-64 patches across McKay, Tepee, Green Ridge, Grandview, Windigo, and Ward Creek; 287 selected cores; 531 excluded unknown-ring pixels; and whole-event train/validation/test roles of Green Ridge plus Tepee, Grandview plus McKay, and Ward Creek plus Windigo. Training is authorized only under the exact U06 rejection-first contract and only after U07 is merged, tagged, and verified.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; the latest verified repository tag remains `v0.51.0-replacement-six-event-sufficiency` until U07 live merge/tag verification. The BurnLens 0.52.0 candidate versions the accepted Phase Two dataset, split, baseline, and explicit U-Net authorization without claiming that a model exists.
...
| 2 | One legally usable, versioned, leakage-resistant data/label/baseline foundation can support a defensible model-or-stop decision. | Accepted on the P2O5-T03 release candidate: dataset v0.1.0, whole-event split v0.1.0, independent QA, baseline v0.1.0, and an explicit rejection-first U-Net authorization. Live release verification remains U07. |
