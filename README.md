...
The latest verified release is `v0.32.0-additional-event-groups`. It freezes Green Ridge 2020, Grandview 2021, and Petes Lake 2023 as three additional metadata-only acquisition groups. The planned pool now has six immutable event identities across six years and repeated MTBS/BAER/RAVG regimes, but zero new provider pixels have been downloaded. Issue #472 owns exact Green Ridge acquisition and source/pixel fitness. Both classes, unknown boundaries, owner review, dataset, split, baseline, model result, accuracy, and a deployed analytical application remain absent.
