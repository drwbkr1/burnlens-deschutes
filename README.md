...
The project has shipped verified Phase Two burn-scar target-decision, real-pixel optical-pair, pair-local content-registration, five-state label-proposal, cross-event metadata-feasibility, exact cross-event source-fitness, and cross-event label-transfer baselines. BurnLens 0.12.1 is the topology-stable provenance remediation candidate: it preserves the exact Tepee/McKay analytical result while making all ten outputs byte-identical under both approved MTBS link topologies. It still creates no accepted dataset, split, or model result.
...
- P2O4-T04-REM / issue #371 / draft PR #372 corrects the public provenance identity without changing any analytical pixel. Both real one-link and exact-two-link MTBS packages produce the same ten `2026-002` files byte for byte, while a third link fails closed. BurnLens `0.12.1`, corrected runs `r004`, `MANIFEST-2026-014`, and the original `2026-001` history make the distinction explicit.
