...
[Open the current local portfolio reviewer experience](portfolio/BURNLENS-PORTFOLIO-REVIEWER-EXPERIENCE-2026-006.html), then use the [two-minute quickstart](portfolio/README.md).
...
The latest verified repository release remains `v0.52.0-dataset-baseline-model-readiness`. P3O1-T01 is the active BurnLens 0.53.0 candidate: U01-U06 pass through exact environment, implementation, frozen protocol, one substantive training run, one consumed model test opening, exact replay, rejected-model package, real render, and a `reject-model-retain-baseline` handoff. The package contains the unchanged 479,573-byte weights at SHA-256 `703d9257...`; 15 exact lineage bindings; and eight tracked files / 583,992 bytes at inventory SHA-256 `ce39c41c...`. The coherent milestone PR, full regression, package/install, merge, fresh-main, and tag gates remain before this becomes a verified release.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; the latest verified repository tag is `v0.52.0-dataset-baseline-model-readiness`. Tag object `7041ef76ff4aac17f3bc2f8ba07b427dc858d2bf` peels exactly to merge `dfb11c8b823e224aceb76be74003464973e33c2d`. BurnLens 0.52.0 versions the accepted Phase Two dataset, split, baseline, and explicit U-Net authorization without claiming that a model exists.
...
| 3 | One bounded model either adds reproducible value beyond the strongest baseline or is rejected honestly. | Candidate complete pending coherent PR/release: one exact U-Net is trained, evaluated once, reproduced byte-for-byte, and rejected against RBR; package v0.1.1 preserves the result. |
| 4 | The accepted model or baseline can become a valid, reproducible, georeferenced run and evidence interface. | Next after verified Phase Three release: RBR-primary analytical raster plus the rejected U-Net as diagnostic evidence. |
