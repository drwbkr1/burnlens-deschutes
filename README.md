...
P2O4-T35 / issue #534 is the active deadline-backed milestone. August 6, 2026 is the portfolio deadline. Windigo 2022 is the only final sixth-event attempt and must complete by July 27 at 6:00 PM America/Indianapolis. U01 passes current official identity, valid provider topology, the exact-pair contract, current terms, and one combined BAER/RAVG/MTBS request contract. U02 registers the exact 2,373,112,076-byte Sentinel pair, exposes both retained transport failures in [the corrected custody report](samples/cross-event/phase-two/windigo/WINDIGO-OPTICAL-CUSTODY-2026-002.json), and preserves [one accepted recipient-free request receipt](samples/cross-event/phase-two/windigo/WINDIGO-REFERENCE-REQUEST-2026-001.json). Delivery is pending; no delivered reference pixel exists. Any terminal or incomplete gate activates the technical-case-study-only fallback; baseline-first remains unauthorized without an accepted dataset and split.
...
- P2O4-T35 / issue #534 freezes the August 6 critical path: one Windigo attempt, a July 27 6:00 PM Indianapolis cutoff, and a technical-case-study-only fallback. U01 passes current source, terms, exact-pair, topology, custody, request, and deadline entry gates while acquiring zero provider archives and changing no accepted scientific state.
