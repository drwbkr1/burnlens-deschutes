...
The latest verified repository release remains `v0.53.0-bounded-unet-rejected-model` until the active Phase Four milestone is merged and independently reverified. P4O1-T01 / issue #570 has completed U01-U07 on branch `codex/p4o1-t01-rbr-geoint-milestone`. Accepted run `BL-2026-07-26-p4o1-t01-u07-package-r001` contains 66 files / 1,795,388 bytes; its exact 487,893-byte ZIP has SHA-256 `91308a2f...`. Both tracked forms validate, a clean detached checkout reproduces the ZIP byte-identically, and the packaged application passes desktop and narrow interaction. U08 owns coherent release review, merge, fresh-main verification, and the Phase Five handoff. No GitHub Release, deployment, access change, or public-sharing change exists.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; the latest verified repository tag is `v0.53.0-bounded-unet-rejected-model`. The active 0.54.0 candidate adds the RBR-primary geospatial run, repository-owned interface, and immutable package without changing the accepted method or promoting the rejected U-Net.
...
| 4 | The accepted model or baseline can become a valid, reproducible, georeferenced run and evidence interface. | Release candidate: exact RBR-primary Ward Creek run, rejected U-Net diagnostic, native rasters/vectors, bounded context, self-contained interface, and immutable package pass U01-U07; U08 release verification remains. |
