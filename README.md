...
The latest verified release is `v0.45.0-petes-lake-material-defer`. It preserves a complete, reproducible Petes Lake evidence chain and stops before candidate generation because the final authorized NWI source-custody attempt remains incomplete. The verified v0.3 prototype label set is unchanged at ten balanced regions, 236 core pixels / 9.44 ha, and 431 excluded unknown-ring pixels across five complete events; it is not ground truth or a dataset.

P2O4-T34 / issue #532 is the active BurnLens 0.46.0 release candidate. Its [official fallback source-gate report](samples/reference/phase-two/OFFICIAL-FALLBACK-SOURCE-GATE-2026-001.html) compares the McKenzie HUC8 `17090004` NWI route with Milli 0843 CS and selects neither. Both routes remain deferred on unresolved source-rights or evidence gates. The report quartet reconstructs exactly, its PNG passes original-resolution inspection, and the owner confirmed the exact HTML at desktop and narrow widths. No provider byte, custody state, source asset, candidate, response, label, sixth event, dataset, split, baseline, or model advances.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; the latest verified repository tag is `v0.45.0-petes-lake-material-defer`. Tag object `0b466402cdc36b0eacfff97d29dd16fe2a88868a` remotely peels to checkpoint `d65c24f59ce0c854ba230aa977eaf718d881d952`. Annotated `v0.21.0-current-reference-inventory` remains preserved as failed-release audit evidence.
- BL-GOV-002 / issue #400 reconciles obsolete Phase One GitHub backlog against current repository truth. Historical issues, PR bodies, and control artifacts remain audit evidence; they do not represent active work or narrow the controlling execution goal. BL-GOV-003 / issue #525 activates the [checkpoint policy](docs/governance/CHECKPOINT_POLICY.md) without changing software or scientific state. The [single-reviewer path decision](docs/phase-two/objective-four/SINGLE_REVIEWER_PATH_DECISION.md) and its 6/0/50 result remain historical evidence. Issue #521 / PR #527 and BL-EXC-001 issue #528 / PR #529 now close the verified Petes Lake material-defer release. Final r003 is immutable at seven promoted / one failed / four unexecuted assets, U05 scientific fitness is blocked, production U06-U10 are deferred, and reaching six events remains an unmet prerequisite that could not by itself bypass dataset fitness. The next bounded checkpoint must register a fresh official fallback or better alternate event from metadata/direct-download proof; it may not create r004 or reuse r001-r003 partial custody.
...
- P2O4-T34 / issue #532 prepares BurnLens 0.46.0 as a coherent `defer` release candidate. [Open the exact source-gate report](samples/reference/phase-two/OFFICIAL-FALLBACK-SOURCE-GATE-2026-001.html) or [read the decision](docs/phase-two/objective-four/OFFICIAL_FALLBACK_SOURCE_GATE_DECISION.md). McKenzie and Milli remain separately blocked and unselected; `provider_bytes_authorized` is false. U01/U04 pass, U02/U03/U05 defer, and shipment remains pending one milestone PR, fresh-main verification, and remote annotated-tag peel.
