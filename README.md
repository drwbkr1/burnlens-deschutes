...
The project has shipped verified Phase Two burn-scar target-decision, real-pixel optical-pair, pair-local content-registration, five-state label-proposal, cross-event metadata-feasibility, exact cross-event source-fitness, and cross-event label-transfer baselines. The current BurnLens 0.13.0 candidate adds a deterministic proposal-blinded review packet and integrity QA for Darlene 3, Tepee, and McKay. The latest verified tag remains v0.12.1 until the candidate completes review, merge, fresh-main, and tag gates. No accepted label set, dataset, split, or model result exists.
