...
The latest verified baseline is `v0.28.0-label-region-remediation-plan`. P2O4-T16 / issue #449 / PR #451 turns the sufficiency stop decision into a region-first owner-review and event-diversity contract. BurnLens will not buffer points or tile them into masks: it requires deterministic evidence-coherent regions, explicit unknown boundaries, at least six whole-event groups, and replicated source regimes before dataset fitness can reopen. Issue #453 owns the next no-promotion candidate pilot. No region label, accepted dataset, split, baseline, model result, accuracy claim, or deployed analytical application exists.
