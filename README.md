...
BurnLens is not yet model-bearing portfolio ready. P2O5-T03 / issue #562 converts the exact passing six-event candidate into `burnlens-dataset-v0.1.0`, locks `burnlens-whole-event-split-v0.1.0` before patching, independently reconstructs and audits the 12 native-grid patches, and evaluates preregistered non-model baselines. The selected relative burn-ratio baseline scores 1.0 Dice and IoU on the 89 frozen selected test cores, under a single sealed test opening. [Open the model-readiness surface](samples/qa/phase-two/model-readiness-v0.1.0/MODEL-READINESS-DECISION-2026-001.html) or [read the decision](records/phase-two/readiness/MODEL-READINESS-DECISION-2026-001.json). P3O1-T01 now trains the one authorized CPU U-Net and opens its frozen Ward Creek/Windigo test once. [Open the exact test surface](samples/evaluation/phase-three/bounded-unet-test-v0.1.0/BOUNDED-UNET-TEST-EVALUATION-2026-001.html): the model predicts all 89 selected cores as burned, scoring event-class macro Dice `0.29874213836477986`, IoU `0.21474358974358976`, and worst-event Dice `0.2641509433962264`, versus RBR's 1.0. It is therefore rejected as the analytical winner. This is a valid evaluated candidate, not an accepted or released model, ground truth, generalization result, or operational product. U06 exact replay/package/decision and the model-to-GEOINT application path remain.
...
[Open the current local portfolio reviewer experience](portfolio/BURNLENS-PORTFOLIO-REVIEWER-EXPERIENCE-2026-005.html), then use the [two-minute quickstart](portfolio/README.md).
...
The latest verified repository release is `v0.52.0-dataset-baseline-model-readiness`. P2O5-T03 / PR #563 accepts and versions the dataset, split, QA, baseline, and model-readiness evidence. Fresh merged `main` at `dfb11c8...` passes 695 tests with one expected skip and 86 subtests; the exact reviewer JSON/HTML replay; two identical 1,050,456-byte wheels at SHA-256 `eff2396b...`; and the previously verified isolated 105-command runtime. Annotated tag object `7041ef76...` peels exactly to that merge. The accepted package contains 12 native-grid 64-by-64 patches across McKay, Tepee, Green Ridge, Grandview, Windigo, and Ward Creek; 287 selected cores; 531 excluded unknown-ring pixels; and whole-event train/validation/test roles of Green Ridge plus Tepee, Grandview plus McKay, and Ward Creek plus Windigo. Phase Three may now execute only the exact U06 rejection-first contract.
...
| 3 | One bounded model either adds reproducible value beyond the strongest baseline or is rejected honestly. | Next after verified U07. The frozen contract permits one CPU-only U-Net and no search, augmentation, test tuning, or added-value claim from merely matching the perfect selected-core baseline. |
