...
The latest verified release baseline is `v0.28.0-label-region-remediation-plan`. P2O4-T17 / issue #453 is the `0.29.0` release candidate: the actual Darlene, McKay, and Tepee pipeline produces six intact, private-safe region candidates with explicit unknown rings and zero promoted labels. Real desktop/mobile rendering, raster contracts, privacy, 241 tests, exact private/public reconstruction, reproducible packaging, and isolated install pass; merged-main and release lifecycle remain pending. At least six whole-event groups and replicated source regimes are still required before dataset fitness can reopen. No region label, accepted dataset, split, baseline, model result, accuracy claim, or deployed analytical application exists.
...
- P2O4-T17 / issue #453 builds the `0.29.0` no-promotion release candidate: six intact native-grid candidate regions, 136 core pixels, 246 unknown-ring pixels, and zero region responses or labels. [Open the candidate evidence](samples/labels/pilot/phase-two/REGION-CANDIDATE-PILOT-2026-001.html) or [read the decision](docs/phase-two/objective-four/REGION_CANDIDATE_PILOT_DECISION.md).
...
| 2 | One legally usable, versioned, leakage-resistant data/label/baseline foundation can support a defensible model-or-stop decision. | Active; verified v0.27.0 proves the 24 reviewed points are insufficient. Verified v0.28.0 specifies region-first review and six-event/source-regime gates. The v0.29.0 candidate proves six regions can be rendered reproducibly without promotion, but three events remain insufficient. No region label, accepted dataset, split, baseline, or model-readiness decision exists. |
