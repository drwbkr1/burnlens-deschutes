...
The latest verified release is `v0.32.0-additional-event-groups`. The current BurnLens 0.33.0 release candidate for issue #472 acquires and gates the exact Green Ridge optical pair: 2,388,456,138 provider bytes, 44,110 native boundary pixels, 99.9887% pair eligibility, and nine of nine registration windows passing. Green Ridge BAER/MTBS/RAVG pixels remain unopened, so burned/background candidates, owner review, labels, dataset, split, baseline, model result, accuracy, and a deployed analytical application remain absent.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; the latest verified repository tag is `v0.32.0-additional-event-groups`. Tag object `d97aadd4aef60b55ce8c4200fed4eaf58f28dc73` remotely peels to checkpoint `25fa2b5ecef05e7a5fba87cd3c634ffb3951be90`. Annotated `v0.21.0-current-reference-inventory` remains preserved as failed-release audit evidence.
