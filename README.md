...
The current Phase Two candidate checkpoint is P2O4-T15. BurnLens `0.26.0` preserves the owner's exact completed 56-unit response and applies the disclosed reproducibility, source, frozen-origin quality, and event-leakage gates. It accepts 24 explicitly owner-approved prototype labels: 12 burned and 12 background, balanced eight per immutable Darlene, McKay, and Tepee group. Twenty-nine additional yes decisions remain quality-blocked; two no and one uncertain remain excluded. Restricted thresholded Tepee BARC stays private and unused. No accepted dataset, split, baseline, model result, accuracy claim, or deployed analytical application exists.
...
- P2O4-T15 / issue #437 adds exact authoritative-response intake and aggregate gate evidence. [Open the response-intake report](samples/labels/review/phase-two/OWNER-RESPONSE-INTAKE-2026-001.html) or [read the decision](docs/phase-two/objective-four/OWNER_RESPONSE_INTAKE_DECISION.md). Private bytes, notes, and unit decisions remain ignored and unpublished.
...
| 2 | One legally usable, versioned, leakage-resistant data/label/baseline foundation can support a defensible model-or-stop decision. | Active; exact proposal/QA, review/custody, verified seven-product fitness, and the verified 56-unit owner surface exist. Issue #437 will preserve the owner's actual yes/no/uncertain export and still require source/reproducibility/quality/leakage gates after any yes. No independent review, owner-approved labels, accepted dataset, split, baseline, or model-readiness decision exists. |
