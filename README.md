...
The project has shipped verified Phase Two burn-scar target-decision, real-pixel optical-pair, and pair-local content-registration baselines. Issue #353 now adds a review-ready five-state label proposal with separate software QA; it is not a dataset or an analytical release.
...
- P2O4-T01 / issue #353 / PR #354 implements `burn-scar-five-state-schema-v0.1.0` as `LABEL-PROPOSAL-2026-001`. The native-grid proposal retains 33.4144% of pixels as unknown, excluded, or review-needed; only burned and background-candidate enter the candidate target. `LABEL-QA-2026-001` independently recomputes all 270,000 state and target pixels with zero mismatch and audits 120 deterministic state/boundary samples. This candidate is reviewable one-event evidence, not ground truth or a dataset.
...
- Corrected run `BL-2026-07-14-target-decision-r002` activates the fallback target without creating label pixels; `r001` is preserved as pre-remediation evidence. Optical run `BL-2026-07-15-optical-pair-evidence-r001` accepts the exact pair for protocol evidence only. Registration run `BL-2026-07-15-content-registration-r001` clears the local translation prerequisite. Proposal run `BL-2026-07-15-label-proposal-r001` and QA run `BL-2026-07-15-label-qa-r001` create reviewable one-event candidate evidence while deferring dataset construction.
