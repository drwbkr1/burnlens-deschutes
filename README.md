...
The latest verified release baseline is `v0.29.0-region-candidate-pilot`. P2O4-T17 / issue #453 / PR #455 runs the actual Darlene, McKay, and Tepee pipeline and publishes six intact, private-safe region candidates with explicit unknown rings and zero promoted labels. Fresh-main tests, exact private/public reconstruction, responsive rendering, raster contracts, reproducible packaging, isolated install, and remote tag verification pass. Issue #457 owns a separately versioned owner yes/no/uncertain surface for these exact six candidates. At least six whole-event groups and replicated source regimes are still required before dataset fitness can reopen. No region label, accepted dataset, split, baseline, model result, accuracy claim, or deployed analytical application exists.
...
- P2O4-T17 / issue #453 / PR #455 ships verified `v0.29.0-region-candidate-pilot`: six intact native-grid candidate regions, 136 core pixels, 246 unknown-ring pixels, and zero region responses or labels. [Open the candidate evidence](samples/labels/pilot/phase-two/REGION-CANDIDATE-PILOT-2026-001.html) or [read the decision](docs/phase-two/objective-four/REGION_CANDIDATE_PILOT_DECISION.md). Lifecycle #456 records the release; issue #457 owns the exact owner-region review surface.
...
| 2 | One legally usable, versioned, leakage-resistant data/label/baseline foundation can support a defensible model-or-stop decision. | Active; verified v0.27.0 proves the 24 reviewed points are insufficient. Verified v0.28.0 specifies region-first review and six-event/source-regime gates. Verified v0.29.0 proves six regions can be rendered reproducibly without promotion; issue #457 owns their separate review, while three events remain insufficient. No region label, accepted dataset, split, baseline, or model-readiness decision exists. |
