...
The project has shipped verified Phase Two burn-scar target-decision, real-pixel optical-pair, pair-local content-registration, five-state label-proposal, cross-event metadata-feasibility, exact cross-event source-fitness, cross-event label-transfer, proposal-blinded label-review-readiness, and isolated offline reviewer-handoff baselines. The active BurnLens 0.15.0 candidate keeps the exact 56-unit scientific packet fixed and proves the isolated workbench's real interaction path in recorded Chrome: invalid-state blocking, draft download/load, completed response export, desktop/mobile rendering, console/runtime inspection, and fixture-only response locking. The fixture is not human evidence. One real returned response has separately been preserved and operator-locked under issue #384; a second qualifying response and adjudication remain required, and the reveal remains unopened. No accepted label set, dataset, split, baseline, model result, or deployed analytical application exists.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; shipped Phase Two evidence runs through verified `v0.14.0-offline-reviewer-handoff`, while P2O4-T07 / issue #383 is the active live-browser QA candidate.
...
- The latest verified tag remains `v0.14.0-offline-reviewer-handoff`. The `0.15.0` candidate passes all 154 tests, compilation, Node syntax, dependency health, exact five-output browser-artifact hashes, original-resolution viewport review, and two byte-identical detached-source wheels at 283,856 bytes / SHA-256 `950472207ac8f75208188584e2f8474f88a8a71e0f14fde864aa297b79076352`; isolated install exposes 25 commands and the packaged controller.
