...
The latest verified Phase Two baseline is P2O4-T13. BurnLens now ships reproducible official-source reconnaissance without duplicating the accepted seven-product request: seven source classes, 23 current Deschutes MTBS fires, and 21 additional candidate fires. GW Fire ranks first for a later small Landsat/cross-program fitness proof, but the advertised Landsat asset route currently requires EROS authentication. Exact bundle delivery, terms/structure, and pixel fitness remain pending under #416. The historical 6 burned / 0 background / 50 ignored reconciliation is preserved and unchanged; the owner-confirmed yes/no/uncertain route will reopen all 56 units rather than inherit those exclusions. No owner responses, accepted label set, dataset, split, baseline, model result, or deployed analytical application exists.
