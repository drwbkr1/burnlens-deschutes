...
The project has shipped verified Phase Two burn-scar target-decision, real-pixel optical-pair, pair-local content-registration, five-state label-proposal, cross-event metadata-feasibility, and exact cross-event source-fitness baselines. BurnLens 0.12.0 is the verified cross-event label-transfer candidate: it renders Tepee/McKay five-state and binary proposal rasters from exact sources and independently reproduces all 63,930 pixels. It still creates no accepted dataset, split, or model result.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; shipped Phase Two evidence runs through verified `v0.11.0-cross-event-source-fitness-baseline`, with the issue #367 BurnLens `0.12.0` candidate verified and awaiting merge/tag.
...
- Corrected run `BL-2026-07-14-target-decision-r002` activates the fallback target without creating label pixels; `r001` is preserved as pre-remediation evidence. Cross-event acquisition/source-fitness prove the exact local pixels; proposal run `BL-2026-07-16-cross-event-label-transfer-r003` and QA run `BL-2026-07-16-cross-event-label-transfer-qa-r003` prove transparent software transfer/reproducibility while dataset construction remains deferred.
...
| 2 | One legally usable, versioned, leakage-resistant data/label/baseline foundation can support a defensible model-or-stop decision. | Active; one proposal passes software QA and two exact additional event pairs pass source fitness with Tepee exclusions. Cross-event labels, independent human evidence, dataset, split, baselines, and model-readiness remain unproved. |
...
The [cross-event label-transfer decision](docs/phase-two/objective-four/CROSS_EVENT_LABEL_TRANSFER_DECISION.md), [semantic proposal report](samples/labels/cross-event/phase-two/CROSS-EVENT-LABEL-TRANSFER-2026-001.html), [proposal render](samples/labels/cross-event/phase-two/CROSS-EVENT-LABEL-TRANSFER-2026-001.png), [separate QA report](samples/labels/cross-event/phase-two/CROSS-EVENT-LABEL-TRANSFER-QA-2026-001.html), and [QA render](samples/labels/cross-event/phase-two/CROSS-EVENT-LABEL-TRANSFER-QA-2026-001.png) compare both events, preserve five-state uncertainty, expose the event-relative fallback, and reproduce every output pixel. They are proposal evidence, not accepted ground truth or a dataset.
