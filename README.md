...
The project has shipped verified Phase Two burn-scar target-decision, real-pixel optical-pair, pair-local content-registration, and five-state label-proposal baselines. BurnLens 0.10.0 now adds an accepted working-branch cross-event metadata-feasibility candidate; it freezes acquisition and leakage groups but does not create a dataset, split, or model result.
...
- P2O4-T02 / issue #357 accepts `CROSS-EVENT-FITNESS-2026-001` as a working analytical candidate. Current MTBS/Census/CDSE metadata selects exact Tepee and McKay Sentinel pairs, excludes Milli at a tile seam, quantifies 10.925/34.926/27.258 km representative-point separations, and freezes whole event/scene/geography/time groups before acquisition. No selected product was downloaded; PR, merge, and tag gates remain pending.
