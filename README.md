...
The latest verified release is `v0.40.0-grandview-reference-fitness`. Exact Grandview MTBS evidence passes, RAVG is explicitly context-limited by its sparse/non-tree warning, and no candidate or label is created. The cumulative prototype set remains balanced at eight regions across four events; dataset, split, baseline, and model work remains blocked below six events.

The issue #503 BurnLens 0.41.0 candidate acquires one exact near-anniversary scene and opens a separate Grandview affirmative-background evidence route after vector-footprint, optical-stability, and registration-exclusion gates. It creates zero candidates or labels and does not change the latest verified release until merge, fresh-main verification, and tagging pass.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; the latest verified repository tag is `v0.40.0-grandview-reference-fitness`. Tag object `48c1c9677b9b41a734be2dcdc4a9691caf4e6688` remotely peels to checkpoint `cdbe49883fb5330c9e7af036a847841b714dafae`. Annotated `v0.21.0-current-reference-inventory` remains preserved as failed-release audit evidence.
