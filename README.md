...
The project has shipped verified Phase Two burn-scar target-decision, real-pixel optical-pair, pair-local content-registration, five-state label-proposal, cross-event metadata-feasibility, exact cross-event source-fitness, cross-event label-transfer, proposal-blinded label-review-readiness, isolated offline reviewer-handoff, and live-browser reviewer-handoff QA baselines. BurnLens 0.16.0 evidence merged through issue #384 / PR #388 for one real returned response: the exact private bytes and a pre-reveal receipt are ignored and operator-locked, while a tracked JSON/HTML/PNG report publishes only completeness, cryptographic bindings, and the one-of-two response state. Review content remains withheld, the reveal remains operator-declared unopened, and software does not qualify the reviewer. Issue #389 corrects manifest chronology before tag. A second qualifying response and adjudication remain required. No accepted label set, dataset, split, baseline, model result, or deployed analytical application exists.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; the latest verified tag remains `v0.15.0-live-browser-reviewer-handoff`, while merged P2O4-T08 evidence awaits issue #389 record correction and the `v0.16.0-first-reviewer-response-lock` tag.
...
- The latest verified tag is `v0.15.0-live-browser-reviewer-handoff`: annotated tag object `69b32b076a7fca40ba5eceacb64aeac2a512e7b9` remotely peels to merge `861716be16be3a0469d2268baed971be65684d48`. A fresh remote-main clone passes all 154 tests, compilation, Node syntax, dependency health, exact seven-file plus archive reconstruction, a new live-browser roundtrip with zero human evidence, exact five-output manifest hashes, 97 local links, and the canonical 283,856-byte wheel / SHA-256 `950472207ac8f75208188584e2f8474f88a8a71e0f14fde864aa297b79076352`.
