...
The project has a shipped, verified Phase Two burn-scar target-decision baseline and real-pixel optical-pair protocol baseline. It is not an analytical release.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; shipped Phase Two evidence runs through `v0.7.0-optical-pair-protocol-baseline`, and issue #347 is the review-ready `0.8.0` local content-registration candidate.
...
- P2O3-T01 / issue #347 / PR #348 analytically accepts `CONTENT-REGISTRATION-2026-001`: all twelve native-20m windows pass, with median residual 0.0224 pixel and maximum 0.0361 pixel / about 0.72 m. Post-merge checkout exposed a JSON/HTML line-ending reconstruction defect, so the `0.8.0` tag is withheld pending issue #349. Registration measurements are unchanged and labels remain unimplemented.
...
- The latest verified repository evidence baseline is merge `136d4d0919eba7144881c22163a149c89fee5a76` from issue #343 / PR #344; annotated tag object `28d12fb5ef5c70054b8af5fd3c4847ba268000a1` remotely dereferences to that exact commit as `v0.7.0-optical-pair-protocol-baseline`.
...
- Corrected run `BL-2026-07-14-target-decision-r002` activates the fallback target without creating label pixels; `r001` is preserved as pre-remediation evidence. Optical run `BL-2026-07-15-optical-pair-evidence-r001` accepts the exact pair for protocol evidence only. Registration run `BL-2026-07-15-content-registration-r001` clears the local translation prerequisite. The next gate is an independently reviewable five-state label proposal with separate QA.
...
| 2 | One legally usable, versioned, leakage-resistant data/label/baseline foundation can support a defensible model-or-stop decision. | Active; the exact pair, five-state protocol, and local registration evidence are accepted, while label implementation and QA, dataset, splits, and baselines remain unproved. |
