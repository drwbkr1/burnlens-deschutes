...
The latest verified release is `v0.31.0-region-owner-response-intake`. The current `0.32.0` release candidate for issue #466 freezes Green Ridge 2020, Grandview 2021, and Petes Lake 2023 as three additional metadata-only acquisition groups. The planned pool now has six immutable event identities across six years and repeated MTBS/BAER/RAVG regimes, but zero new provider pixels have been downloaded. Source fitness, both classes, unknown boundaries, owner review, dataset, split, baseline, model result, accuracy, and a deployed analytical application remain absent.
...
- P2O4-T20 / issue #466 builds the `0.32.0` release candidate. [Open the additional-event plan](samples/cross-event/phase-two/ADDITIONAL-EVENT-GROUP-PLAN-2026-001.html) or [read the decision](docs/phase-two/objective-four/ADDITIONAL_EVENT_GROUP_PLAN_DECISION.md). Six whole-event identities are now frozen at metadata level; exact pixel/source fitness remains the next gate.
