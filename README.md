...
BurnLens is not yet model-bearing portfolio ready. P2O5-T03 / issue #562 converts the exact passing six-event candidate into `burnlens-dataset-v0.1.0`, locks `burnlens-whole-event-split-v0.1.0` before patching, independently reconstructs and audits the 12 native-grid patches, and evaluates preregistered non-model baselines. The selected relative burn-ratio baseline scores 1.0 Dice and IoU on the 89 frozen selected test cores, under a single sealed test opening. [Open the model-readiness surface](samples/qa/phase-two/model-readiness-v0.1.0/MODEL-READINESS-DECISION-2026-001.html) or [read the decision](records/phase-two/readiness/MODEL-READINESS-DECISION-2026-001.json). Verified BurnLens 0.52.0 makes U06's one rejection-first, CPU-only U-Net contract executable. P3O1-T01 U01-U04 now lock the exact model environment, reference implementation, experiment protocol, and one-test-opening mechanism, then perform the only substantive run. [Open the exact training surface](samples/models/burnlens-unet-binary-v0.1.0/BOUNDED-UNET-TRAINING-2026-001.html): epoch 10 is frozen from validation at macro Dice `0.7253521126760564`, while McKay visibly collapses to all-burned predictions. This is not an accepted or released model; Ward Creek/Windigo remain unopened, and no test evaluation, inference, or model-to-GEOINT application exists yet.
