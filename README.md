...
The project has shipped verified Phase Two burn-scar target-decision, real-pixel optical-pair, pair-local content-registration, five-state label-proposal, cross-event metadata-feasibility, exact cross-event source-fitness, cross-event label-transfer, proposal-blinded label-review-readiness, isolated offline reviewer-handoff, live-browser reviewer-handoff QA, first returned-response hash-lock, mixed-version dual-lock custody-readiness, atomic response-intake readiness, owner-waiver reveal readiness, and reduced-validation single-reviewer reconciliation baselines. Verified BurnLens 0.20.0 reconciles the one exact blinded response against the exact proposal under issue #403. The active BurnLens 0.21.0 candidate then confirms current official BAER/RAVG/MTBS catalog coverage across all three events and closes label promotion until exact current bundles pass fitness review. Six burned candidate units, zero background units, and fifty ignored units remain unchanged. The real response, receipts, authorization, unit-level reconciliation, and provider bundles remain only in ignored storage. No second reviewer, inter-rater validation, consensus, adjudication, accepted dataset, split, baseline, model result, or deployed analytical application exists.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; the latest verified tag is `v0.20.0-single-reviewer-reconciliation`, annotated tag object `5bb840b43ae8594e02a9b026b3932bb16f83bf5b`, which remotely peels to merge `4918024dfb83270dbbd83a5880c455cc3c01771c`.
- BL-GOV-002 / issue #400 reconciles obsolete Phase One GitHub backlog against current repository truth. Historical issues, PR bodies, and control artifacts remain audit evidence; they do not represent active work or narrow the controlling execution goal. The owner-authorized [single-reviewer path decision](docs/phase-two/objective-four/SINGLE_REVIEWER_PATH_DECISION.md) supersedes issue #393; issue #403 owns the reconciliation candidate and issue #411 owns the next reference-evidence remediation.
...
- The latest verified tag is `v0.20.0-single-reviewer-reconciliation`: annotated tag object `5bb840b43ae8594e02a9b026b3932bb16f83bf5b` remotely peels to merge `4918024dfb83270dbbd83a5880c455cc3c01771c`. A fresh clean checkout passes 181 tests, compilation, dependency health, 58 tracked-JSON parses, 95 local-link checks, exact manifest outputs, and two byte-identical canonical 340,283-byte wheels / SHA-256 `ad506e60ac7480c14382d4d5d0e53468ecb11a39801feae350a0ee397d068b74`; isolated install reports `0.20.0` and 33 entry points.
...
| 2 | One legally usable, versioned, leakage-resistant data/label/baseline foundation can support a defensible model-or-stop decision. | Active; exact three-event proposal evidence, separate software QA, a proposal-blinded review instrument, verified handoff/custody/waiver controls, and shipped reduced-validation single-reviewer reconciliation exist. The owner waived reviewer two, so inter-rater validation and adjudication remain absent. The reconciliation retains 6 burned candidates, 0 background candidates, and 50 ignored units, selecting remediation and dataset deferral. Issue #411 must strengthen independently traceable burned/background evidence across all events before accepted labels, dataset, split, baselines, or model-readiness can be considered. |
