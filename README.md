...
BurnLens is not yet model-bearing portfolio ready. Verified BurnLens 0.50.0 / `v0.50.0-six-event-dataset-sufficiency` reconstructs the six complete prototype events and tests all 90 whole-event 2/2/2 assignments. Eight readiness gate families pass, but leakage/split fitness and evaluation design block because Darlene is the sole NIFC-context event. [Open the sufficiency decision surface](samples/labels/readiness/phase-two/SIX-EVENT-DATASET-SUFFICIENCY-2026-001.html) or [read the decision](docs/phase-two/objective-five/SIX_EVENT_DATASET_SUFFICIENCY_DECISION.md). Dataset, split, baseline, U-Net, evaluation, and model-to-GEOINT integration remain absent. Issue #554 owns the bounded replacement-event remediation.
...
The exact Windigo response was locked before reveal and is the sole valid final. Both owner decisions are yes. Final r003 recomputes the exact Sentinel and BAER/MTBS/RAVG source pixels, both proposal routes, candidate rasters, registration, terms, uncertainty, and leakage gates. [Open the aggregate intake report](samples/labels/review/windigo/phase-two/intake/WINDIGO-OWNER-RESPONSE-INTAKE-2026-001.html) or [read the decision](docs/phase-two/objective-four/WINDIGO_OWNER_RESPONSE_INTAKE_DECISION.md). `owner-approved-prototype-region-labels-v0.4.0` is balanced at 12 regions, 286 core pixels / 11.44 ha, and 533 excluded unknown-ring pixels across six complete events. It is owner-approved prototype evidence, not ground truth or a dataset. The full sufficiency evaluator now blocks all 90 prospective whole-event 2/2/2 assignments; dataset, split, baseline, and model remain closed.
