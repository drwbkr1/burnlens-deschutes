...
The latest verified release is `v0.41.0-grandview-background-evidence`. One exact near-anniversary scene opens a separate Grandview affirmative-background evidence route after vector-footprint, optical-stability, and registration-exclusion gates. It creates zero candidates or labels. The cumulative prototype set remains balanced at eight regions across four events; dataset, split, baseline, and model work remains blocked below six events.

Issue #508 / PR #512 is the BurnLens 0.42.0 analytical candidate. Fresh merged-main verification caught checkout-dependent JSON line endings and correctly withheld the tag. Issue #513 / PR #514 applies an LF-only checkout-contract remediation without changing evidence or candidate bytes. It publishes one 25-pixel proposed burned core and one 25-pixel proposed background core with 98 total unknown-ring pixels. Both remain unreviewed; issue #511 owns a separately versioned blank owner surface after remediation release gates pass.
...
- The controlling goal remains versioned at `v0.0.8-execution-goal-baseline`; the latest verified repository tag is `v0.41.0-grandview-background-evidence`. Tag object `61288c1c641d1f119aee83eae5931e79f3b0c621` remotely peels to checkpoint `07c35346ad1369c2fe1e2a4570996c23ba56dac4`. Annotated `v0.21.0-current-reference-inventory` remains preserved as failed-release audit evidence.
