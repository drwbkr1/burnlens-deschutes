...
The first clean branch clone catches one release-only defect: the new nested intake JSON checks out with CRLF and no longer matches its authoritative LF bytes. Rebuilt outputs themselves remain exact. BurnLens adds explicit LF rules for the nested JSON/HTML path and repeats clean-clone reconstruction rather than weakening the hash gate.
