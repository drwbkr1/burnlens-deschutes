...
| 2 — Data foundation | Legally usable, versioned, leakage-resistant data/label/baseline package with model-readiness decision | **Active; one-event proposal shipped with separate software QA; dataset not created** | P2O4-T01 / #353 / PR #354 is shipped at verified `v0.9.0-label-proposal-baseline`. Cross-event evidence, independent human review, leakage-resistant grouping, a dataset, baselines, and the model-readiness decision remain open. |
...
| Evidence tooling | BurnLens `0.9.0` shipped baseline; acquisition/intake/source/geometry/target paths plus exact optical inspection, pair-local registration, five-state GeoTIFF proposal, separate all-pixel QA, LF-stable JSON/HTML, original-resolution PNG rendering, and safe failure states |
...
P2O4-T01 is shipped at verified `v0.9.0-label-proposal-baseline`. `LABEL-PROPOSAL-2026-001` and `LABEL-QA-2026-001` pass their proposal and separate-software gates while explicitly deferring a dataset. The next analytical checkpoint must begin by running this workflow, then test cross-event label evidence and leakage-resistant event/scene/geography/time grouping before any dataset or split.
