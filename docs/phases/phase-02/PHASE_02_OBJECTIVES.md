...
**Active - one-event proposal passes software QA and two additional acquisition groups are frozen; dataset not created.**

P2O1-T01 through P2O1-T03 establish the public metadata, exact route, terms, and delivery-integrity baseline. P2O2-T01 through P2O2-T06 ship the final AOI, exact source transactions, real source/observation evidence, active burn-scar target, and same-orbit optical pair/protocol. P2O3-T01 ships verified pair-local registration. P2O4-T01 ships verified `v0.9.0-label-proposal-baseline` and implements `burn-scar-five-state-schema-v0.1.0`: 33.4144% of the exact native-grid AOI remains explicitly ignored, and a separate implementation reproduces every one of the 270,000 state and target pixels plus a 120-sample audit. P2O4-T02 ships verified `v0.10.0-cross-event-feasibility-baseline`, freezing exact Tepee/McKay pairs and groups. P2O4-T03 / issue #361 / PR #362 has analytical merge `6a6da910849daefa918ed56af6631b2ec44bc211`: four exact archives are registered, McKay passes source fitness, and Tepee retains visible SCL/window exclusions. Pre-tag verification opened trace-remediation issue #363 to align the label protocol and implemented schema identifiers without changing results. A reviewable one-event proposal and exact three-event source foundation exist; no accepted cross-event labels, dataset, split, baseline, or model exists.

Current objective posture: Objective One is satisfied for the bounded acquisition actions. Objective Two has the accepted source/reference stack, observation comparison, approved target, and exact pairs. Objective Three has immutable Darlene registration/proposal preprocessing plus acquired and inspected Tepee/McKay source packages, but not multi-event labels or a dataset. Objective Four has a reviewable one-event proposal and exact cross-event source fitness; independent human/cross-event label truth remains unproved. Objective Five has pre-tiling group identities but no split, duplicate/overlap audit on derived pixels, baselines, or untouched test set. Objective Six remains open. After the `0.11.0` candidate ships, the next checkpoint should transfer the five-state proposal to Tepee/McKay and run separate QA with exclusions binding before label acceptance or dataset construction. This ordering is evidence-responsive, not a checklist completion claim.
