...
**Current status:** P2O1-T02 / issue #312 narrows the discovery set to one exact Sentinel-2 L2A product and the closest same-day NOAA-21 VIIRS active-fire/geolocation pair. Fresh primary-source review records stable provider routes, formats, geolocation and quality requirements, exact Sentinel provider checksums, retention constraints, and a conservative future label role. The normalized readiness fixture contains metadata only. No credential, provider asset, source dataset, label, baseline, model, run, map, or analytical output exists. F04-A stops before the first paired acquisition because the selected Sentinel route requires an owner-approved CDSE credential.
