...
recommendations for a separately governed follow-on experiment; that
experiment is outside the current project's planning and implementation scope.
...
| 6 | Portfolio publication, public release, communication, and closeout | Publish one coherent, traceable portfolio release whose demo, case study, repository, presentation, licensing, citation, archive, and maintenance posture all refer to the same accepted evidence and limitations. | **active — verified local pre-publication candidate; owner publication gate next** | Issue #578 starts from exact lifecycle merge `3097ac5...`. U01-U07 pass locally. Risk-bearing U07 checkpoint `bc9646c...` retains and repairs checksum-checkout and living-source replay failures, then passes fresh-clone exact reconstruction, 802 tests, deterministic wheel/install/122-command, rendered-recipient, rights, claims, privacy, rollback, and live-truth gates. One coherent milestone PR and reviewed-merge verification remain before the owner publication stop gate. | [Phase Six objectives](../phases/phase-06/PHASE_06_OBJECTIVES.md) |
...
**Current status:** P2O1-T02 through P2O3-T01 ship exact route/access, AOI, source/reference, target, optical-pair, and registration evidence. Twenty-four original owner-approved prototype points remain immutable audit evidence. Verified P2O4-T19 through P2O4-T35 establish 12 balanced regions across six events, while P2O4-T33 preserves Petes Lake's material defer. P2O5-T02 proves the original Darlene-bearing pool cannot support a defensible split, and P2O4-T39 replaces it with Ward Creek. P2O5-T03 / issue #562 / PR #563 ships verified `burnlens-dataset-v0.1.0` and `burnlens-whole-event-split-v0.1.0`: 12 native-grid 64-by-64 patches, 287 selected cores, 531 excluded unknown-ring pixels, and locked 2/2/2 whole-event roles. Independent QA reconstructs every patch and freezes normalization from train only. Preregistered baseline evaluation selects RBR at threshold `0.041043221950531006`; it reaches 1.0 Dice and IoU on 89 selected test cores under one sealed test opening. P3O1-T01 / issue #566 / PR #567 locks, trains, tests once, reproduces, and packages one rejection-first CPU U-Net in verified BurnLens 0.53.0. It predicts all 89 selected cores as burned and scores macro Dice `0.29874213836477986`, IoU `0.21474358974358976`, and worst-event Dice `0.2641509433962264`; it is rejected against RBR's 1.0. P4O1-T01 / issue #570 then carries the accepted RBR into native rasters/vectors, bounded official context, deterministic observations, a self-contained interface, and immutable package while preserving the model as rejected diagnostic evidence. Restricted thresholded Tepee BARC remains private and unused.
...
**Current status:** P3O1-T01 / issue #566 / PR #567 is verified at BurnLens 0.53.0. One exact CPU U-Net trains, consumes its model test opening once, predicts all selected cores as burned, and is rejected against RBR. The 35-row training replay and 479,573-byte weights reproduce exactly. `burnlens-unet-rejected-package-v0.1.1` binds the complete lineage and selects `baseline-primary-with-rejected-model-diagnostic` for Phase Four. Fresh main, deterministic package/install, and annotated-tag gates pass.
...
**Current route:** Complete the project through RBR-primary analytical
integration with the frozen rejected U-Net as a first-class diagnostic result.
...
and a separate detached checkout reproduces the exact historical v0.54 wheel.
One medium historical builder-identity omission remains visible with its exact
setuptools 82.0.1 workaround; the U04 dependency advisory is also visible with
...
The controlling execution goal remains `v0.0.8-execution-goal-baseline` at `22a8d88435cb8d5b900a398b7482c3b7277d2ee6`, amended operationally by BL-GOV-003 and `checkpoint-policy-v0.1.0`. P2O4-T10B is immutable historical reconciliation, not the current acceptance route. Verified v0.31 through v0.45 establish ten balanced prototype regions across five events and close Petes Lake with disposition `defer`. Verified v0.46 defers both fallback routes. Verified v0.47 establishes the sixth owner-approved prototype event without opening dataset work.

August 6, 2026 is an aggressive portfolio-delivery target and prioritization constraint. It does not authorize skipping dataset, split, baseline, model, evaluation, or GEOINT gates. The current critical path is:
...
2. **Full sufficiency decision — block:** P2O5-T02 / issue #552 passes eight gate families but finds zero valid whole-event 2/2/2 assignments. Dataset, split, baseline, and model stay closed.
...
6. **Contingency package — preserved:** P2O4-T38 / issue #548 / PR #549 verifies the exact offline technical-case-study bundle. It is an interim contingency artifact, not the intended final model-bearing submission.
7. **Bounded remediation:** add one complete event under the existing MTBS-current or BAER/MTBS/RAVG-current regime, exclude Darlene from the next dataset candidate, and rerun sufficiency. A second equivalent NIFC event is the alternative.
8. **If sufficiency passes:** create the versioned dataset, freeze whole-event train/validation/test splits before patching, run dataset QA and justified non-model baselines, and merge the explicit model-readiness decision.
9. **Model and GEOINT path:** train one bounded U-Net, evaluate it against the strongest baseline, preserve error analysis and reproducibility, then ship the smallest georeferenced inference/vector/overlay/run-package vertical slice.

Low-value source scouting, repeated provider retries, recursive lifecycle churn, and roadmap breadth are deferred. Do not create a dataset, split, baseline, or model merely when a sixth event arrives. Do not activate a terminal technical-case-study fallback without owner direction. Deployment or a public-sharing change remains an owner stop condition.

P6O1-T01 / issue #578 is now the active closeout checkpoint. U01-U07 pass
locally with disposition `ready-for-owner-publication-gate`. The immediate
repository action is one coherent reviewed milestone PR and exact post-merge
verification. Tagging, GitHub Release creation, deployment, access or ownership
change, public sharing, publication, and external submission remain outside
that PR and require explicit owner stop-gate handling.
