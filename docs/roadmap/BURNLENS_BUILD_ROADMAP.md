...
**Gate outcome:** 24 prototype points pass owner-response gates but fail dataset sufficiency. P2O4-T16 defines evidence-coherent native-grid regions, one-pixel unknown rings, exact owner review, and at least six whole-event groups with replicated source regimes. Verified P2O4-T17 proves six intact candidates can be rendered reproducibly without promotion. Issue #457 now owns the exact region-review surface; add comparable events only through separately gated work before repeating dataset and split gates.
