...
| 2 | Data acquisition, labels, baselines, and dataset versioning | Build a complete, traceable data foundation for one bounded Deschutes County experiment, including authorized sources and AOI, reproducible preprocessing, defensible positive/negative/unknown labels, leakage-resistant splits, non-model baselines, dataset QA, and a model-readiness decision. | **active - incomplete; current reference inventory passes; product fitness and dataset not created** | P2O4-T11 confirms seven exact current catalog records across all events and closes label promotion pending exact bundle fitness. Inter-rater validation, consensus, adjudication, accepted labels, splits, baselines, dataset QA, and model-readiness remain open. | [Phase Two objectives](../phases/phase-02/PHASE_02_OBJECTIVES.md) |
...
**Current status:** P2O1-T02 through P2O3-T01 ship exact route/access, AOI, source/reference, target, optical-pair, and registration evidence. P2O4-T01 through P2O4-T10B ship proposal, cross-event, review, handoff, custody, atomic-intake, owner-waiver readiness, and reduced-validation reconciliation through verified `v0.20.0-single-reviewer-reconciliation`. P2O4-T11 has a `0.21.0` candidate that confirms seven exact current cross-program catalog records and records the late-2025 official archive reprocessing boundary. One blinded response still yields 6 burned candidate units, 0 background units, 50 ignored units, and no Tepee candidate. Catalog availability passes, but exact product-bundle fitness is pending. An accepted dataset, split, baseline, model, or deployed analytical application does not exist.

**Gate outcome:** the owner selected the burn-scar fallback; Darlene, Tepee, and McKay proposal/QA evidence exists with uncertainty and groups preserved; the review instrument, isolated handoff, exact current-Chrome software path, response custody, waiver authorization, and single-reviewer reconciliation are executable. The reconciliation chooses remediation, not dataset candidacy. Issue #411 must strengthen independently traceable burned/background evidence across every event, with fresh official-source validation and weak/conflicting evidence retained as ignored. Do not create a split, baseline, or model until those label-evidence and group-overlap gates pass.
...
The controlling execution goal remains `v0.0.8-execution-goal-baseline` at `22a8d88435cb8d5b900a398b7482c3b7277d2ee6`. P2O4-T10B records the reduced-validation reconciliation and defers the dataset. P2O4-T11 / issue #411 confirms current cross-program availability and now requires exact current bundle terms, bytes, structure, and pixel fitness. Preserve disagreement/uncertainty as ignored, strengthen both binary classes across all events, keep whole-event groups and ignore states binding, and do not begin partition or baseline work early.
