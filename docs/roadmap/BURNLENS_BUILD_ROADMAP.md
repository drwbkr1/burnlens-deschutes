...
**Current status:** P2O1-T02 through P2O3-T01 ship the exact route/access, AOI, source/reference, target, optical-pair, and pair-local registration evidence. P2O4-T01 / issue #353 / PR #354 ships verified `v0.9.0-label-proposal-baseline`: a native-grid five-state proposal retains 33.4144% explicit ignore, and a separate implementation reproduces all 270,000 state and target pixels with zero disagreement plus a 120-sample audit. P2O4-T02 / issue #357 adds the accepted BurnLens 0.10.0 working candidate: current official metadata selects exact Tepee and McKay pairs, excludes Milli at a tile seam, and freezes all event/scene/geography/time group IDs before acquisition. A reviewable one-event proposal and metadata-feasible cross-event plan exist; selected cross-event imagery, cross-event labels, an accepted dataset, split, baseline, and model do not.

**Gate outcome:** the owner selected the burn-scar fallback; the Darlene pair/protocol, local translation, and reviewable proposal prerequisites are shipped. Two additional whole-event source identities are now frozen without calling them dataset partitions. Acquire and validate only those exact sources next; do not create a split until cross-event pixels, labels, unknown coverage, and group-overlap evidence pass.
...
The controlling execution goal remains `v0.0.8-execution-goal-baseline` at `22a8d88435cb8d5b900a398b7482c3b7277d2ee6`. P2O4-T01 is shipped at verified `v0.9.0-label-proposal-baseline`; P2O4-T02 / issue #357 is the accepted working candidate that freezes Tepee and McKay exact acquisition groups. After its shipment gates pass, the next bounded checkpoint should acquire only those four frozen Sentinel products and prove authenticated delivery, integrity, native-pixel quality, and pair-local registration before cross-event label or partition work. It may remediate or stop if source fitness fails.
