...
**Current status:** P3O1-T01 / issue #566 passes U01-U06 on the milestone branch. One exact CPU U-Net trains, consumes its model test opening once, predicts all selected cores as burned, and is rejected against RBR. The 35-row training replay and 479,573-byte weights reproduce exactly. `burnlens-unet-rejected-package-v0.1.1` binds the complete lineage and selects `baseline-primary-with-rejected-model-diagnostic` for Phase Four. Coherent PR, full regression, merge, fresh-main, and tag verification remain.
