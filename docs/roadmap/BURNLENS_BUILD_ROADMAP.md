...
**Gate outcome:** Green Ridge and Grandview have passed their complete optical/reference/background/proposal/review/intake paths. Verified v0.44 includes owner-confirmed local HTML rendering, exact fresh-main reconstruction, canonical packaging, isolated installation, and remote-tag verification. Under `checkpoint-policy-v0.1.0`, issue #521 groups the complete Petes Lake source, terms, custody, optical/reference/background, proposal, owner-response, quality, uncertainty, and leakage evidence while retaining each unit independently. U01/U02 pass; U03 r001 rejects the planned snow-dominated post scene. A deterministic metadata-only remediation selects the exact 19 October same-regime product; replacement-specific terms, the exact singleton contract, and the clean live preflight pass. Replacement custody and local native-pixel/render fitness still block U04. Passing the eventual milestone still does not authorize a dataset: Objective Five's separate class/unknown completeness, source-regime replication, never-tuned transfer, dominance, leakage, baseline, and split gates remain authoritative.
...
1. **Active six-event evidence milestone — issue #521:** U01/U02 pass. U03 r001 rejects the planned post scene because local snow leaves zero passing windows. Metadata-only remediation selects the exact 19 October same-event/same-regime product; contract checkpoint `ab6ecc04...`, replacement-specific terms, and PRECHECK-2026-048 pass. Record the gate on issue #521, execute one exact singleton custody transaction, then pass native-pixel, registration, and render gates before continuing the Petes Lake reference/background/proposal/owner chain. Each unit and remediation run retains independent immutable records and failures.
