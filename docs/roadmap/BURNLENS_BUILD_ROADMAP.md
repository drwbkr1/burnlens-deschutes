...
| 3 | CV model development, controlled evaluation, and model versioning | Build and evaluate one bounded U-Net-style segmentation model against the accepted Phase Two baselines, then package either a defensible model or a transparent baseline-only decision with complete lineage, error analysis, and limitations. | **active — U01-U04 passed** | P3O1-T01 / issue #566 locks the exact CPU environment, reference path, experiment, and one-test-opening mechanism, then performs the only substantive run through `5179a74...`. Epoch 10 is a validation-only candidate; McKay's all-burned failure remains visible. U05 may open Ward Creek/Windigo exactly once after U04 is pushed. Matching the perfect selected-core baseline is not added value. | [Phase Three objectives](../phases/phase-03/PHASE_03_OBJECTIVES.md) |
