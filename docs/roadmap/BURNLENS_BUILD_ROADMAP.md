...
| 2 | Data acquisition, labels, baselines, and dataset versioning | Build a complete, traceable data foundation for one bounded Deschutes County experiment, including authorized sources and AOI, reproducible preprocessing, defensible positive/negative/unknown labels, leakage-resistant splits, non-model baselines, dataset QA, and a model-readiness decision. | **active - incomplete; Petes Lake r003 intake next** | Verified Green Ridge plus verified P2O4-T32 Grandview intake establish five complete prototype-label events. Issue #521 is active; U01/U02 pass, U03 r001 remains a planned-post failure, replacement U03 passes with explicit spatial exclusions, and U04 passes the exact terms-first MTBS native contract with zero accepted reference pixels. U05 retains r001 and terminal r002 failures; r002 promoted seven assets before its only Data Source pre-count request failed before response open. U05R2's offline observability and disjoint-custody code gate passes; the final full r003 intake is registered but remains uninitialized. | [Phase Two objectives](../phases/phase-02/PHASE_02_OBJECTIVES.md) |
...
**Gate outcome:** Green Ridge and Grandview have passed their complete optical/reference/background/proposal/review/intake paths. Verified v0.44 includes owner-confirmed local HTML rendering, exact fresh-main reconstruction, canonical packaging, isolated installation, and remote-tag verification. Under `checkpoint-policy-v0.1.0`, issue #521 groups the complete Petes Lake source, terms, custody, optical/reference/background, proposal, owner-response, quality, uncertainty, and leakage evidence while retaining each unit independently. U01/U02 pass; U03 r001 retains the planned snow-dominated failure. The exact 19 October replacement passes replacement-specific terms, singleton custody, native CRS/grid/nodata/SCL/CLD/SNW inspection, 97.8360% paired eligibility, five passing registration windows, three spatial exclusions, zero failed windows, and original-resolution rendering. U04 then passes one exact MTBS request/delivery/custody chain, delivered-notice terms, complete member manifest, native raster/vector/media contract, and byte-identical report replay while accepting zero reference pixels. U05 r001 retains a local qualified-field validator failure. U05R1 fixes it, then r002 promotes seven ordered NWI assets before one Data Source pre-count request fails at `PROVIDER_OPEN`. Its cause is unknowable from retained evidence because the error path discarded the underlying exception. U05R2 now passes privacy-safe failure classification, exact 24-file prior-evidence and same-source request binding, retained-failed-byte rehash, no-retry, full-suite, independent-review, and remote-equality gates. One final wholly disjoint full r003 intake remains uninitialized and must pass before custody, wetland measurement, optical-grid coverage, temporal support, or source precedence can pass. Passing the eventual milestone still does not authorize a dataset: Objective Five's separate class/unknown completeness, source-regime replication, never-tuned transfer, dominance, leakage, baseline, and split gates remain authoritative.
...
1. **Active six-event evidence milestone — issue #521:** U01/U02 pass. U03 r001 remains an immutable rejection because local snow leaves zero passing windows. Replacement run `BL-2026-07-21-petes-lake-replacement-source-fitness-r002` passes native-pixel, SCL/SNW, usable-fraction, registration-window, continuous-change, and real-render fitness with explicit spatial exclusions. U04 run `BL-2026-07-21-petes-lake-reference-native-contract-r001` passes the exact terms-first MTBS native contract and accepts zero reference pixels. U05 r001 retains one byte-identical NWI metadata response and a false local structure rejection; U05R1 fixes it. R002 retains seven promoted assets, one terminal provider-open failure, and four unexecuted assets. U05R2's offline privacy-safe observability, exact r001/r002 and same-source binding, disjoint r003, retained-byte, no-retry, test, and review gates pass at `1b8c1ad52043536f243b178d279922ceb469103d`. Continue with fresh initialization and the one final disjoint complete run `BL-2026-07-22-petes-lake-nwi-context-r003`; only a complete passing custody package may proceed to U05 wetland/optical-grid/source-precedence fitness.
