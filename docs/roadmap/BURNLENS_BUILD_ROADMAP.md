...
**Current status:** P2O1-T02 / issue #312 narrows the discovery set to one exact Sentinel-2 L2A product and the closest same-day NOAA-21 VIIRS active-fire/geolocation pair. P2O1-T03 / issue #317 then follows the exact LP DAAC delivery routes and proves that unauthenticated responses are Earthdata Login HTML rather than NetCDF/HDF5 source assets, including when the final status is `200`. BurnLens now has a deterministic fail-closed validator and rendered access report, but still has zero provider source assets, pixel arrays, labels, datasets, baselines, models, or maps. Paired intake requires owner approval for both the CDSE and Earthdata credential boundaries.
