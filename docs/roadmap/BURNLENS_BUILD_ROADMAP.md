...
| 2 | Data acquisition, labels, baselines, and dataset versioning | Build a complete, traceable data foundation for one bounded Deschutes County experiment, including authorized sources and AOI, reproducible preprocessing, defensible positive/negative/unknown labels, leakage-resistant splits, non-model baselines, dataset QA, and a model-readiness decision. | **active - incomplete; final bounded sixth-event attempt** | Verified v0.46 defers McKenzie and Milli without acquisition. P2O4-T35 / issue #534 selects only Windigo for one final bounded attempt through July 27, 2026 at 6:00 PM Indianapolis. Any incomplete gate activates the technical-case-study-only portfolio fallback. The accepted label set remains ten balanced regions across five events; dataset, split, baseline, and model remain null. | [Phase Two objectives](../phases/phase-02/PHASE_02_OBJECTIVES.md) |
