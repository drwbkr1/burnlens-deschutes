...
The controlling execution goal remains `v0.0.8-execution-goal-baseline` at `22a8d88435cb8d5b900a398b7482c3b7277d2ee6`. P2O4-T10B is immutable historical reconciliation, not the current acceptance route. Verified `v0.31.0-region-owner-response-intake` accepts six prototype regions only after all gates pass. Verified `v0.32.0-additional-event-groups` freezes three additional event identities. Verified `v0.33.0-green-ridge-source-fitness` passes Green Ridge optical fitness. Verified `v0.34.0-green-ridge-reference-fitness` passes exact official reference fitness as bounded evidence. Verified `v0.35.0-green-ridge-background-evidence` / issue #480 / PR #484 opens a conjunctive affirmative background-evidence route but creates no candidate or label. Issue #483 must deterministically propose bounded Green Ridge burned/background regions before separate owner review. Do not create a dataset or baseline until class/unknown and owner-review gates pass.
