...
The control baseline remains `v0.0.8-execution-goal-baseline` at `22a8d88435cb8d5b900a398b7482c3b7277d2ee6`. P2O1-T01 through P2O1-T03 are shipped and tagged through `v0.1.2-access-integrity-baseline`. P2O2-T01 / issue #321 is the reviewable `v0.2.0-aoi-baseline` candidate and freezes `aoi-darlene3-model-v0.2.0`; shipment identity remains pending. After it ships, the highest-leverage checkpoint remains exact paired asset intake and visual inspection, but it must not begin until the owner explicitly authorizes both CDSE and Earthdata credentials. Geometry and metadata coverage remain evidence of scope, never a detection.
