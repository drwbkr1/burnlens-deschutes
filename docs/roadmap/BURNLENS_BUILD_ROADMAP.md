...
| 3 | CV model development, controlled evaluation, and model versioning | Build and evaluate one bounded U-Net-style segmentation model against the accepted Phase Two baselines, then package either a defensible model or a transparent baseline-only decision with complete lineage, error analysis, and limitations. | **active — U01-U05 passed; U06 next** | P3O1-T01 / issue #566 trains and test-evaluates the only candidate under one consumed opening. It predicts all 89 cores as burned: macro Dice `0.29874213836477986`, IoU `0.21474358974358976`, worst-event Dice `0.2641509433962264`; RBR remains 1.0 and the model is rejected as analytical winner. U06 exact train replay, immutable evaluation verification, package/model card/inference contract, and Phase Four decision remain; no second test opening is authorized. | [Phase Three objectives](../phases/phase-03/PHASE_03_OBJECTIVES.md) |
