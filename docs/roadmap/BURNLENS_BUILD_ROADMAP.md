...
The controlling execution goal remains `v0.0.8-execution-goal-baseline` at `22a8d88435cb8d5b900a398b7482c3b7277d2ee6`. P2O2-T06 / issue #343 accepts the exact pre/post pair and `burn-scar-label-protocol-v0.1.0` for protocol evidence; `aoi-darlene3-model-v0.2.0` remains frozen. The active next checkpoint must measure local content registration and produce a reviewable five-state label proposal with independent QA. It may defer or reject labels and must not create a dataset, baseline, or model before that gate passes.
