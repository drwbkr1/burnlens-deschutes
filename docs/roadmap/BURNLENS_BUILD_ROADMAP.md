...
The controlling execution goal remains `v0.0.8-execution-goal-baseline` at `22a8d88435cb8d5b900a398b7482c3b7277d2ee6`. P2O3-T01 ships the exact pair's local-translation prerequisite at `v0.8.0-content-registration-baseline`; `aoi-darlene3-model-v0.2.0` and `burn-scar-label-protocol-v0.1.0` remain frozen. The active next checkpoint is a reviewable five-state label proposal with independent QA. It may defer or reject labels and must not create a dataset, baseline, or model before that gate passes.
