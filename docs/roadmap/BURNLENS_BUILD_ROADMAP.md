...
1. **Active six-event evidence milestone — issue #521:** U01/U02 pass. U03 r001 rejects the planned post scene because local snow leaves zero passing windows. Metadata-only remediation selects the exact 19 October same-event/same-regime product for contract revision only; commit and record the exact contract, then pass clean preflight, custody, native-pixel, registration, and render gates before continuing the Petes Lake reference/background/proposal/owner chain. Each unit and remediation run retains independent immutable records and failures.
