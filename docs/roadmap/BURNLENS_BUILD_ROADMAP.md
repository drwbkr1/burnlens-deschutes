...
| 2 | Data acquisition, labels, baselines, and dataset versioning | Build a complete, traceable data foundation for one bounded Deschutes County experiment, including authorized sources and AOI, reproducible preprocessing, defensible positive/negative/unknown labels, leakage-resistant splits, non-model baselines, dataset QA, and a model-readiness decision. | **active - incomplete; candidate pilot release and event diversity required** | P2O4-T17 executes the existing-event pilot: six rendered intact candidates pass local evidence gates with zero labels. Release lifecycle and a separately versioned owner-region review remain before metadata-first expansion; three events remain below the six-event gate. | [Phase Two objectives](../phases/phase-02/PHASE_02_OBJECTIVES.md) |
...
**Gate outcome:** 24 prototype points pass owner-response gates but fail dataset sufficiency. P2O4-T16 defines evidence-coherent native-grid regions, one-pixel unknown rings, exact owner review, and at least six whole-event groups with replicated source regimes. P2O4-T17 proves six intact candidates can be rendered reproducibly without promotion. Ship the pilot, review the exact regions through a separately versioned owner surface, then add comparable events before repeating dataset and split gates.
...
The controlling execution goal remains `v0.0.8-execution-goal-baseline` at `22a8d88435cb8d5b900a398b7482c3b7277d2ee6`. P2O4-T10B is immutable historical reconciliation, not the current acceptance route. Verified P2O5-T01 proves the 24 owner-approved center pixels are sparse prototype audit evidence, not masks or an independent split foundation. Verified P2O4-T16 defines the smallest defensible region-first and event/source-regime plan. P2O4-T17 / issue #453 now has a locally verified six-candidate no-promotion release candidate. Preserve whole-event groups and ignore states, review exact regions separately, strengthen both binary classes, and do not create a dataset or baseline until the pilot ships, added events exist, owner review finishes, and a new fitness gate passes.
