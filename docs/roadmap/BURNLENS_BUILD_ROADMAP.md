...
| 3 | CV model development, controlled evaluation, and model versioning | Build and evaluate one bounded U-Net-style segmentation model against the accepted Phase Two baselines, then package either a defensible model or a transparent baseline-only decision with complete lineage, error analysis, and limitations. | **active — U01-U03 passed** | P3O1-T01 / issue #566 locks the exact CPU environment and reference path, then freezes the experiment and exact rendered preflight through `fbb2e92...`. U04 may now run the one substantive training experiment; Ward Creek/Windigo remain sealed. Matching the perfect selected-core baseline is not added value. | [Phase Three objectives](../phases/phase-03/PHASE_03_OBJECTIVES.md) |
