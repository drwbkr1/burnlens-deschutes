...
| 2 | Data acquisition, labels, baselines, and dataset versioning | Build a complete, traceable data foundation for one bounded Deschutes County experiment, including authorized sources and AOI, reproducible preprocessing, defensible positive/negative/unknown labels, leakage-resistant splits, non-model baselines, dataset QA, and a model-readiness decision. | **active - incomplete; exact request accepted; delivery, product fitness, and dataset not complete** | P2O4-T11 confirms seven exact current catalog records. P2O4-T12A preserves the accepted seven-product request and four provider cautions while reporting zero archives received; issue #416 owns exact delivery and fitness. Inter-rater validation, consensus, adjudication, accepted labels, splits, baselines, dataset QA, and model-readiness remain open. | [Phase Two objectives](../phases/phase-02/PHASE_02_OBJECTIVES.md) |
...
**Current status:** P2O1-T02 through P2O3-T01 ship exact route/access, AOI, source/reference, target, optical-pair, and registration evidence. P2O4-T01 through P2O4-T11 ship proposal, cross-event, review, handoff, custody, atomic-intake, owner-waiver readiness, reduced-validation reconciliation, and checkout-stable current cross-program catalog evidence through verified `v0.21.1-reference-inventory-checkout-stability`. P2O4-T12A / #421 / PR #422 is the bounded accepted-request checkpoint: the official queue accepted the exact seven-product/18-family request, four provider cautions bind later fitness, and zero archives were received. One blinded response still yields 6 burned candidate units, 0 background units, 50 ignored units, and no Tepee candidate. Exact delivery and product fitness remain pending under #416. An accepted dataset, split, baseline, model, or deployed analytical application does not exist.

**Gate outcome:** the owner selected the burn-scar fallback; Darlene, Tepee, and McKay proposal/QA evidence exists with uncertainty and groups preserved; the review instrument, isolated handoff, exact current-Chrome software path, response custody, waiver authorization, single-reviewer reconciliation, and current cross-program catalog inventory are executable. The reconciliation chooses remediation, not dataset candidacy. Issue #416 must inspect exact current bundles and strengthen independently traceable burned/background evidence across every event, with weak/conflicting evidence retained as ignored. Do not create a split, baseline, or model until those label-evidence and group-overlap gates pass.
...
The controlling execution goal remains `v0.0.8-execution-goal-baseline` at `22a8d88435cb8d5b900a398b7482c3b7277d2ee6`. P2O4-T10B records the reduced-validation reconciliation and defers the dataset. P2O4-T11 confirms current cross-program availability. P2O4-T12A preserves an accepted exact request without confusing it with delivery. Issue #416 owns exact current bundle terms, bytes, structure, and pixel fitness. Preserve disagreement/uncertainty as ignored, strengthen both binary classes across all events, keep whole-event groups and ignore states binding, and do not begin partition or baseline work early.
