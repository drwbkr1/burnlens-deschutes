...
**Current status:** P2O1-T02 through P2O3-T01 ship the exact route/access, AOI, source/reference, target, optical-pair, and pair-local registration evidence. P2O4-T01 ships a native-grid five-state proposal plus separate all-pixel QA. P2O4-T02 ships exact Tepee/McKay metadata candidates and frozen groups. P2O4-T03 ships through issue #361 / PR #362 and trace-remediation issue #363 / PR #364 at `v0.11.0-cross-event-source-fitness-baseline`: all four exact archives are registered, McKay passes source fitness, Tepee is accepted only with visible SCL/window exclusions, and protocol/schema trace is explicit. Cross-event labels, an accepted dataset, split, baseline, and model do not exist.

**Gate outcome:** the owner selected the burn-scar fallback; the Darlene proposal/QA and two exact additional source pairs are now evidenced. Transfer the five-state proposal to Tepee/McKay and run separate QA with exclusions binding. Do not create a split until cross-event proposal, unknown coverage, and group-overlap evidence pass.
...
The controlling execution goal remains `v0.0.8-execution-goal-baseline` at `22a8d88435cb8d5b900a398b7482c3b7277d2ee6`. P2O4-T03 is shipped at verified `v0.11.0-cross-event-source-fitness-baseline`. Next, test cross-event five-state proposal transfer and separate QA on the exact Tepee/McKay pixels. Keep Tepee exclusions binding and continue to defer partitions.
