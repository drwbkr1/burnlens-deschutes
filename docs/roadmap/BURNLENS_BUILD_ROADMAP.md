...
The controlling execution goal remains `v0.0.8-execution-goal-baseline` at `22a8d88435cb8d5b900a398b7482c3b7277d2ee6`, amended operationally by BL-GOV-003 and `checkpoint-policy-v0.1.0`. P2O4-T10B is immutable historical reconciliation, not the current acceptance route. Verified v0.31 through v0.45 establish ten balanced prototype regions across five events and close Petes Lake with disposition `defer`. Verified v0.46 defers both fallback routes. P2O4-T35 U01-U05 now establish the sixth candidate event without opening dataset work.

August 6, 2026 is the controlling portfolio-submission deadline. The current critical path is:

1. **Final bounded sixth-event attempt:** P2O4-T35 / issue #534 executes only Windigo 2022. U01-U05 pass the exact source, custody, fitness, proposal, pre-reveal response custody, and two-class promotion chain. Windigo is the sixth candidate accepted prototype event.
2. **Deadline decision:** U06 must verify and ship Windigo without opening dataset work, then hand off directly to portfolio assembly. Technical-case-study-only remains the fallback if an unresolved release gate prevents verified shipment.
