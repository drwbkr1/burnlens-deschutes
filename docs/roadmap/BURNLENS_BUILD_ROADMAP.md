...
| 2 | Data acquisition, labels, baselines, and dataset versioning | Build a complete, traceable data foundation for one bounded Deschutes County experiment, including authorized sources and AOI, reproducible preprocessing, defensible positive/negative/unknown labels, leakage-resistant splits, non-model baselines, dataset QA, and a model-readiness decision. | **active - incomplete; reviewer-two gate owner-waived; single-reviewer reconciliation required; dataset not created** | P2O4-T10A / issue #407 is the exact owner-waiver reveal-readiness candidate; it performs zero reveal actions. Once shipped, #403 controls conservative single-reviewer reconciliation. Inter-rater validation and adjudication remain absent; accepted labels, splits, baselines, dataset QA, and model-readiness remain open. | [Phase Two objectives](../phases/phase-02/PHASE_02_OBJECTIVES.md) |
...
**Current status:** P2O1-T02 through P2O3-T01 ship exact route/access, AOI, source/reference, target, optical-pair, and registration evidence. P2O4-T01 through P2O4-T09B ship proposal, cross-event, review, handoff, custody, and atomic-intake readiness through verified `0.18.0`. P2O4-T10A / issue #407 is the `0.19.0` exact owner-waiver reveal-readiness candidate. The owner explicitly waived reviewer two; #393 is superseded and #403 controls later single-reviewer reconciliation with permanently reduced validation. The reveal remains operator-declared unopened, and an accepted dataset, split, baseline, model, or deployed analytical application does not exist.
...
The controlling execution goal remains `v0.0.8-execution-goal-baseline` at `22a8d88435cb8d5b900a398b7482c3b7277d2ee6`. P2O4-T10A / issue #407 is the current bounded checkpoint. It must ship the exact [owner-waiver reveal-readiness decision](../phase-two/objective-four/OWNER_WAIVER_REVEAL_READINESS_DECISION.md) before issue #403 opens the reveal in private reconciliation. Preserve disagreement/uncertainty as ignored. Keep all whole-event groups and ignore states binding; do not begin dataset-candidacy or partition work early.
