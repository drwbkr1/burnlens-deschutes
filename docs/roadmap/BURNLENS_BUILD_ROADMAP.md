...
| 3 | CV model development, controlled evaluation, and model versioning | Build and evaluate one bounded U-Net-style segmentation model against the accepted Phase Two baselines, then package either a defensible model or a transparent baseline-only decision with complete lineage, error analysis, and limitations. | **active — U01 passed** | P3O1-T01 / issue #566 locks and freshly reconstructs the exact PyTorch `2.13.0+cpu` environment before model code or array access. U02 is next. Matching the perfect selected-core baseline is not added value. | [Phase Three objectives](../phases/phase-03/PHASE_03_OBJECTIVES.md) |
