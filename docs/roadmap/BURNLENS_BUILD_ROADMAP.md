...
**Current status:** P2O1-T02 / issue #312 narrows the discovery set to one exact Sentinel-2 L2A product and the closest same-day NOAA-21 VIIRS active-fire/geolocation pair. P2O1-T03 / issue #317 proves that unauthenticated LP DAAC responses are Earthdata Login HTML rather than source assets. P2O2-T01 / issue #321 / PR #322 ships `aoi-darlene3-model-v0.2.0`; P2O2-T02 / issue #325 / PR #326 ships the three-asset registration boundary; P2O2-T03 / issue #329 / PR #330 registers the exact source package and opens the real Sentinel/VIIRS arrays; and P2O2-T04 / issue #333 rejects active-fire direct labels after complete observation comparison. P2O2-T05 activates the burn-scar fallback at verified `v0.6.0-burn-scar-target-baseline`. P2O2-T06 ships the exact 2,254,805,631-byte Sentinel-2A pair at verified `v0.7.0-optical-pair-protocol-baseline`; P2O3-T01 ships pair-local registration at verified `v0.8.0-content-registration-baseline`. P2O4-T01 / #353 is a review-ready `0.9.0` candidate: a native-grid five-state proposal retains 33.4144% explicit ignore, and a separate implementation reproduces all 270,000 state and target pixels with zero disagreement plus a 120-sample audit. Current official MTBS occurrence layers expose no Darlene 3 feature in the frozen AOI. A reviewable proposal now exists; an accepted dataset, split, baseline, and model do not.

**Gate outcome:** the owner selected the burn-scar fallback; the exact pair/protocol and pair-local translation prerequisites are shipped. The one-event proposal and separate software QA gates pass as a review-ready candidate, but this does not prove independent label truth or cross-event generalization. Test cross-event evidence and leakage-resistant grouping before deciding whether dataset/baseline work is defensible.
...
The controlling execution goal remains `v0.0.8-execution-goal-baseline` at `22a8d88435cb8d5b900a398b7482c3b7277d2ee6`. P2O4-T01 / issue #353 implements `burn-scar-five-state-schema-v0.1.0` and passes separate software QA as reviewable one-event evidence. After its merge and release verification, the next bounded checkpoint should test cross-event label support and leakage-resistant grouping. It may reject dataset readiness or choose a baseline-only/stop route; it must not manufacture independence by tiling one event.
