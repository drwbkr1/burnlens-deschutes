...
| 2 | Data acquisition, labels, baselines, and dataset versioning | Build a complete, traceable data foundation for one bounded Deschutes County experiment, including authorized sources and AOI, reproducible preprocessing, defensible positive/negative/unknown labels, leakage-resistant splits, non-model baselines, dataset QA, and a model-readiness decision. | **accepted on the U07 release candidate** | P2O5-T03 versions the dataset and split, passes independent QA, evaluates preregistered baselines, and authorizes one rejection-first U-Net. Live merge/tag verification remains. | [Phase Two objectives](../phases/phase-02/PHASE_02_OBJECTIVES.md) |
| 3 | CV model development, controlled evaluation, and model versioning | Build and evaluate one bounded U-Net-style segmentation model against the accepted Phase Two baselines, then package either a defensible model or a transparent baseline-only decision with complete lineage, error analysis, and limitations. | **next after verified U07** | Execute exactly one CPU-only U-Net under the frozen P2O5-T03-U06 contract; matching the perfect selected-core baseline is not added value. | [Phase Three objectives](../phases/phase-03/PHASE_03_OBJECTIVES.md) |
...
**Current status:** P2O1-T02 through P2O3-T01 ship exact route/access, AOI, source/reference, target, optical-pair, and registration evidence. Twenty-four original owner-approved prototype points remain immutable audit evidence. Verified P2O4-T19 through P2O4-T35 establish 12 balanced regions across six events, while P2O4-T33 preserves Petes Lake's material defer. P2O5-T02 proves the original Darlene-bearing pool cannot support a defensible split, and P2O4-T39 replaces it with Ward Creek. P2O5-T03 / issue #562 accepts `burnlens-dataset-v0.1.0` and `burnlens-whole-event-split-v0.1.0`: 12 native-grid 64-by-64 patches, 287 selected cores, 531 excluded unknown-ring pixels, and locked 2/2/2 whole-event roles. Independent QA reconstructs every patch and freezes normalization from train only. Preregistered baseline evaluation selects RBR at threshold `0.041043221950531006`; it reaches 1.0 Dice and IoU on 89 selected test cores under one sealed test opening. U06 authorizes one rejection-first CPU-only U-Net after verified U07. Restricted thresholded Tepee BARC remains private and unused. No model, weights, training run, model evaluation, inference, or deployed analytical application exists.
