...
**Current status:** P2O1-T02 through P2O3-T01 ship the exact route/access, AOI, source/reference, target, optical-pair, and pair-local registration evidence. P2O4-T01 ships a native-grid five-state proposal plus separate all-pixel QA. P2O4-T02 ships exact Tepee/McKay metadata candidates and frozen groups. P2O4-T03 ships exact source fitness. P2O4-T04 / issue #367 / PR #368 ships verified BurnLens `0.12.0`: two exact MTBS references and four registered Sentinel archives produce cross-event five-state proposals, while a separate implementation reproduces all 63,930 pixels with zero mismatch. An accepted dataset, split, baseline, model, and independent human label review do not exist.
...
The controlling execution goal remains `v0.0.8-execution-goal-baseline` at `22a8d88435cb8d5b900a398b7482c3b7277d2ee6`. P2O4-T04 / issue #367 / PR #368 is shipped at verified `v0.12.0-cross-event-label-transfer-baseline`. Next, create bounded independent label-fitness/adjudication evidence and decide whether the three-event proposal supports dataset candidacy or requires remediation/baseline-only deferral. Keep all whole-event groups and ignore states binding.
