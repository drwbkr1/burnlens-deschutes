...
| 2 | Data acquisition, labels, baselines, and dataset versioning | Build a complete, traceable data foundation for one bounded Deschutes County experiment, including authorized sources and AOI, reproducible preprocessing, defensible positive/negative/unknown labels, leakage-resistant splits, non-model baselines, dataset QA, and a model-readiness decision. | **active — incomplete; final AOI shipped; atomic intake candidate verified; credential use authorized; paired imagery not acquired** | P2O2-T01 ships the 108 km2 AOI. P2O2-T02 / #325 adds a fail-closed exact-pair transaction candidate using temporary synthetic evidence. `ACCESS-2026-006` clears the owner-approval stop; authenticated delivery and pixel-level source fitness remain unverified. | [Phase Two objectives](../phases/phase-02/PHASE_02_OBJECTIVES.md) |
