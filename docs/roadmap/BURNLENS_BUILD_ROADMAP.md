...
| 2 | Data acquisition, labels, baselines, and dataset versioning | Build a complete, traceable data foundation for one bounded Deschutes County experiment, including authorized sources and AOI, reproducible preprocessing, defensible positive/negative/unknown labels, leakage-resistant splits, non-model baselines, dataset QA, and a model-readiness decision. | **active - incomplete; proposal-blinded review-readiness candidate built; dataset not created** | P2O4-T05 / issue #375 adds a 56-unit, three-event review instrument and integrity QA after exact proposal reconstruction. Independent responses/adjudication, accepted labels, splits, baselines, dataset QA, and model-readiness remain open. Latest verified tag remains `v0.12.1-topology-stable-label-transfer` pending candidate shipment. | [Phase Two objectives](../phases/phase-02/PHASE_02_OBJECTIVES.md) |
...
**Current status:** P2O1-T02 through P2O3-T01 ship the exact route/access, AOI, source/reference, target, optical-pair, and pair-local registration evidence. P2O4-T01 ships a native-grid five-state proposal plus separate all-pixel QA. P2O4-T02 ships exact Tepee/McKay metadata candidates and frozen groups. P2O4-T03 ships exact source fitness. P2O4-T04 ships BurnLens `0.12.0`; issue #371 / PR #372 ships the bounded `0.12.1` reproducibility remediation. Real one-link and exact-two-link MTBS packages produce the same ten files byte for byte while a third link fails closed. P2O4-T05 / issue #375 is the current `0.13.0` candidate: exact real-source reconstruction precedes deterministic selection of 56 proposal-blinded review units, and separate integrity QA records zero human responses/adjudications. An accepted dataset, split, baseline, model, and independent human label decision do not exist.

**Gate outcome:** the owner selected the burn-scar fallback; Darlene, Tepee, and McKay proposal/QA evidence now exists with uncertainty and groups preserved, and the review instrument is executable. Next obtain qualifying independent responses, lock their hashes before reveal, adjudicate disagreements or keep units ignored, and make a dataset-candidacy, remediation, or baseline-only deferral decision. Do not create a split, baseline, or model until the label evidence and group-overlap gates pass.
...
The controlling execution goal remains `v0.0.8-execution-goal-baseline` at `22a8d88435cb8d5b900a398b7482c3b7277d2ee6`. P2O4-T05 / issue #375 is the current BurnLens `0.13.0` review-readiness candidate; the latest verified tag remains `v0.12.1-topology-stable-label-transfer` until release gates pass. Next, obtain qualifying independent responses to the proposal-blinded packet, preserve their pre-reveal hashes, adjudicate disagreements or keep units ignored, and decide whether the three-event proposal supports dataset candidacy or requires remediation/baseline-only deferral. Keep all whole-event groups and ignore states binding.
