...
The controlling execution goal remains `v0.0.8-execution-goal-baseline` at `22a8d88435cb8d5b900a398b7482c3b7277d2ee6`. P2O4-T08 is shipped at verified `v0.16.0-first-reviewer-response-lock`. P2O4-T09A / issue #394 is the active bounded readiness candidate: verify the historical first lock and current receipt protocol together with one explicitly non-human software fixture, then ship only if packaging, PR, fresh-main, and tag gates pass. Parent issue #393 remains the next scientific checkpoint: obtain and lock the required second qualifying human response without exposing the first response or opening the reveal. Only then compare and adjudicate disagreements. Keep all whole-event groups and ignore states binding; do not begin dataset-candidacy or partition work early.
