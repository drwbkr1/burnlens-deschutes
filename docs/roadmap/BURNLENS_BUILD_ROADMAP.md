...
The controlling execution goal remains `v0.0.8-execution-goal-baseline` at `22a8d88435cb8d5b900a398b7482c3b7277d2ee6`. P2O4-T10B is immutable historical reconciliation, not the current acceptance route. Verified v0.31 through v0.38 establish eight balanced prototype regions across four events and the complete Green Ridge source/proposal/review/intake path. Verified v0.39-v0.42 pass Grandview optical fitness, exact reference fitness, affirmative-background evidence, and deterministic region proposal. Issue #511 now contains the active v0.43 blank Grandview owner-review surface candidate for one core per class plus explicit unknown rings. Issue #517 owns later exact response intake and is blocked until the surface is verified and the owner supplies a completed export. Petes Lake remains after the complete Grandview path. The cumulative v0.2.0 prototype set remains below the frozen six-event dataset gate. Do not create a dataset, split, or baseline until both remaining event groups pass equivalent source, class, uncertainty, owner, and leakage gates.
