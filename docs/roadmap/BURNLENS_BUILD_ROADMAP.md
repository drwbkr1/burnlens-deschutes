...
| 3 | CV model development, controlled evaluation, and model versioning | Build and evaluate one bounded U-Net-style segmentation model against the accepted Phase Two baselines, then package either a defensible model or a transparent baseline-only decision with complete lineage, error analysis, and limitations. | **active — U01-U04 passed; U05 pre-open passed** | P3O1-T01 / issue #566 freezes the only validation-selected candidate, then preflights the complete evaluator/no-retry path through `aa6b7f3...`. Ward Creek/Windigo remain sealed at zero openings. The exact committed authorization is next; matching the perfect selected-core baseline is not added value. | [Phase Three objectives](../phases/phase-03/PHASE_03_OBJECTIVES.md) |
