...
| 2 | Data acquisition, labels, baselines, and dataset versioning | Build a complete, traceable data foundation for one bounded Deschutes County experiment, including authorized sources and AOI, reproducible preprocessing, defensible positive/negative/unknown labels, leakage-resistant splits, non-model baselines, dataset QA, and a model-readiness decision. | **active - incomplete; 24 prototype labels accepted; dataset not complete** | P2O4-T15 preserves the exact owner response and admits 12 burned plus 12 background prototype labels after reproducibility, source, quality, and event-group gates. Twenty-nine yes and three no/uncertain units remain excluded. Splits, baselines, dataset QA, and model-readiness remain open. | [Phase Two objectives](../phases/phase-02/PHASE_02_OBJECTIVES.md) |
...
**Gate outcome:** the owner selected the burn-scar fallback; Darlene, Tepee, and McKay proposal/QA evidence exists with uncertainty and groups preserved; the exact owner response passes custody and reconstruction; and 24 units pass owner-label, source, reproducibility, quality, and immutable event-group gates. Before any dataset or baseline is created, the next checkpoint must prove sufficient coverage, leakage-resistant partition feasibility, and an honest evaluation role for these sparse reviewed units.
