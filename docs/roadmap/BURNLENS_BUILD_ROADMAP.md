...
| 2 | Data acquisition, labels, baselines, and dataset versioning | Build a complete, traceable data foundation for one bounded Deschutes County experiment, including authorized sources and AOI, reproducible preprocessing, defensible positive/negative/unknown labels, leakage-resistant splits, non-model baselines, dataset QA, and a model-readiness decision. | **active - incomplete; six-event prototype evidence verified** | Verified P2O4-T35 ships `owner-approved-prototype-region-labels-v0.4.0` with 12 balanced regions across six events. The count minimum passes, but the separate sufficiency evaluator has not passed; dataset, split, baseline, and model remain null. The August 6 portfolio path is next. | [Phase Two objectives](../phases/phase-02/PHASE_02_OBJECTIVES.md) |
...
5. **Submission QA and package — candidate active:** P2O4-T37 / issue #544 builds one deterministic August 6 ZIP from verified v0.48. Exact replay, safe dual extraction, reviewer instructions, inventory, desktop/narrow opening, links, images, privacy, and claims pass. Full release gates remain.
