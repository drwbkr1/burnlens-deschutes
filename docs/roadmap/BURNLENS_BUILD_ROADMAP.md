...
**Current status:** P2O1-T02 through P2O3-T01 ship the exact route/access, AOI, source/reference, target, optical-pair, and pair-local registration evidence. P2O4-T01 ships a native-grid five-state proposal plus separate all-pixel QA. P2O4-T02 ships exact Tepee/McKay metadata candidates and frozen groups. P2O4-T03 ships exact source fitness. P2O4-T04 ships BurnLens `0.12.0`; issue #371 / PR #372 is the bounded `0.12.1` reproducibility remediation. Real one-link and exact-two-link MTBS packages now produce the same ten files byte for byte while a third link fails closed. The scientific result and zero-mismatch 63,930-pixel QA are unchanged. An accepted dataset, split, baseline, model, and independent human label review do not exist.

**Gate outcome:** the owner selected the burn-scar fallback; Darlene, Tepee, and McKay proposal/QA evidence now exists with uncertainty and groups preserved. Next obtain independent label-fitness/adjudication evidence and make a dataset-candidacy decision. Do not create a split, baseline, or model until the label evidence and group-overlap gates pass.
...
The controlling execution goal remains `v0.0.8-execution-goal-baseline` at `22a8d88435cb8d5b900a398b7482c3b7277d2ee6`. P2O4-T04 is shipped at verified v0.12.0; issue #371 / PR #372 is the active topology-stable v0.12.1 remediation and changes no analytical output. Once its fresh-main and tag gates pass, resume bounded independent label-fitness/adjudication evidence and decide whether the three-event proposal supports dataset candidacy or requires remediation/baseline-only deferral. Keep all whole-event groups and ignore states binding.
