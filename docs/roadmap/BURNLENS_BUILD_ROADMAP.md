...
The controlling execution goal remains `v0.0.8-execution-goal-baseline` at `22a8d88435cb8d5b900a398b7482c3b7277d2ee6`. P2O4-T10B is immutable historical reconciliation, not the current acceptance route. Verified v0.31 through v0.37 establish six earlier prototype regions, three additional event identities, and the complete Green Ridge source/proposal/review path. P2O4-T26 / issue #491 locks the exact owner response and accepts one Green Ridge region per class only after all gates pass. The cumulative v0.2.0 prototype set is balanced across four events, still below the frozen six-event dataset gate. Issue #495 owns Grandview native source fitness; Petes Lake remains after Grandview. Do not create a dataset, split, or baseline until both remaining event groups pass equivalent source, class, uncertainty, owner, and leakage gates.
