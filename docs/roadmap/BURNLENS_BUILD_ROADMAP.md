...
The controlling execution goal remains `v0.0.8-execution-goal-baseline` at `22a8d88435cb8d5b900a398b7482c3b7277d2ee6`. P2O4-T01 / issue #353 / PR #354 is shipped at verified `v0.9.0-label-proposal-baseline`. The next bounded checkpoint must run the current proposal workflow first, then test cross-event label support and leakage-resistant event/scene/geography/time grouping. It may reject dataset readiness or choose a baseline-only/stop route; it must not manufacture independence by tiling one event.
