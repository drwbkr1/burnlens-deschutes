...
| 2 | Data acquisition, labels, baselines, and dataset versioning | Build a complete, traceable data foundation for one bounded Deschutes County experiment, including authorized sources and AOI, reproducible preprocessing, defensible positive/negative/unknown labels, leakage-resistant splits, non-model baselines, dataset QA, and a model-readiness decision. | **active - incomplete; Petes Lake material-defer candidate** | Verified Green Ridge plus verified P2O4-T32 Grandview intake establish five complete prototype-label events. Issue #521 reaches U11: U01/U02 pass, U03 r001 remains a planned-post failure, replacement U03 passes with explicit spatial exclusions, and U04 passes the exact terms-first MTBS native contract with zero accepted reference pixels. U05 r001-r003 remain immutable; final r003 promotes seven assets before asset eight observes HTTP 500, then stops with four assets unexecuted. U05 scientific fitness and production U06-U10 do not run, Petes Lake is not event six, and the accepted set does not change. | [Phase Two objectives](../phases/phase-02/PHASE_02_OBJECTIVES.md) |
...
1. **Close the Petes Lake material-defer milestone — issue #521:** Final r003 is terminal at seven promoted / one failed / four unexecuted assets after the exact Data Source pre-count request observes HTTP 500. Preserve the complete U01-U11 ledger, keep Petes Lake outside the accepted-event count, ship and verify the coherent BurnLens 0.45.0 candidate, and close #521 without executing U05 scientific fitness or production U06-U10.
2. **Open one bounded official-fallback checkpoint:** Register the official McKenzie HUC8 `17090004` package or a better alternate event under fresh source, terms, custody, scientific-fitness, privacy, and failure-retention gates. Do not create r004 or seed from r001-r003. Prefer metadata/direct-download proof before acquisition and stop if raw-use terms remain unresolved.
3. **Dataset-fitness milestone:** rerun the full sufficiency evaluator across all accepted evidence and decide pass, remediation, fallback, deferral, or stop.
4. **Dataset-and-split milestone, conditional:** create a versioned dataset and leakage-resistant split only if fitness passes every class, unknown, regime, transfer, dominance, and separation gate.
5. **Baseline milestone, conditional:** establish the strongest justified non-model baseline only from the accepted locked dataset and split.
