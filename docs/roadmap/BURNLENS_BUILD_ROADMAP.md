...
1. **Active six-event evidence milestone — issue #521:** U01/U02 pass. U03 r001 rejects the planned post scene because local snow leaves zero passing windows. The exact 19 October same-event/same-regime replacement passes contract and immutable custody at run `BL-2026-07-21-petes-lake-optical-remediation-intake-r001`. Run replacement-pair native-pixel, SCL/SNW, usable-fraction, registration-window, continuous-change, and real-render fitness before continuing the Petes Lake reference/background/proposal/owner chain. Each unit and remediation run retains independent immutable records and failures.
