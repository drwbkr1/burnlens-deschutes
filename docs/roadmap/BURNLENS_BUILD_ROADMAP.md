...
| 2 | Data acquisition, labels, baselines, and dataset versioning | Build a complete, traceable data foundation for one bounded Deschutes County experiment, including authorized sources and AOI, reproducible preprocessing, defensible positive/negative/unknown labels, leakage-resistant splits, non-model baselines, dataset QA, and a model-readiness decision. | **active - incomplete; current reference availability verified; product fitness and dataset not created** | P2O4-T11 confirms seven exact current catalog records across all events, closes label promotion, and ships checkout-stable evidence at 0.21.1; issue #416 owns exact bundle fitness. Inter-rater validation, consensus, adjudication, accepted labels, splits, baselines, dataset QA, and model-readiness remain open. | [Phase Two objectives](../phases/phase-02/PHASE_02_OBJECTIVES.md) |
