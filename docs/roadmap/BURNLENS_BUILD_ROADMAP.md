...
| 2 | Data acquisition, labels, baselines, and dataset versioning | Build a complete, traceable data foundation for one bounded Deschutes County experiment, including authorized sources and AOI, reproducible preprocessing, defensible positive/negative/unknown labels, leakage-resistant splits, non-model baselines, dataset QA, and a model-readiness decision. | **active - incomplete; one-event proposal passes QA and two exact additional event pairs pass source fitness with exclusions; dataset not created** | P2O4-T03 / #361 / PR #362 is analytically merged; issue #363 is the pre-tag trace remediation. Exact cross-event labels, independent human evidence, splits, baselines, and dataset QA remain open. | [Phase Two objectives](../phases/phase-02/PHASE_02_OBJECTIVES.md) |
...
**Current status:** P2O1-T02 through P2O3-T01 ship the exact route/access, AOI, source/reference, target, optical-pair, and pair-local registration evidence. P2O4-T01 ships a native-grid five-state proposal plus separate all-pixel QA. P2O4-T02 ships exact Tepee/McKay metadata candidates and frozen groups. P2O4-T03 / issue #361 / PR #362 has analytical merge `6a6da910849daefa918ed56af6631b2ec44bc211`: all four exact archives are registered, McKay passes source fitness, and Tepee is accepted only with visible SCL/window exclusions. Issue #363 aligns explicit label-protocol/schema trace before tagging. Cross-event labels, an accepted dataset, split, baseline, and model do not exist.

**Gate outcome:** the owner selected the burn-scar fallback; the Darlene proposal/QA and two exact additional source pairs are now evidenced. First merge and verify issue #363 so public trace names both the governing label protocol and implemented schema. After the `0.11.0` candidate ships, transfer the five-state proposal to Tepee/McKay and run separate QA with exclusions binding. Do not create a split until cross-event proposal, unknown coverage, and group-overlap evidence pass.
...
The controlling execution goal remains `v0.0.8-execution-goal-baseline` at `22a8d88435cb8d5b900a398b7482c3b7277d2ee6`. P2O4-T02 remains shipped at verified `v0.10.0-cross-event-feasibility-baseline`; P2O4-T03 / issue #361 / PR #362 is analytically merged, and issue #363 is the fully rendered `0.11.0` trace-remediation candidate. After remediation PR/merge/tag gates pass, test cross-event five-state proposal transfer and separate QA on the exact Tepee/McKay pixels. Keep Tepee exclusions binding and continue to defer partitions.
