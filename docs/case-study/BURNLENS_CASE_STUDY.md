...
The highest remaining risk is still burn-scar label truth across events. BurnLens now has a candidate set of 12 balanced owner-approved prototype regions across Darlene, McKay, Tepee, Green Ridge, Grandview, and Windigo. Windigo's exact 2,373,112,076-byte Sentinel pair and 12,526,858-byte BAER/RAVG/MTBS delivery pass custody with failed attempts exposed. One burned and one affirmative-background region advance only after the sole exact owner response is locked before reveal and every source-pixel, terms, raster, quality, uncertainty, leakage, and reproducibility gate re-passes. The six-event count minimum passes. It still does not establish independent truth or authorize a dataset.

P2O5-T02 runs the full sufficiency evaluator rather than treating six as a success condition. It reconstructs all 12 rasters, 286 accepted core pixels, 533 excluded unknown-ring pixels, and every whole-event 2/2/2 assignment. Eight gate families pass. Leakage/split fitness and evaluation design block. Darlene is the only NIFC-context event, so all 90 assignments confound that program and exact regime with one split role. The independent audit agrees and keeps `training_authorized` false.

The smallest credible remediation is one complete replacement event under the already replicated MTBS-current or BAER/MTBS/RAVG-current regime, followed by exclusion of Darlene and a full rerun. That work must repeat source, terms, custody, native-pixel, two-class, owner-review, uncertainty, and leakage gates. The verified August 6 ZIP remains useful as an interim technical case study, but it does not satisfy BurnLens's intended model-bearing portfolio threshold.

Verified BurnLens 0.50.0 preserves that negative result as a release-quality evidence checkpoint. Merge `85f827e...` reproduces all six outputs, passes 595 tests with one expected skip and 86 subtests, rebuilds the exact 899,515-byte wheel twice, passes an isolated 89-command install, and retains owner-confirmed desktop/narrow rendering. Annotated tag object `915ca1e...` peels to the merge. Issue #554 owns the bounded replacement-event attempt; no dataset or model is implied by this release.

Issue #554 then completes Ward Creek as a seventh prototype event. Its exact
owner response is locked before reveal, and both proposed classes pass every
source, custody, quality, uncertainty, leakage, and reproducibility gate.

The replacement candidate uses McKay, Tepee, Green Ridge, Grandview, Windigo,
and Ward Creek. Darlene remains preserved but excluded. The candidate contains
287 accepted core pixels and 531 explicit unknown-ring exclusions.

All ten readiness gates pass. Fifty-four whole-event 2/2/2 assignments satisfy
the frozen transfer, source-program, and source-regime rules. The independent
audit agrees byte for byte and keeps training unauthorized.

This is the strongest Phase Two result so far, but it is not a dataset or
...
One burned and one background Grandview region pass, adding 50 core pixels / 2.00 ha while all 98 ring pixels remain unknown and excluded. Verified label set `owner-approved-prototype-region-labels-v0.3.0` was balanced at five regions per class, 236 core pixels / 9.44 ha, and 431 excluded ring pixels across five events. BL-GOV-003 then groups later work into complete milestones. Petes Lake, McKenzie, and Milli stop honestly. Windigo later advances candidate v0.4 to six regions per class and six events. That is better prototype evidence, not independent ground truth, a dataset, a split, a baseline, a model, or field validation.
