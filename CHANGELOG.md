...
- Preserve the lean runtime/dev plus opt-in `geo-research` design. All 81 commands load and show help without optional geo packages; real Petes reference-fitness execution exits 2 with a bounded profile instruction when GeoPandas, Pyogrio, PyProj, or Shapely is absent or its installed stack is unusable.
- Retain the 801,911-byte / `055e56b1...` pre-review wheel after exact-head review found that installed-but-unusable optional geo imports could escape the bounded error contract. Correct package-sensitive code `4bf76ea...` sanitizes import-time `ImportError`, `OSError`, `AttributeError`, and other `Exception` failures before repository preflight and adds exact adversarial coverage.
