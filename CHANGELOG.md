...
- Add `paired-intake-contract-v0.3.0`, pinning the exact Sentinel-2 SAFE ZIP and NOAA-21 VIIRS active-fire/geolocation files, identities, sizes, containers, pair token, available provider checksums, and transaction invariants under one digest.
...
Candidate report-generator source is commit `41f86dec62f0a5e8cd159c75932999790ab0f840` on issue #325 / PR #326. Thirty-six tests, isolated install/dependency checks, deterministic reconstruction, original-resolution visual review, claims review, and zero-secret/provider-byte checks pass. This checkpoint does not prove provider delivery, real-file integrity, source fitness, fire presence, label readiness, a dataset, baseline, model, application, deployment, or performance.
