...
- Record PR #527, reviewed head `925b660...`, true merge `7d5c07e...`, and issue #521 closure while retaining REGISTRY-2026-059 as immutable candidate-stage evidence.
- Retain the 800,916-byte / `4e0bd99f...` merged wheel as failed BL-EXC-001 U01 evidence: its isolated base-runtime install loads only 80 of 81 entry points, and clean `dev` cannot collect the documented suite.
- Preserve the lean runtime/dev plus opt-in `geo-research` design. All 81 commands load and show help without optional geo packages; real Petes reference-fitness execution exits 2 with a bounded profile instruction when GeoPandas, Pyogrio, PyProj, or Shapely is unavailable.
- Pass U03 at final code `5e55ec8...`: fresh dev 499/25, fresh geo 515/23, canonical custody-enabled 537/1, all with 78 subtests; exact U03/U04 products remain unchanged; fresh `uv.lock` is checkout-stable.
- Produce the corrected 801,911-byte wheel / SHA-256 `055e56b1...` with 170 exact entries, 81 unchanged commands, and complete RECORD coverage from two separate fresh no-hardlink clones at pushed evidence checkpoint `184bfc9...`; earlier builds at `77d32c5...` and `5e55ec8...` retain the same identity.
...
PR #518 reviewed head `248f601485c813d7dd2f629195ead75eb325465f` squash-merges at checkpoint `aeea6f5cc488ae975badbdf654d0164570db77c4`. A new remote-main clone passes 313 tests with 17 expected custody skips, compilation, 111 JSON parses, zero surface JSON/HTML CRLF, all six exact artifact hashes, and two canonical wheels. Annotated tag object `d679521edcc1d5b1bfd3e8022036cae06a9f1978` remotely peels to the checkpoint. Lifecycle issue #519 / PR #520 records the release; issue #517 remains blocked on a later completed owner export.
...
Generator `78bb79eb04d76cff4ed183274de881557b176413` creates run `BL-2026-07-20-grandview-region-proposal-r001`. Public JSON/HTML/PNG and two EPSG:32610 rasters are exact and render actual optical, dNBR, MTBS/background-route, core, and unknown-ring evidence. The custody worktree passes all 326 tests in two complete bounded shards; fresh remote head passes 312 tests with 14 expected custody skips, 108 JSON parses, and exact five-output reconstruction. Two fixed-epoch 602,463-byte wheels are byte-identical at SHA-256 `15e15173b345df7caae4b12bb03975df9e0646345bd890bc9fdd0ec62ef192d2`; isolated import and dependency health pass. At the analytical candidate stage, merged-main and tag verification remain pending; issue #511 owns the separately versioned blank owner surface.

PR #512 squash-merges analytically at `0905711e3f1dc2e281ff830ff3b9d94ed2c50f66`. Fresh merged main passes tests and reproduces HTML, PNG, and both rasters, but the tracked JSON is checked out as 4,733 CRLF bytes rather than the generator's original 4,620 LF bytes. Normalizing line endings makes the JSON identical. The tag is correctly withheld; issue #513 / PR #514 adds only the missing Grandview proposal LF checkout contract. Fresh remediation source `d3b11e78e03492f8348b980f7304864ded85b06a` and reviewed PR head `18f1b89c360a2e35e36e03514ed37522527fb19f` reconstruct all five original artifact hashes and pass 312 tests with 14 expected custody skips, 108 JSON parses, and canonical packaging. The candidate package remains reproducible under its recorded fixed epoch.

PR #514 merges at verified checkpoint `6124f76de04b5e503a9da7655a293a023480b39c`. A new remote `main` clone contains zero CRLF sequences in the JSON and HTML, preserves all five artifact hashes, passes 312 tests with 14 expected custody skips, compilation, and 108 JSON parses, and builds two byte-identical 602,463-byte wheels / SHA-256 `15e15173b345df7caae4b12bb03975df9e0646345bd890bc9fdd0ec62ef192d2` under the recorded `SOURCE_DATE_EPOCH=1784590790`. An isolated dependency-complete install imports BurnLens 0.42.0 from `site-packages`, exposes 67 commands including the Grandview proposal CLI, and passes dependency health. Annotated tag object `388c81677b3c49d2312779f8fc98ad3dc7b9048c` remotely peels to the checkpoint. Lifecycle issue #515 / PR #516 records the release; issue #511 remains next.

## v0.41.0-grandview-background-evidence - verified - 2026-07-20

### P2O4-T29 - Establish affirmative Grandview background evidence

- Rerun verified v0.40.0 first and preserve its affirmative-background blocker.
- Freeze and acquire one exact 950,987,607-byte Sentinel-2B near-anniversary scene under current CDSE identity, checksums, terms, attribution, and ignored no-overwrite custody.
- Apply metadata-derived BOA scaling across the disclosed 05.00/05.10 processing-baseline difference.
- Accept 7/9 registration windows with two review-needed windows spatially excluded; p95 residual is 0.0944 pixel and no window fails.
...
PR #504 squash-merges at checkpoint `cdbe49883fb5330c9e7af036a847841b714dafae`. Fresh merged main repeats 303 tests with 13 expected custody skips, compilation, 104 JSON parses, exact 3-of-3 reconstruction through unchanged ignored custody, and canonical wheel identity. Annotated tag object `48c1c9677b9b41a734be2dcdc4a9691caf4e6688` remotely peels to the checkpoint.
...
- Validate the exact hash-named two-record export and atomically preserve response plus receipt in ignored no-overwrite custody before reveal.
- Re-run exact proposal/raster, source/terms, quality/registration, uncertainty-ring, reproducibility, and event-identity gates.
- Accept one burned and one background Green Ridge core into `owner-approved-prototype-region-labels-v0.2.0`; retain all 87 ring pixels as unknown and excluded.
...
Generator `690137f1530a00ca79b8f5831f700541809b2a2b` produces run `BL-2026-07-20-green-ridge-region-proposal-r001`. PR #488 merges at checkpoint `71c3ffedeb2bb54bbeca8e97686c8e37e710e354`. Fresh main passes 287 tests, 95 JSON parses, exact 5-of-5 reconstruction, and canonical 531,038-byte packaging. Tag object `bc0110afa0c4ccf6e3314d394d544cfde16aaae9` remotely peels to the checkpoint. Lifecycle #489 / PR #490 records the release; issue #487 owns the blank owner yes/no/uncertain surface. Dataset, split, baseline, model, metric, accuracy, ground-truth, field, official, endorsed, and operational claims remain absent.
...
- Begin from verified v0.34 and reproduce the zero-background-evidence blocker before changing the route.
- Freeze and acquire one exact 1,193,992,663-byte near-anniversary Sentinel-2B L2A scene under current CDSE identity, checksum, terms, attribution, and quota boundaries; provider bytes remain ignored.
- Pass nine of nine fixed content-registration windows, with 0.02-pixel median and 0.03-pixel maximum residual.
...
- Preserve the exact 1,635-byte completed owner export without overwrite in ignored repository-local custody.
- Bind all six answers to the shipped surface, pilot report, candidate order, raster hashes, and exact source/terms/review records.
- Reopen every GeoTIFF and verify native grids, CRS, nodata, class domain, connected cores, one-pixel unknown rings, quality/registration, and event identity.
- Accept six owner-approved prototype regions: three burned and three background, 136 core pixels / 5.44 hectares across three events.
...
Generator source is `7fedd84c4ac9c732f698cf3b40b07e70bfe01cc6`; public artifacts are `f0412fd85092f00a5122b775c3e07891d2f156d7`; issue #457 / PR #459 merges at `674e9edb04a4016ef71cad805be52f4459670d6a`. Final review closes the completed-session file loader so a locked response cannot be replaced. Fresh main passes 246 tests, compilation, dependency health, 82 JSON parses, exact ten-file reconstruction, and the canonical 450,787-byte wheel / SHA-256 `760a4a94cc7befc449a7121e4e4ab54b9789c637d129d35c197321a376d54f1b`; isolated 0.30.0 exposes 44 commands and 96 clean entries. Annotated tag object `dc0b27e47a71353d8e1e7c8691dbf6b9c2a7598f` remotely peels to the merge. Lifecycle #460 / PR #462 records the release; issue #461 owns exact owner-response intake.
...
Generator source is `407981e1f98570bb8bd7695951d6cc5d67dab042`; public artifacts are `0e5306c6d94c54b47df40486a75e5bbd69bab62f`; issue #453 / PR #455 merges at `3d3b0128f1f4e4b4aab8e14fc3fabc089fbd8f0a`. Fresh main passes 241 tests with exact ignored owner-intake bytes restored, compilation, dependency health, 79 JSON parses, exact 10-of-10 reconstruction, responsive real-browser QA, and two byte-identical 440,972-byte wheels / SHA-256 `d97e11058aaf18c8a146df3eb5392788061c5daa784620bd20ac1c09e8197d35`. Isolated 0.29.0 exposes 43 commands and 94 clean entries. Annotated tag object `55adb8e376988cc3dc4d6bd662e2a5200b8fe1ed` remotely peels to the merge. Lifecycle #456 / PR #458 records the release; issue #457 owns the separate six-candidate owner-region review surface.
...
Generator source is `89e69172163c97ea8f4c86e72867578065626f22`; issue #443 / PR #445 merged the analysis at `dbc24c57442d0c2564ce7ae4e4da17a2c966b910`. Issue #446 / PR #447 supplies the verified release checkpoint `bf98a47040be589dad07bcfd6767015ae2e6ee81`. Fresh main preserves the exact HTML bytes, passes 228 tests and exact three-output reconstruction, and produces two byte-identical 422,069-byte wheels / SHA-256 `741eb378433c8240809fb9de144b4db6884a2612a335e2d26be71e376bfe41da`. Annotated tag object `51f39bc68b725fb1b7cbd696f593fcd17316a8ca` remotely peels to the remediation merge. Lifecycle sync is #448; issue #449 owns the next bounded label-coverage plan.
