...
| Current repository baseline | Latest verified: `v0.20.0-single-reviewer-reconciliation`; `v0.21.0-current-reference-inventory` is preserved as an analytical tag with failed release verification; BurnLens `0.21.1` / issue #417 is the checkout-stability candidate |
...
| Baseline verification | 0.21.0 passed analytical, test, live-inventory, packaging, and remote-tag checks but failed exact fresh-checkout manifest reconstruction because JSON/HTML line endings were checkout-dependent. The 0.21.1 candidate adds explicit LF rules; clean-checkout exact hashes, 188 tests, deterministic 348,032-byte wheel, isolated install, and privacy gates pass. PR/merge/tag/fresh-main verification remains pending. |
