...
| Current phase posture | Phase Two active under `checkpoint-policy-v0.1.0`; P2O4-T35 U01-U05 accept Windigo as the sixth candidate owner-approved prototype event. The count minimum passes while dataset fitness remains a separate closed gate. |
...
| Baseline verification | Verified v0.46 remains the repository baseline. BurnLens 0.47.0 is a release-ready candidate that adds exact Windigo response reconciliation and a sixth prototype event. Owner-confirmed HTML rendering, full tests, reproducible package, isolated install, and claim/privacy audits pass; PR, merge, fresh-main output/package, and tag gates remain pending. |
