...
| Current phase posture | Phase Two active under `checkpoint-policy-v0.1.0`; verified v0.44 establishes ten balanced prototype regions across five complete events; issue #521 reaches a material-defer exit after terminal Petes r003; dataset fitness remains a separate closed gate |
| Current repository baseline | Latest verified analytical tag remains `v0.44.0-grandview-owner-response-intake` at merge `5e1d5a05dbb09e8ac42be5928b2d042a0737336e`; tag object `e4f834cd3c55d44895766695a40746fa224df9bd`. BurnLens `0.45.0` / candidate tag `v0.45.0-petes-lake-material-defer` is pending milestone PR, merged-main replay, and remote-tag verification. |
...
| Baseline verification | Verified v0.44 locks the exact Grandview response before reveal, reconstructs its outputs from fresh main, passes its complete release gate, and verifies the remote annotated tag. The v0.45 candidate must separately pass r001-r003 custody validation, actual Petes render/native replay, full regression, canonical package, isolated install, merged-main reconstruction, and remote annotated-tag verification. |
