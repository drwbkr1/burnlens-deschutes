...
| Current phase posture | Phase Two active under `checkpoint-policy-v0.1.0`; verified v0.46 preserves ten balanced prototype regions across five complete events and defers McKenzie and Milli without provider bytes. P2O4-T35 gives only Windigo one final bounded attempt through the July 27 cutoff; dataset fitness remains a separate closed gate. |
...
| Baseline verification | Verified v0.46 adds a deterministic two-route source gate whose exact HTML passes owner-confirmed desktop/narrow rendering. PR #533, true merge `0e58459e...`, fresh-main package/output gates, and annotated tag object `ec2aad7a...` peeling exactly to the merge pass. T35 U01 is an evidence-unit checkpoint, not a new software release. |
