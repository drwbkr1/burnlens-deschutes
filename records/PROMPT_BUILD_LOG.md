...
The controlling execution goal and six-phase roadmap are merged and tagged at `v0.0.8-execution-goal-baseline`. Phase Two is active. P2O4-T12A ships verified accepted-request evidence at `v0.22.0-current-bundle-request-evidence`; delivery remains pending under #416. Candidate P2O4-T13 maps seven official source classes and 21 additional events without duplicating that request or acquiring large products. The exact historical response still yields immutable 6 burned / 0 background / 50 ignored evidence, but the owner-confirmed route will reopen all 56 units with disclosed evidence and yes/no/uncertain. No owner response, independent review, inter-rater validation, accepted label set, dataset, split, baseline, or model exists. Authorized credentials are used without retaining secrets; exact raw packages, private response bytes, receipts, fixture bytes, authorization, unit-level reconciliation, queue response, retrieval details, and provider bundles remain local/ignored. Historical Objective Seven restrictions that conflict with the controlling goal are retained only as audit evidence and do not govern current execution.
