...
The controlling execution goal and six-phase roadmap are merged and tagged at `v0.0.8-execution-goal-baseline`. Phase Two is active. Verified P2O4-T12 supplies exact current seven-product evidence; verified P2O4-T14 at `v0.25.0-owner-confirmed-review-surface` reopens all 56 units with disclosed evidence and blank yes/no/uncertain responses. Issue #437 owns actual response intake. The historical 6 burned / 0 background / 50 ignored result remains immutable history and is not inherited. No owner response, independent review, inter-rater validation, accepted label set, dataset, split, baseline, or model exists. Authorized credentials are used without retaining secrets; exact raw packages, private response bytes, receipts, fixture bytes, authorization, unit-level reconciliation, queue response, retrieval details, and provider bundles remain local/ignored. Historical Objective Seven restrictions that conflict with the controlling goal are retained only as audit evidence and do not govern current execution.
