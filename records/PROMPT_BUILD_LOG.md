...
The controlling execution goal and six-phase roadmap are merged and tagged at `v0.0.8-execution-goal-baseline`. Phase Two is active. Verified P2O4-T12 supplies exact current seven-product evidence; verified P2O4-T14 supplies the owner surface; candidate P2O4-T15 accepts 24 explicitly owner-approved prototype labels after exact-response and promotion gates. The historical 6 burned / 0 background / 50 ignored result remains immutable history and is not inherited. No independent review, inter-rater validation, accepted dataset, split, baseline, or model exists. Authorized credentials are used without retaining secrets; exact raw packages, private response bytes, receipts, unit-level reconciliation, retrieval details, and provider bundles remain local/ignored. Historical Objective Seven restrictions that conflict with the controlling goal are retained only as audit evidence and do not govern current execution.
