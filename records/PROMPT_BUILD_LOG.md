...
The controlling execution goal and six-phase roadmap are merged and tagged at `v0.0.8-execution-goal-baseline`. Phase Two is active. Verified P2O5-T01 at `v0.27.0-prototype-label-sufficiency` proves the 24 owner-approved center pixels are insufficient; verified P2O4-T16 at `v0.28.0-label-region-remediation-plan` defines region-first review and six-event/source-regime gates; issue #453 owns the no-promotion pilot. The historical 6 burned / 0 background / 50 ignored result remains immutable history and is not inherited. No region labels, independent review, inter-rater validation, accepted dataset, split, baseline, or model exist. Authorized credentials are used without retaining secrets; exact raw packages, private response bytes, receipts, unit-level reconciliation, retrieval details, and provider bundles remain local/ignored. Historical Objective Seven restrictions that conflict with the controlling goal are retained only as audit evidence and do not govern current execution.
