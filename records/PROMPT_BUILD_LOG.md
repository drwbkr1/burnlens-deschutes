...
| P2O4-T29 | `records/prompt-build-log/2026-07-20-p2o4-t29.md` | issue #503; BurnLens 0.41.0 release candidate | Acquires one exact near-anniversary Grandview scene, enforces baseline-aware reflectance and registration exclusions, uses exact delivered program vectors, and opens a background-evidence route with zero candidates, owner responses, labels, dataset, split, baseline, or model. |
...
The controlling execution goal and six-phase roadmap are merged and tagged at `v0.0.8-execution-goal-baseline`. Phase Two is active. P2O4-T26 operator-locks the exact Green Ridge response before reveal and accepts two prototype regions only after all gates pass. `owner-approved-prototype-region-labels-v0.2.0` is balanced at eight regions across four events. Verified P2O4-T27-T28 pass Grandview optical and reference fitness. P2O4-T29 / issue #503 is the BurnLens 0.41.0 candidate: one exact near-anniversary scene, delivered vector footprints, unchanged stability thresholds, and registration exclusions open a background-evidence route while creating zero candidates or labels. Petes Lake remains before the frozen six-event dataset gate. The historical 6 burned / 0 background / 50 ignored result remains immutable history and is not inherited. No independent review, inter-rater validation, accepted dataset, split, baseline, or model exists. Authorized credentials are used without retaining secrets; exact raw packages, private response bytes, receipts, unit-level reconciliation, retrieval details, and provider bundles remain local/ignored. Historical Objective Seven restrictions that conflict with the controlling goal are retained only as audit evidence and do not govern current execution.
