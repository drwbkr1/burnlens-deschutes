...
The controlling execution goal and six-phase roadmap are merged and tagged at `v0.0.8-execution-goal-baseline`. Phase Two is active. P2O4-T11 ships checkout-stable current catalog evidence at verified `v0.21.1-reference-inventory-checkout-stability`; failed 0.21.0 release evidence remains preserved. One exact blinded response still yields 6 burned candidate units, 0 background units, and 50 ignored units. Seven exact current BAER/RAVG/MTBS catalog records pass across Darlene, McKay, and Tepee, but product fitness remains pending and zero labels move. Reviewer two remains waived and absent; no inter-rater validation, consensus, adjudication, dataset, split, baseline, or model exists. Issue #416 owns exact current bundle acquisition and fitness. Authorized credentials are used without retaining secrets; exact raw packages, private response bytes, receipts, fixture bytes, authorization, unit-level reconciliation, and provider bundles remain local/ignored. Historical Objective Seven restrictions that conflict with the controlling goal are retained only as audit evidence and do not govern current execution.
