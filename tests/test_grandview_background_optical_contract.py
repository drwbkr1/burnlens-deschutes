...
from burnlens.grandview_background_optical_contract import (
...
def metadata_payload() -> dict:
    return {
        ...
        "PublicationDate": "2024-12-10T16:00:22.516015Z",
        "S3Path": f"/eodata/Sentinel-2/MSI/L2A_N0500/2022/06/28/{EXTENDED_CONTRACT.native_id}",
        "Checksum": [
...
class GrandviewBackgroundOpticalContractTests(unittest.TestCase):
    def test_exact_extended_asset_contract_passes(self) -> None:
        ...
        self.assertEqual(EXTENDED_CONTRACT.expected_size_bytes, 950_987_607)
        self.assertIn("05.10", ROUTE_PRECEDENCE["baseline_boundary"])

    def test_scene_identity_drift_fails_closed(self) -> None:
        changed = replace(EXTENDED_CONTRACT, native_id=EXTENDED_CONTRACT.native_id.replace("R113", "R013"))
        self.assertIn(
    ...
    def test_live_metadata_shape_is_exact_and_private_route_absent(self) -> None:
        ...
        snapshot = refresh_background_optical_metadata(
            observed_at_utc="2026-07-20T22:00:00Z", urlopen_fn=fake_urlopen
        )
    ...
    def test_checksum_drift_fails_closed(self) -> None:
        ...
        with self.assertRaisesRegex(AcquisitionError, "PUBLIC_METADATA_DRIFT"):
            refresh_background_optical_metadata(
                observed_at_utc="2026-07-20T22:00:00Z",
