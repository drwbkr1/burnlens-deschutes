...
import burnlens
from burnlens.ward_creek_owner_response_intake import (
...
PROPOSAL = (
    ROOT
    / "samples/labels/pilot/ward-creek/phase-two/region-proposal-v0.1.0/"
    "WARD-CREEK-REGION-PROPOSAL-2026-001.json"
)
SURFACE = (
    ROOT
    / "samples/labels/review/ward-creek/phase-two/"
    "WARD-CREEK-OWNER-REVIEW-SURFACE-2026-001.json"
)
LOCKED = (
    ROOT
    / "downloads/phase-two/custody/ward-creek/"
    "owner-review-response-v0.1.0"
)
RESPONSE = (
    LOCKED
    / "WARD-CREEK-OWNER-REVIEW-SURFACE-2026-001-"
    "RESPONSE-aadd221da037ab7f.json"
)
RECEIPT = (
    LOCKED
    / "WARD-CREEK-OWNER-REVIEW-SURFACE-2026-001-"
    "RECEIPT-aadd221da037ab7f.json"
)
PUBLIC_DIRECTORY = (
    ROOT
...
BOUND_RECORD_PATHS = (
    ...
    "records/phase-two/prechecks/PRECHECK-2026-074.md",
    "records/phase-two/prechecks/PRECHECK-2026-076.md",
    "records/phase-two/prechecks/PRECHECK-2026-078.md",
    "records/phase-two/prechecks/PRECHECK-2026-079.md",
    "records/phase-two/prechecks/PRECHECK-2026-080.md",
    "records/phase-two/prechecks/PRECHECK-2026-081.md",
    "records/phase-two/prechecks/PRECHECK-2026-082.md",
    "records/phase-two/prechecks/PRECHECK-2026-083.md",
    "records/phase-two/prechecks/PRECHECK-2026-084.md",
)


def _ignored_temporary_directory():
    root = ROOT / "downloads/phase-two/runs/P2O4-T39-U07"
...
@unittest.skipUnless(
    PRIVATE_AVAILABLE,
    "exact ignored Ward Creek custody is unavailable",
)
class WardCreekOwnerResponseIntakeTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        with _ignored_temporary_directory() as temporary:
            cls.private = build_private_reconciliation(
                ...
                generated_at_utc="2026-07-25T04:45:00Z",
                run_id="BL-TEST-WARD-CREEK-OWNER-RESPONSE-INTAKE",
                git_source_commit=head,
            )

    def test_version_entry_point_and_exact_custody_constants(self) -> None:
        self.assertEqual(burnlens.__version__, "0.50.0")
    ...
    def test_two_yes_and_all_non_owner_gates_accept_only_prototype_event(
        self,
    ) -> None:
        private = self.private
        self.assertEqual(
            private["decision_counts"],
            {"yes": 2, "no": 0, "uncertain": 0},
        )
        self.assertEqual(private["label_set_version"], LABEL_SET_VERSION)
        self.assertTrue(private["outcome"]["ward_creek_event_complete"])
        self.assertTrue(private["outcome"]["no_partial_event_promotion"])
        self.assertEqual(
            private["outcome"]["ward_creek_owner_approved_region_labels"],
            2,
        )
        self.assertEqual(
            private["outcome"]["ward_creek_class_counts"],
            {"background": 1, "burned": 1},
        )
        self.assertEqual(
            private["outcome"]["ward_creek_accepted_core_pixels"],
            39,
        )
        self.assertEqual(
            private["outcome"]["ward_creek_excluded_unknown_ring_pixels"],
            66,
        )
        self.assertEqual(
            private["outcome"]["cumulative_owner_approved_region_labels"],
            14,
        )
        self.assertEqual(
            private["outcome"]["cumulative_prototype_label_class_counts"],
            {"background": 7, "burned": 7},
        )
        self.assertEqual(
            private["outcome"]["cumulative_accepted_core_pixels"],
            325,
        )
        self.assertEqual(
            private["outcome"]["cumulative_accepted_core_area_ha"],
            13.0,
        )
        self.assertEqual(
            private["outcome"]["cumulative_excluded_unknown_ring_pixels"],
            599,
        )
        self.assertEqual(private["outcome"]["event_group_count"], 7)
        self.assertTrue(
            private["outcome"]["six_event_sufficiency_rerun_eligible"]
        )
        self.assertFalse(
            private["outcome"]["separate_sufficiency_evaluator_passed"]
        )
        self.assertFalse(private["outcome"]["dataset_fitness_reopened"])
        self.assertFalse(private["outcome"]["training_authorized"])
    ...
    def test_every_candidate_reconstructs_from_exact_source_pixels(self) -> None:
        ...
        self.assertEqual(
            [unit["proposed_class"] for unit in units],
            ["burned", "background"],
        )
        self.assertEqual(
            [unit["core_pixels"] for unit in units],
            [14, 25],
        )
        self.assertEqual(
            [unit["unknown_ring_pixels"] for unit in units],
            [26, 40],
        )
        for unit in units:
    ...
    def test_public_report_is_aggregate_only_and_no_overwrite(self) -> None:
        with _ignored_temporary_directory() as temporary:
            ...
            binding = write_private_no_overwrite(
                ROOT,
                private_path,
                self.private,
            )
            ...
            serialized = (
                public_directory / f"{REPORT_ID}.json"
            ...
            html = (
                public_directory / f"{REPORT_ID}.html"
            ).read_text(encoding="utf-8")
            self.assertIn(
                "The seven-event prototype pool is not a dataset",
                html,
            )
    ...
    def test_public_candidate_is_aggregate_only_and_self_bound(self) -> None:
        ...
        self.assertEqual(
            report["outcome"]["cumulative_owner_approved_region_labels"],
            14,
        )
        self.assertEqual(report["outcome"]["event_group_count"], 7)
        self.assertFalse(report["outcome"]["dataset_fitness_reopened"])
        self.assertFalse(report["outcome"]["training_authorized"])
