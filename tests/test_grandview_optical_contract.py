...
from burnlens import acquire_grandview_optical as acquire_cli
from burnlens.grandview_optical_contract import (
    GRANDVIEW_CONTRACTS,
    EXPECTED_METADATA,
    ROUTE_PRECEDENCE,
    CdseCredentials,
...
def metadata_payload(role: str) -> dict:
    contract = next(item for item in GRANDVIEW_CONTRACTS if item.role == role)
...
class GrandviewOpticalContractTests(unittest.TestCase):
    def test_two_assets_form_one_exact_grandview_pair(self) -> None:
        self.assertEqual(validate_grandview_contracts(GRANDVIEW_CONTRACTS), [])
        self.assertEqual(len(GRANDVIEW_CONTRACTS), 2)
        self.assertEqual(sum(item.expected_size_bytes for item in GRANDVIEW_CONTRACTS), 1_923_481_794)

    def test_pair_geometry_drift_fails_closed(self) -> None:
        changed = replace(
            GRANDVIEW_CONTRACTS[1],
            native_id=GRANDVIEW_CONTRACTS[1].native_id.replace("R113", "R013"),
        )
        self.assertIn(
            "GRANDVIEW_PAIR_GEOMETRY_IDENTITY_MISMATCH",
            validate_grandview_contracts((GRANDVIEW_CONTRACTS[0], changed)),
        )

    def test_live_metadata_shape_accepts_only_exact_odata_contract(self) -> None:
        payloads = {item.provider_id: metadata_payload(item.role) for item in GRANDVIEW_CONTRACTS}

        def fake_urlopen(request: Request, timeout: float) -> FakeResponse:
            provider_id = next(key for key in payloads if key in request.full_url)
            return FakeResponse(payloads[provider_id])

        snapshot = refresh_grandview_metadata(
            observed_at_utc="2026-07-20T18:20:00Z", urlopen_fn=fake_urlopen
        )
        self.assertEqual(validate_grandview_metadata(snapshot), [])
    ...
    def test_metadata_checksum_drift_fails_closed(self) -> None:
        payloads = {item.provider_id: metadata_payload(item.role) for item in GRANDVIEW_CONTRACTS}
        payloads[GRANDVIEW_CONTRACTS[0].provider_id]["Checksum"][0]["Value"] = "0" * 32
    ...
    def test_working_directory_rejects_unexpected_entries(self) -> None:
        with TemporaryDirectory() as directory:
            path = Path(directory)
            accepted = path / f"~${GRANDVIEW_CONTRACTS[0].expected_filename}.tmp"
    ...
    def test_cli_persists_secret_free_failure_state(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            args = SimpleNamespace(
                ...
                state_file=state,
                generated_at_utc="2026-07-20T18:20:00Z",
                run_id="BL-TEST-GRANDVIEW",
            )
            with patch.object(acquire_cli, "parse_args", return_value=args), patch.object(
                ...
                acquire_cli, "refresh_grandview_metadata", return_value={}
            ), patch.object(
                acquire_cli,
