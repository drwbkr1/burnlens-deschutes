"""Build the private reconciliation and public-safe Grandview intake report."""
...
from .grandview_owner_response_intake import (
...
def main() -> int:
    ...
    except (GrandviewOwnerResponseIntakeError, OSError, ValueError, KeyError) as error:
        print(f"GRANDVIEW_OWNER_RESPONSE_INTAKE_FAILED: {error}", file=sys.stderr)
        return 2
    print(report["decision"])
    print(f"grandview_owner_responses={sum(report['decision_counts'].values())}")
    print(
        "grandview_owner_approved_region_labels="
        f"{report['outcome']['grandview_owner_approved_region_labels']}"
    )
    print(
