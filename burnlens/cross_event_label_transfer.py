...
<section class="card"><h2>Boundaries and precedence</h2><p>{escape(report['source_precedence'])}</p><ul>{''.join(f'<li>{escape(item)}</li>' for item in report['claims']['not_proven'])}</ul><p>{escape(report['attribution'])}</p></section>
</main></body></html>"""
