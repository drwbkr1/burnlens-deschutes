"""Submit the exact Windigo reference request and preserve its private receipt."""
...
def main() -> int:
    ...
    try:
        ...
        print("WINDIGO_REFERENCE_REQUEST_ACCEPTED")
        print(f"mapping_ids={len(receipt['request']['mapping_ids'])}")
        print(f"mapping_products={len(receipt['request']['mapping_products'])}")
        print("delivery=PENDING_EMAIL_DELIVERY")
        return 0
    except (WindigoReferenceRequestError, OSError, ValueError) as error:
        print(f"WINDIGO_REFERENCE_REQUEST_FAILED: {error}", file=sys.stderr)
        return 2
