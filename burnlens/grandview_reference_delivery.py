"""Safe, identity-first preflight for delivered Grandview reference archives."""
...
CONTRACT_VERSION = "grandview-reference-delivery-preflight-v0.1.0"
EXPECTED_MAP_IDS = {10019092: "BAER", 10019464: "RAVG", 10023989: "MTBS"}
EXPECTED_EVENT_ID = "OR4446612140020210711"
MAX_ARCHIVES = 4
...
class GrandviewReferenceDeliveryError(RuntimeError):
    """Delivered Grandview archive preflight failed closed."""
...
def inspect_delivery(archives: list[Path]) -> dict[str, Any]:
    if not 1 <= len(archives) <= MAX_ARCHIVES:
        raise GrandviewReferenceDeliveryError("delivery must contain one to four archives")
    resolved = [path.resolve() for path in archives]
    if len(set(resolved)) != len(resolved):
        raise GrandviewReferenceDeliveryError("duplicate archive path")
    if any(path.suffix.lower() != ".zip" or not path.is_file() for path in resolved):
        raise GrandviewReferenceDeliveryError("every delivery input must be an existing ZIP")
    sizes = [path.stat().st_size for path in resolved]
    if any(size <= 0 or size > MAX_ARCHIVE_BYTES for size in sizes):
        raise GrandviewReferenceDeliveryError("archive size outside bounded range")
    if sum(sizes) > MAX_TOTAL_ARCHIVE_BYTES:
        raise GrandviewReferenceDeliveryError("total archive size exceeds bounded range")
    ...
    try:
        for path, size in zip(resolved, sizes, strict=True):
            with ZipFile(path) as archive:
                ...
                if not infos or len(infos) > MAX_MEMBERS_PER_ARCHIVE:
                    raise GrandviewReferenceDeliveryError("archive member count outside bounded range")
                if any(not _safe_member(name) for name in names):
                    raise GrandviewReferenceDeliveryError("unsafe archive member path")
                if len({name.casefold() for name in names}) != len(names):
                    raise GrandviewReferenceDeliveryError("case-insensitive duplicate member path")
                if any(item.flag_bits & 0x1 for item in infos):
                    raise GrandviewReferenceDeliveryError("encrypted archive member")
                if any(_is_symlink(item.external_attr) for item in infos):
                    raise GrandviewReferenceDeliveryError("archive symlink member")
                if any(name.lower().endswith((".zip", ".7z", ".rar")) for name in names):
                    raise GrandviewReferenceDeliveryError("nested archive member")
                uncompressed = sum(item.file_size for item in infos)
                if uncompressed > MAX_UNCOMPRESSED_BYTES_PER_ARCHIVE:
                    raise GrandviewReferenceDeliveryError("archive expansion exceeds bounded bytes")
                if uncompressed > max(size, 1) * MAX_EXPANSION_RATIO:
                    raise GrandviewReferenceDeliveryError("archive expansion ratio exceeds bound")
                bad = archive.testzip()
                if bad is not None:
                    raise GrandviewReferenceDeliveryError("archive CRC failure")
        ...
        raise GrandviewReferenceDeliveryError("delivery contains an unreadable ZIP") from error

    observed_map_ids = _map_ids_from_names(all_names)
    missing = sorted(set(EXPECTED_MAP_IDS) - observed_map_ids)
    if missing:
        raise GrandviewReferenceDeliveryError(f"missing expected Grandview map IDs: {missing}")
    ...
    if missing_programs:
        raise GrandviewReferenceDeliveryError(
            f"missing expected Grandview program tokens: {missing_programs}"
        )
    event_id_present = EXPECTED_EVENT_ID.casefold() in joined
    if not event_id_present:
        raise GrandviewReferenceDeliveryError("Grandview event ID is absent from member names")
    return {
        ...
        "archives": summaries,
        "decision": "PASS_SAFE_EXACT_GRANDVIEW_REFERENCE_DELIVERY_PREFLIGHT",
        "next_gate": (
...
def write_receipt(report: dict[str, Any], output_path: Path) -> None:
    if output_path.exists():
        raise GrandviewReferenceDeliveryError("preflight receipt exists; no overwrite allowed")
