...
<section class="basis"><h2>Read before deciding</h2><p>The fixed dNBR bin is a coherence partition, not a universal burn threshold. MTBS classes 2-4 and the multi-signal affirmative background route are candidate evidence, not independent ground truth. RAVG modeled classes remain context-only under the delivered sparse/non-tree warning and are not affirmative evidence. Gold pixels are uncertainty rings and cannot be approved by this surface.</p><p><strong>Exact-byte custody:</strong> export once to the hash-named response file. Do not overwrite, rename, or edit it. A later checkpoint must preserve and verify those exact bytes before considering any label.</p></section>
