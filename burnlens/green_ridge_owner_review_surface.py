"""Build a blank owner review surface for the exact Green Ridge proposals."""
...
SURFACE_ID = "GREEN-RIDGE-OWNER-REVIEW-SURFACE-2026-001"
SURFACE_VERSION = "green-ridge-owner-review-surface-v0.1.0"
RESPONSE_SCHEMA_VERSION = "burnlens-green-ridge-owner-review-response-v0.1.0"
TASK_ISSUE = 487
PROPOSAL_RUN_ID = "BL-2026-07-20-green-ridge-region-proposal-r001"
PROPOSAL_GENERATED_AT = "2026-07-20T01:25:00Z"
PROPOSAL_SOURCE_COMMIT = "690137f1530a00ca79b8f5831f700541809b2a2b"
...
EXPECTED_PROPOSAL_OUTPUTS = {
    f"{PROPOSAL_ID}.json": (4327, "b984ea5675f5114b615ab540552630847532a6492f0133ce80dd125b1c9d0281"),
    f"{PROPOSAL_ID}.html": (2081, "102751c749babe2853b8c2c44c61668c4b3a19922a22771c15bd5db8217a7e63"),
    f"{PROPOSAL_ID}.png": (67467, "cc5fbe18515e1c149ec6bd507a90fc5485b018d092ff9d075e7d43353dea57f1"),
    f"{PROPOSAL_ID}-GRP-001.tif": (3504, "6a1244b9f84917699100be0621c6e2916b6345a734e904e7b1c9b2f242af2f8f"),
    f"{PROPOSAL_ID}-GRP-002.tif": (3496, "0367f034a3df187a04523b9179a82be7e563a8a4bc00a7683669c9530a2812b8"),
}


class GreenRidgeOwnerReviewSurfaceError(RuntimeError):
...
def _verify_proposal(
    ...
    published = repository_root / "samples/labels/pilot/green-ridge/phase-two"
    bindings: dict[str, dict[str, Any]] = {}
    with TemporaryDirectory(prefix="burnlens-green-ridge-proposal-") as temporary:
        ...
        for name, (expected_bytes, expected_sha256) in EXPECTED_PROPOSAL_OUTPUTS.items():
            ...
            if tracked != expected or regenerated != expected:
                raise GreenRidgeOwnerReviewSurfaceError(f"proposal binding changed: {name}")
...
def _verify_raster(path: Path, candidate: dict[str, Any]) -> dict[str, Any]:
    with rasterio.open(path) as dataset:
        ...
        if dataset.crs is None or dataset.crs.to_epsg() != 32610:
            raise GreenRidgeOwnerReviewSurfaceError(f"{path.name} CRS changed")
        if domain - {0, 1, 2}:
            raise GreenRidgeOwnerReviewSurfaceError(f"{path.name} domain changed")
        if int((values == 1).sum()) != candidate["core_pixels"]:
            raise GreenRidgeOwnerReviewSurfaceError(f"{path.name} core count changed")
        if int((values == 2).sum()) != candidate["unknown_ring_pixels"]:
            raise GreenRidgeOwnerReviewSurfaceError(f"{path.name} ring count changed")
        if tags.get("label_created") != "false" or tags.get("run_id") != PROPOSAL_RUN_ID:
            raise GreenRidgeOwnerReviewSurfaceError(f"{path.name} promotion/run tags changed")
        return {
            "crs": "EPSG:32610",
    ...
    *, repository_root: Path, original_package: Path, extended_package: Path,
    ...
    for item in proposal["candidates"]:
        ...
        candidates.append({
            ...
            "limitations": [proposal["limitations"][0], proposal["limitations"][1]],
            "owner_decision": None,
    ...
    report = {
        ...
        "source_records": ["MANIFEST-2026-035", "MANIFEST-2026-036", "MANIFEST-2026-037"],
        "proposal_records": ["REGISTRY-2026-036", "LABEL_FITNESS-2026-023", "USE_BOUNDARY-2026-033", "MANIFEST-2026-038"],
        ...
        "warning": WARNING,
        "decision": "SURFACE_READY_FOR_GREEN_RIDGE_OWNER_REVIEW_KEEP_RESPONSE_INTAKE_SEPARATE",
...
def _agreement(previews: dict[str, Any]) -> np.ndarray:
    result = np.full(previews["route"].shape, 255, dtype=np.uint8)
    result[(previews["mtbs_dnbr6"] == 0) & (previews["ravg_cbi4"] == 0)] = 0
    result[np.isin(previews["mtbs_dnbr6"], (2, 3, 4)) & np.isin(previews["ravg_cbi4"], (2, 3, 4))] = 2
    return result
...
def _render_evidence_pages(selected: list[dict[str, Any]], previews: dict[str, Any], directory: Path) -> list[Path]:
    ...
    agreement = _evidence_image(_agreement(previews), "reference")
    paths: list[Path] = []
    for item in selected:
        ...
        dnbr = previews["pre_post_dnbr"] if item["candidate_class"] == "burned" else previews["dnbr"]
        sources = (pre, post, extended, _evidence_image(dnbr, "dnbr"), agreement)
        labels = ("pre-fire optical", "post-fire optical", "2021 extended optical", "dNBR coherence", "MTBS + RAVG agreement")
...
def _render_overview(report: dict[str, Any], pages: list[Path], path: Path) -> None:
    ...
    draw.text((55, 34), "BurnLens Green Ridge owner review", fill="white", font=_font(34))
...
<div class="candidate-head"><div><p class="eyebrow">{escape(item['candidate_id'])} / Green Ridge</p><h2>Is this exact region a usable prototype {escape(item['proposed_class'])} label candidate?</h2></div><span class="badge">unreviewed</span></div>
...
<div class="evidence-scroll" tabindex="0" aria-label="Scrollable five-panel evidence for {escape(item['candidate_id'])}"><img src="{escape(item['evidence_image'])}" width="1800" height="430" alt="{escape(item['candidate_id'])}: pre, post, extended optical, dNBR, MTBS and RAVG evidence with core and unknown ring"></div>
...
<fieldset><legend>Owner decision</legend><div class="choices">{choices}</div><label class="notes">Optional notes<textarea data-notes maxlength="1000"></textarea></label></fieldset></article>''')
    ...
    return f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>BurnLens Green Ridge owner review</title><style>
...
</style></head><body><header><p>BURNLENS / PHASE TWO / ISSUE #487</p><h1>Green Ridge owner-confirmed region review</h1><p>Review each exact unreviewed core and its excluded unknown ring against actual pre-fire, post-fire, extended optical, dNBR, MTBS, and RAVG evidence. Answer yes, no, or uncertain.</p></header><main>
...
<section class="basis"><h2>Read before deciding</h2><p>The fixed dNBR bin is a coherence partition, not a universal burn threshold. MTBS/RAVG agreement and the multi-signal background route are candidate evidence, not independent ground truth. Gold pixels are uncertainty rings and cannot be approved by this surface.</p></section>
...
def write_surface(report: dict[str, Any], selected: list[dict[str, Any]], previews: dict[str, Any], output_directory: Path) -> list[dict[str, Any]]:
    if output_directory.exists():
        raise GreenRidgeOwnerReviewSurfaceError("output directory already exists")
