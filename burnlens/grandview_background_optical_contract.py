"""Exact fail-closed acquisition contract for Grandview extended optical evidence."""
...
from .grandview_optical_contract import _metadata_url, _normalize_metadata
...
PACKAGE_ID = "grandview-s2-background-extended-v0.1.0"
CONTRACT_VERSION = "grandview-background-optical-intake-contract-v0.1.0"
...
EXTENDED_CONTRACT = AssetContract(
    role="grandview-2022-extended",
    provider="Copernicus Data Space Ecosystem",
    source_record_id=SOURCE_RECORD_ID,
    provider_id="43da9509-e01e-4c33-a85a-0ec45c04035a",
    native_id="S2B_MSIL2A_20220628T184919_N0510_R113_T10TFQ_20240628T114848.SAFE",
    expected_filename="S2B_MSIL2A_20220628T184919_N0510_R113_T10TFQ_20240628T114848.SAFE.zip",
    stable_route=(
        "https://download.dataspace.copernicus.eu/odata/v1/"
        "Products(43da9509-e01e-4c33-a85a-0ec45c04035a)/$value"
    ),
    expected_size_bytes=950_987_607,
    container="zip-safe",
    package_id=PACKAGE_ID,
    provider_md5="f42d0d1e618c712314947de897bd997d",
    provider_blake3="ad2747cb713d6f0cf9f1aba88241cd7c669c2e281b4e155d5ac2278158f7bda2",
    expected_zip_root="S2B_MSIL2A_20220628T184919_N0510_R113_T10TFQ_20240628T114848.SAFE",
)
BACKGROUND_OPTICAL_CONTRACTS = (EXTENDED_CONTRACT,)

EXPECTED_METADATA = {
    "acquisition_utc": "2022-06-28T18:49:19.024000Z",
    "platform_serial_identifier": "B",
    "tile_id": "10TFQ",
    "relative_orbit_number": 113,
    "processor_version": "05.10",
    "product_type": "S2MSI2A",
    "cloud_cover_percent": 0.169377,
}

ROUTE_PRECEDENCE = {
    "selection": (
        "Current CDSE STAC full-context coverage; Sentinel-2B, MGRS-10TFQ, relative orbit 113; "
        "closest low-cloud near-anniversary acquisition to the 2021-06-03 pre-fire scene in the bounded window."
    ),
    ...
    "baseline_boundary": (
        "The extended archive is processing baseline 05.10 while the original pair is 05.00; "
        "metadata-derived BOA scale and offset plus content registration must pass before spectral comparison."
    ),
...
def validate_background_optical_contracts(contracts: Iterable[AssetContract]) -> list[str]:
    ...
    if any("S2B_MSIL2A_20220628T184919_N0510_R113_T10TFQ" not in item.native_id for item in items):
...
def validate_background_optical_metadata(snapshot: dict[str, Any]) -> list[str]:
    ...
    expected = {
        ...
        "PROVIDER_ID": EXTENDED_CONTRACT.provider_id,
        "NATIVE_ID": EXTENDED_CONTRACT.native_id,
        "SIZE": EXTENDED_CONTRACT.expected_size_bytes,
    ...
    if checksums.get("MD5") != EXTENDED_CONTRACT.provider_md5:
        reasons.append("MD5")
    if checksums.get("BLAKE3") != EXTENDED_CONTRACT.provider_blake3:
        reasons.append("BLAKE3")
    s3_path = record.get("s3_path")
    if not isinstance(s3_path, str) or not s3_path.endswith(f"/{EXTENDED_CONTRACT.native_id}"):
