"""Fail-closed intake for the exact Green Ridge owner response."""
...
import burnlens
from .green_ridge_owner_review_surface import (
    ALLOWED_DECISIONS,
...
from .green_ridge_region_proposal import REPORT_ID as PROPOSAL_ID


REPORT_ID = "GREEN-RIDGE-OWNER-RESPONSE-INTAKE-2026-001"
REPORT_VERSION = "green-ridge-owner-response-intake-v0.1.0"
PRIVATE_REPORT_VERSION = "green-ridge-owner-response-private-reconciliation-v0.1.0"
LOCK_VERSION = "green-ridge-owner-response-exact-byte-lock-v0.1.0"
LABEL_SET_VERSION = "owner-approved-prototype-region-labels-v0.2.0"
PRIOR_LABEL_SET_VERSION = "owner-approved-prototype-region-labels-v0.1.0"
LABEL_SCHEMA_VERSION = "burn-scar-five-state-schema-v0.1.0"
AOI_VERSION = "multi-event-native-grids-v0.2.0"
TARGET_VERSION = "target-burn-scar-v0.2.0"
TASK_ISSUE = 491
MINIMUM_EVENT_GROUPS = 6
EXPECTED_SURFACE_SHA256 = "d965658a54398642cc5c8d29a6a19d750e6345c46951e023bb81e8e9359a40eb"
PRIOR_INTAKE_SHA256 = "b3ee04e682c99113aa9189a905c160f99be77a98c23255eb69fa7a7c46b1dfcf"
SOURCE_RECORDS = (
    "SOURCE-2026-020",
    "SOURCE-2026-021",
    "SOURCE-2026-022",
...
REVIEW_RECORDS = (
    "PRECHECK-2026-036",
    "LABEL-FITNESS-2026-023",
    "LABEL-FITNESS-2026-024",
    "SOURCE-PRECEDENCE-2026-015",
    "USE-BOUNDARY-2026-033",
    "USE-BOUNDARY-2026-034",
)
MANIFEST_RECORDS = tuple(f"MANIFEST-2026-{value:03d}" for value in range(35, 40))
WARNING = (
...
class GreenRidgeOwnerResponseIntakeError(RuntimeError):
    """Raised when response custody or any promotion gate fails closed."""


def _assert(condition: bool, message: str) -> None:
    if not condition:
        raise GreenRidgeOwnerResponseIntakeError(message)
...
def _json(data: bytes, name: str) -> dict[str, Any]:
    try:
        ...
        raise GreenRidgeOwnerResponseIntakeError(f"invalid UTF-8 JSON: {name}") from error
...
def _timestamp(value: Any, name: str) -> datetime:
    ...
    try:
        ...
        raise GreenRidgeOwnerResponseIntakeError(f"{name} is invalid") from error
...
def preserve_response(
    *,
    ...
    try:
        ...
        raise GreenRidgeOwnerResponseIntakeError("custody destination is outside the repository") from error
    ...
    receipt = {
        ...
        "evidence_origin": "owner-returned-green-ridge-response",
        ...
        "model_version": None,
        "decision": "PASS_EXACT_GREEN_RIDGE_OWNER_RESPONSE_LOCK_DEFER_PROMOTION_GATES",
    }
    ...
    try:
        ...
        if wrote_response and exact_path.exists():
            exact_path.unlink()
        raise GreenRidgeOwnerResponseIntakeError("refusing to overwrite exact response or receipt") from error
    except Exception:
        if wrote_response and exact_path.exists():
            exact_path.unlink()
...
def _record_bindings(repository_root: Path) -> list[dict[str, Any]]:
    ...
    for record_id in (*SOURCE_RECORDS, *TERMS_RECORDS, *REVIEW_RECORDS, *MANIFEST_RECORDS):
        ...
        if record_id == "TERMS-2026-017":
            _assert("RESOLVED_FOR_PRIVATE_EXACT_BUNDLE_REQUEST" in text, "Green Ridge reference request terms are unresolved")
        if record_id == "TERMS-2026-018":
            _assert("access constraints are `None`" in text and "thresholded preliminary" in text, "delivered reference terms changed")
        if record_id == "TERMS-2026-019":
            _assert("No unresolved licensing or terms issue blocks exact acquisition" in text, "extended optical terms are unresolved")
...
def _validate_receipt(receipt: dict[str, Any], response_bytes: bytes, surface_bytes: bytes) -> None:
    ...
    _assert(receipt.get("evidence_origin") == "owner-returned-green-ridge-response", "receipt origin changed")
...
def _load_prior_intake(repository_root: Path) -> tuple[dict[str, Any], bytes]:
    path = repository_root / "samples/labels/review/regions/phase-two/intake/REGION-OWNER-RESPONSE-INTAKE-2026-001.json"
    ...
    _assert(report.get("outcome", {}).get("owner_approved_region_labels") == 6, "prior accepted label count changed")
    _assert(report.get("outcome", {}).get("event_group_count") == 3, "prior event count changed")
...
def build_private_reconciliation(
    *,
    ...
    _assert(excluded_ring_pixels == 87, "Green Ridge unknown-ring aggregate changed")
    prior_outcome = prior["outcome"]
    cumulative_classes = Counter(prior_outcome["prototype_label_class_counts"])
    cumulative_classes.update(added_class_counts)
    cumulative_labels = prior_outcome["owner_approved_region_labels"] + added_labels
    cumulative_core_pixels = prior_outcome["accepted_core_pixels"] + accepted_core_pixels
    cumulative_ring_pixels = prior_outcome["excluded_unknown_ring_pixels"] + excluded_ring_pixels
    ...
    return {
        ...
        "outcome": {
            "green_ridge_owner_approved_region_labels": added_labels,
            "green_ridge_class_counts": dict(sorted(added_class_counts.items())),
            "green_ridge_accepted_core_pixels": accepted_core_pixels,
            "green_ridge_excluded_unknown_ring_pixels": excluded_ring_pixels,
        ...
        "decision": (
            "ACCEPT_GREEN_RIDGE_OWNER_APPROVED_PROTOTYPE_LABELS_DEFER_DATASET_SPLIT_BASELINE_MODEL"
            if added_labels
            else "NO_NEW_GREEN_RIDGE_OWNER_APPROVED_PROTOTYPE_LABELS"
        ),
    }
...
def public_report(private: dict[str, Any], private_binding: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "attribution": [
            ...
            "RAVG evidence: USDA Forest Service and ESA; program-specific timing, vegetation, and model limitations apply.",
        ],
        "warning": WARNING,
        "decision": private["decision"],
        "next_gate": "Acquire, review, and gate at least two additional comparable whole-event groups before dataset fitness, split, or baseline work can reopen.",
    }
...
def _render_png(report: dict[str, Any], path: Path) -> None:
    ...
    draw.text((70, 42), "BurnLens Green Ridge owner response", fill="white", font=_font(42))
    ...
    metrics = (
        (str(sum(report["decision_counts"].values())), "Green Ridge responses"),
        (str(outcome["green_ridge_owner_approved_region_labels"]), "new prototype labels"),
...
def _render_html(report: dict[str, Any]) -> str:
    ...
    outcome = report["outcome"]
    return f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>BurnLens Green Ridge owner response intake</title><style>
...
</style></head><body><header><p>BURNLENS / PHASE TWO / ISSUE #491</p><h1>Green Ridge owner response intake</h1><p>Exact private custody, deterministic gate evaluation, and content-safe public aggregates.</p></header><main>
<p class="warning">{escape(report['warning'])}</p><section class="metrics"><div class="metric"><strong>{sum(counts.values())}</strong>Green Ridge responses</div><div class="metric"><strong>{outcome['green_ridge_owner_approved_region_labels']}</strong>new prototype labels</div><div class="metric"><strong>{outcome['cumulative_owner_approved_region_labels']}</strong>cumulative labels</div><div class="metric"><strong>{outcome['event_group_count']}</strong>accepted event groups</div></section>
<section class="card"><h2>Aggregate result</h2><p>Owner decisions: {counts['yes']} yes / {counts['no']} no / {counts['uncertain']} uncertain. Green Ridge adds {outcome['green_ridge_class_counts'].get('burned',0)} burned and {outcome['green_ridge_class_counts'].get('background',0)} background prototype labels.</p><p>Every admitted core passes exact-byte reconstruction, source and terms, native-grid quality, explicit uncertainty-ring exclusion, and event-identity gates. Notes and unit decisions remain private.</p></section>
<img src="{REPORT_ID}.png" alt="BurnLens Green Ridge owner response intake gate summary">
...
def write_private_no_overwrite(repository_root: Path, path: Path, report: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        raise GreenRidgeOwnerResponseIntakeError("refusing to overwrite private reconciliation") from error
...
def write_public_no_overwrite(report: dict[str, Any], output_directory: Path) -> list[dict[str, Any]]:
    if output_directory.exists():
        raise GreenRidgeOwnerResponseIntakeError("public output directory already exists")
