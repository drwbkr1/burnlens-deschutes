"""Capture the exact private Ward Creek MTBS delivery without retaining its URL."""
...
from .replacement_event_reference_custody import (
...
def main() -> int:
    ...
    try:
        ...
        return 0
    except (WardCreekReferenceCustodyError, OSError, ValueError) as error:
        print(
            f"WARD_CREEK_REFERENCE_CUSTODY_FAILED: {type(error).__name__}",
            file=sys.stderr,
        )
        return 2
    finally:
