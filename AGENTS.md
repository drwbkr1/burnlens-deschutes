...
P2O4-T01 through P2O4-T04 ship verified one-event and cross-event five-state proposal evidence, source fitness, all-pixel software QA, and topology-stable provenance through `v0.12.1-topology-stable-label-transfer`. P2O4-T05 / issue #375 / PR #376 ships verified `v0.13.0-label-review-readiness`: 56 proposal-blinded units cover 14 present event/state strata. P2O4-T06 / issue #379 / PR #380 ships verified `v0.14.0-offline-reviewer-handoff`: it packages the exact blind packet into a deterministic isolated offline workbench, independently verifies the archive/interface contract, and locks returned response bytes before reveal. P2O4-T07 / issue #383 / PR #385 ships verified `v0.15.0-live-browser-reviewer-handoff`: the exact extracted workbench passes a recorded current-Chrome invalid-state, draft download/load, full response-export, responsive-viewport, console/runtime, and software-fixture-lock run. The fixture is explicitly non-human and cannot authorize reveal. P2O4-T08 / issue #384 / PR #388 merged BurnLens `0.16.0` evidence: one actual returned response and its private receipt are exact, ignored, and operator-locked; a bounded public JSON/HTML/PNG receipt exposes only hashes, completeness, packet binding, chronology, content withholding, and the one-of-two state. Post-merge audit withheld the tag and issue #389 corrects only manifest chronology and release synchronization. A second qualifying response is still required and the reveal remains operator-declared unopened. No adjudication, accepted label set, dataset, split, baseline, model, deployed analytical application, or independent scientific fitness decision exists.
