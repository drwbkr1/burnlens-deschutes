...
P2O4-T01 / issue #353 / PR #354 is shipped at verified `v0.9.0-label-proposal-baseline`: BurnLens `0.9.0` implements a native-grid five-state proposal and a separate all-pixel software verifier. The proposal preserves 33.4144% explicit ignore; all 270,000 state and target pixels and all 120 audit samples agree. P2O4-T02 / issue #357 is the accepted BurnLens `0.10.0` working candidate: current MTBS/Census/CDSE metadata selects Tepee and McKay exact acquisition pairs, excludes Milli at a tile seam, and freezes whole event/scene/geography/time groups before acquisition. It is not shipped until its PR/merged-main/tag gates pass, and it creates no dataset or split.
