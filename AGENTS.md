...
P2O4-T01 through P2O4-T04 ship verified one-event and cross-event five-state proposal evidence, source fitness, all-pixel software QA, and topology-stable provenance through `v0.12.1-topology-stable-label-transfer`. P2O4-T05 / issue #375 / PR #376 ships verified `v0.13.0-label-review-readiness`: 56 proposal-blinded units cover 14 present event/state strata, with zero completed independent responses or adjudications. P2O4-T06 / issue #379 is the active BurnLens `0.14.0` candidate: it packages the exact blind packet into a deterministic isolated offline workbench, independently verifies the archive/interface contract, and locks returned response bytes before reveal. The candidate does not count its software fixture as human evidence. An accepted label set, dataset, split, baseline, model, deployed application, and independent human review still do not exist.
